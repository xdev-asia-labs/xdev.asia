...
title: 'レッスン 11: 構造化データのクエリ — Text-to-SQL、ナレッジ グラフ、およびデータベース エージェント'
slug: bai-11-structured-data-querying
description: >-
  Text-to-SQL パイプライン、スキーマ インジェクション、SQL 検証とサニタイズ、ナレッジ グラフ クエリ、データベース エージェント
  パターン、結果の書式設定、自然言語 BI。
duration_minutes: 90
is_free: true
video_url: null
sort_order: 11
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="60" y="140" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">レッスン 11: 構造化データのクエリ —</tspan>
      <tspan x="60" dy="42">テキストから SQL、ナレッジ グラフ、データベースへの変換</tspan>
...
<h2 id="1-text-to-sql-overview"><strong>1. Text-to-SQL — 質問をクエリに変換します</strong></h2>

<p>Text-to-SQL を使用すると、チャットボットが自然言語からデータベースに直接クエリを実行できます (「今月の収益はいくらですか?」)。 → <code>注文の場所から SUM(金額) を選択してください...</code>。</p>
...
<h2 id="2-schema-injection"><strong>2. スキーマの挿入 — LLM へのデータベース コンテキストの提供</strong></h2>
...
<h2 id="3-sql-generator"><strong>3. 安全性を備えた SQL ジェネレーター</strong></h2>
...
<h2 id="4-read-only-execution"><strong>4. 読み取り専用実行層</strong></h2>
...
<h2 id="5-knowledge-graph"><strong>5. ナレッジグラフのクエリ</strong></h2>
...
<h2 id="6-result-formatting"><strong>6. 自然言語結果のフォーマット</strong></h2>
...
<h2 id="tong-ket"><strong>レッスン 11 のまとめ</strong></h2>

<ul>
<li><strong>テキストから SQL へ</strong>: スキーマインジェクション → SQL 生成 → 検証 → 読み取り専用実行</li>
<li><strong>安全性</strong>: 読み取り専用ユーザー、ステートメントのタイムアウト、テナント フィルター、ブロックされたキーワード、LIMIT が必要</li>
<li><strong>ナレッジグラフ</strong>: エンティティ抽出 → 暗号生成 → グラフクエリ + ベクトルハイブリッド</li>
<li><strong>結果のフォーマット</strong>: 形式の自動検出 (単一値、テーブル、時系列) → ベトナム語ローカリゼーション</li>
<li><strong>行レベルのセキュリティ</strong>:PostgreSQL RLS+ <code>app.tenant_id を設定する</code> データ分離を確実にする</li>
</ul>

<p><strong>次の記事:</strong> ガードレールと AI の安全性 — 入出力フィルタリング、毒性検出、PII マスキング、コンテンツ管理、ジェイルブレイク防止。</p>
