...
title: 'レッスン 12: ガードレールと AI の安全性 — 入力/出力フィルタリング、PII マスキング、脱獄防止'
slug: bai-12-guardrails-ai-safety
description: >-
  入出力ガードレール、毒性検出、PII
  マスキング、コンテンツ管理、ジェイルブレイク防止、即時インジェクション防御、安全性スコアリング、コンプライアンス監査証跡。
duration_minutes: 90
is_free: true
video_url: null
sort_order: 12
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="60" y="140" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">レッスン 12: ガードレールと AI の安全性 —</tspan>
      <tspan x="60" dy="42">入力/出力フィルタリング、PII マスキング、</tspan>
      <tspan x="60" dy="42">脱獄防止</tspan>
  </text>
...
<h2 id="1-guardrail-architecture"><strong>1. ガードレールアーキテクチャの概要</strong></h2>

<p>ガードレールは <strong>必須の保護層</strong> エンタープライズ チャットボット — 入力 (ユーザー送信) と出力 (LLM 返される) の両方をフィルタリングして、安全性、コンプライアンス、品質を確保します。</p>
...
<h2 id="2-guardrail-engine"><strong>2. ガードレール エンジンの実装</strong></h2>
...
<h2 id="3-jailbreak-prevention"><strong>3. 脱獄と即時注入の防止</strong></h2>
...
<h2 id="4-pii-masking"><strong>4. PII の検出とマスキング</strong></h2>
...
<h2 id="5-toxicity-detection"><strong>5. 有害性とコンテンツの管理</strong></h2>
...
<h2 id="6-topic-restriction"><strong>6. トピックの制限とブランドの調整</strong></h2>
...
<h2 id="tong-ket"><strong>レッスン 12 のまとめ</strong></h2>

<ul>
<li><strong>ガードレールパイプライン</strong>: 入力ガードレール → LLM → 出力ガードレール、各ルールには重大度 (ブロック/警告/ログ) があります。</li>
<li><strong>脱獄防止</strong>: 3 層 — 正規表現パターン、LLM 分類、埋め込み類似性</li>
<li><strong>PII マスキング</strong>: 電話/メール/ID カードの正規表現 + 名前/住所の NER</li>
<li><strong>毒性</strong>: OpenAI Moderation API が有害なコンテンツを検出</li>
<li><strong>トピックの制限</strong>: LLM ベースの許可/禁止トピックのチェック + ブランドの調整</li>
<li>すべてがそこにあります <strong>監査ログ</strong> コンプライアンス報告用</li>
</ul>

<p><strong>次の記事:</strong> ナレッジベース管理 — ドキュメントの取り込み、マルチフォーマット解析、ナレッジライフサイクル、バージョン管理、ドキュメントごとのアクセス制御。</p>
