...
description: >-
  ...
  データベース接続構成、WebAPI REST エンドポイント (ソース、語彙、コホート定義、IR、推定)、認証/認可、およびマルチソース構成。
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="92" y="69" font-family="system-ui,-apple-system,sans-serif" font-size="13" font-weight="600" fill="#fbbf24">🏗️ アーキテクチャ — レッスン 8</text>

  <!-- Title -->
  <text x="60" y="140" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">レッスン 8: WebAPI — インストール、構成、REST</tspan>
...
![レッスン 8: WebAPI — REST API アーキテクチャ](/storage/uploads/2026/03/ohdsi-bai-8-webapi-architecture.png)

## はじめに

**WebAPI** は ATLAS の REST API バックエンドであり、Java/Spring Boot で書かれており、OHDSI エコシステムにすべての分析サービスを提供します。 ATLAS (フロントエンド) は、WebAPI を通じて完全に CDM データベースと通信します。
...
|コンセプト |説明 |
|----------|----------|
|ウェブAPI | ATLAS 用バックエンド REST API (Spring Boot/Java) |
| WebAPI DB |データベースには、構成、コホート定義、分析設定が保存されます。
| CDM大門 |スキーマには臨床データが含まれています (読み取り専用) |
|結果 大門 |スキーマにはコホート結果が含まれています (読み取り/書き込み) |
|出典 | 1 つの CDM データベース接続 (マルチソースをサポート) |
|フライウェイ |データベース移行ツール（WebAPIスキーマ自作） |

**次の記事**: ATLAS — インストール、WebAPI 統合、インターフェイスの概要
