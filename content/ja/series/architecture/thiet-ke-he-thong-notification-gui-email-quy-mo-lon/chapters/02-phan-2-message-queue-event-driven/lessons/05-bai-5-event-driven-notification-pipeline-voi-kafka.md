...
description: >-
  通知イベントのイベント スキーマを設計します。トピックデザイン、Kafka
  Connect、ストリーム処理、イベントソーシング。完全なパイプラインを実践的に構築します。
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="92" y="69" font-family="system-ui,-apple-system,sans-serif" font-size="13" font-weight="600" fill="#34d399">🏗️ アーキテクチャ — レッスン 5</text>

  <!-- Title -->
  <text x="60" y="140" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">レッスン 5: イベント駆動型の通知パイプライン</tspan>
      <tspan x="60" dy="42">カフカと</tspan>
  </text>
...
前のレッスンでは、Message Queue がバックボーンであることを理解しました。次に、通知リクエストから配信追跡まで、すべて Kafka イベントを通じて、**完全なイベント駆動型パイプライン**を構築します。
...
## 3. Kafka Connect — データ統合

### ソースコネクタ: データベース → Kafka
...
完全なイベント駆動型パイプラインが構築されました。
- **イベント スキーマ** はすべての通知イベントに対して標準化されています
- **マルチトピック アーキテクチャ** は、送信、ステータス、Webhook、DLQ を分離します。
- **ストリーム処理**によるリアルタイム集約
- **イベント ソーシング**による完全な監査証跡

**次の記事:** 優先キューとスケジューリング エンジン - 適切なタイミングで、適切な優先順位で電子メールを送信します。
