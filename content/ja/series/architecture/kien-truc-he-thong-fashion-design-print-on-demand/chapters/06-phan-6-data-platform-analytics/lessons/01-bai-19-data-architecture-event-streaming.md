...
title: 'レッスン 19: データ アーキテクチャとイベント ストリーミング — イベント ソーシング、Kafka、データ レイク、リアルタイム処理'
slug: bai-19-data-architecture-event-streaming
description: >-
  POD 用のデータ プラットフォーム アーキテクチャ、イベント ソーシング、Apache Kafka イベント ストリーミング、CDC
  (Debezium)、データ レイク (S3 + Iceberg)、リアルタイム処理 (Flink)、データ ガバナンス、スキーマ レジストリ、データ
  リネージ。
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="92" y="69" font-family="system-ui,-apple-system,sans-serif" font-size="13" font-weight="600" fill="#f87171">🏗️ アーキテクチャ — レッスン 19</text>

  <!-- Title -->
  <text x="60" y="140" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">レッスン 19: データ アーキテクチャとイベント</tspan>
      <tspan x="60" dy="42">ストリーミング — イベント ソーシング、Kafka、データ</tspan>
      <tspan x="60" dy="42">レイクとリアルタイム処理</tspan>
  </text>
  ...
  <!-- Section -->
  <text x="60" y="310" font-family="system-ui,-apple-system,sans-serif" font-size="13" fill="#64748b" opacity="0.6">パート 6: データ プラットフォームと分析</text>

  <!-- xDev watermark -->
  <text x="1140" y="320" font-family="system-ui,-apple-system,sans-serif" font-size="12" fill="#475569" text-anchor="end" opacity="0.4">xdev.asia</text>
</svg>

<h2 id="1-data-architecture"><strong>1. データアーキテクチャの概要</strong></h2>
...
<h2 id="2-event-sourcing"><strong>2. イベントソーシングパターン</strong></h2>
...
<h2 id="3-kafka-configuration"><strong>3. Kafka イベントストリーミング</strong></h2>
...
<h2 id="4-cdc-debezium"><strong>4. Debezium を使用した CDC (変更データ キャプチャ)</strong></h2>
...
<h2 id="5-stream-processing"><strong>5. ストリーム処理 (Apache Flink)</strong></h2>
...
<h2 id="6-data-lake"><strong>6. データレイクアーキテクチャ</strong></h2>
...
<h2 id="7-schema-registry"><strong>7. スキーマレジストリとデータガバナンス</strong></h2>
...
<table>
<thead>
<tr><th>コンポーネント</th><th>テクノロジー</th><th>目的</th></tr>
</thead>
<tbody>
<tr><td>イベントストリーミング</td><td>Apache Kafka (3 ノード)</td><td>すべてのサービスの信頼できるイベント バックボーン</td></tr>
<tr><td>CDC</td><td>Debezium + PostgreSQL</td><td>DB の変更 → Kafka トピックが自動的に変更される</td></tr>
<tr><td>ストリーム処理</td><td>アパッチフリンク</td><td>リアルタイムの集計、不正行為の検出</td></tr>
<tr><td>データレイク</td><td>S3 + アパッチ アイスバーグ</td><td>ブロンズ/シルバー/ゴールド メダリオン建築</td></tr>
<tr><td>スキーマレジストリ</td><td>Confluent スキーマ レジストリ</td><td>スキーマの進化、互換性の検証</td></tr>
<tr><td>データガバナンス</td><td>データハブ/Apache アトラス</td><td>データ系統、カタログ、アクセス制御</td></tr>
</tbody>
</table>
