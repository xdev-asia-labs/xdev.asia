...
description: >-
  垂直分割と水平分割。シャーディング戦略:
  範囲ベース、ハッシュベース、ディレクトリベース。一貫したハッシュ。クロスシャードクエリと分散トランザクション。再シャーディング戦略。実践的なシャーディング
  スキームの設計。
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="60" y="160" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">レッスン 11: データベースのシャーディングとパーティショニング</tspan>
  </text>
  ...
  <!-- Section -->
  <text x="60" y="246" font-family="system-ui,-apple-system,sans-serif" font-size="13" fill="#64748b" opacity="0.6">パート 3: データベース アーキテクチャとデータ管理</text>
...
データベースが数百 GB または TB に膨れ上がると、1 台のサーバーでは処理できなくなります。 **シャーディング** では、データを小さな部分に分割し、各部分を別のサーバーに配置します。
...
|戦略 |データ配布 |範囲クエリ |シャードの追加/削除 |複雑さ |
|----------|--------|---------------|-------------|----------|
|範囲 |逸脱する可能性があります |良い |簡単 |低い |
|ハッシュ |すべて |悪い |難しい（焼き直し） |平均 |
|一貫したハッシュ |すべて |悪い |簡単 (データ移動が少ない) |曹操 |
|ディレクトリ |カスタム |カスタムマッピング |柔軟 |曹操 |

---

## 演習

1. **シャード キーの選択:** 1 億ユーザーを抱えるソーシャル メディア システム、機能: ユーザー プロフィール、投稿、コメント、いいね。各テーブルのシャードキーを選択して説明します。

2. **一貫性のあるハッシュ:** 4 つのノードで一貫性のあるハッシュ リングを描画し、5 番目のノードを追加します。移動する必要があるデータの割合を計算します。

3. **再シャーディング計画:** 4 つのシャードがあり、シャード 2 がデータの 60% を占めます (時間の経過に伴う範囲ベースのためホット スポット)。ダウンタイムゼロの再シャード計画を作成します。
