...
description: >-
  FHIR サーバーのパフォーマンス、キャッシュ戦略、データベースのインデックス作成、水平スケーリング、ロード バランシング、静的コンテンツの
  CDN、Prometheus/Grafana による監視、ヘルス チェック、ヘルス SLA を最適化します。
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="92" y="69" font-family="system-ui,-apple-system,sans-serif" font-size="13" font-weight="600" fill="#c084fc">🏗️ アーキテクチャ — レッスン 22</text>

  <!-- Title -->
  <text x="60" y="140" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">レッスン 22: FHIR のパフォーマンス、スケーラビリティ、および</tspan>
...
<h2 id="1-performance-challenges"><strong>1. 医療におけるパフォーマンスの課題</strong></h2>

<p>医療用 FHIR サーバー システムは、稼働時間 ≥ 99.9%、応答時間 < 500ms、数百万のリソースなどの高い要件を満たす必要があります。</p>

<table>
<thead>
<tr><th>因子</th><th>チャレンジ</th><th>ターゲット</th></tr>
</thead>
<tbody>
<tr><td>応答時間</td><td>大規模なデータセットに対する複雑な検索</td><td>< 500ms (P95)</td></tr>
<tr><td>スループット</td><td>多くの病院が同時に問い合わせる</td><td>≥ 1000 リクエスト/秒</td></tr>
<tr><td>データ量</td><td>何百万もの患者、何十億もの観察</td><td>結核スケール</td></tr>
<tr><td>可用性</td><td>24時間365日の救急対応</td><td>≥ 99.9%</td></tr>
<tr><td>一貫性</td><td>医療データは正確でなければなりません</td><td>強い一貫性</td></tr>
</tbody>
</table>

<h2 id="2-database-optimization"><strong>2. データベースの最適化</strong></h2>

<h3 id="postgresql-indexing"><strong>HAPI FHIR の PostgreSQL インデックス作成</strong></h3>
...
<h3 id="query-optimization"><strong>クエリの最適化</strong></h3>
...
<h2 id="3-caching"><strong>3. キャッシュ戦略</strong></h2>
...
<h3 id="http-caching"><strong>HTTP キャッシュヘッダー</strong></h3>
...
<h2 id="4-horizontal-scaling"><strong>4. 水平方向のスケーリング</strong></h2>
...
<h2 id="5-monitoring"><strong>5. Prometheus + Grafana によるモニタリング</strong></h2>

<h3 id="metrics-exposure"><strong>メトリクスの公開</strong></h3>
...
<h3 id="prometheus-config"><strong>プロメテウス構成</strong></h3>
...
<h3 id="key-metrics"><strong>主要な指標は追跡が必要です</strong></h3>

<table>
<thead>
<tr><th>メトリック</th><th>説明</th><th>アラートしきい値</th></tr>
</thead>
<tbody>
<tr><td>http_server_requests_秒_p95</td><td>P95 応答時間</td><td>> 500ミリ秒</td></tr>
<tr><td>http_server_requests_total</td><td>リクエストレート</td><td>異常検知</td></tr>
<tr><td>jvm_memory_used_bytes</td><td>JVM ヒープ使用量</td><td>> 80%</td></tr>
<tr><td>hikaricp_connections_active</td><td>アクティブな DB 接続</td><td>> 80% のプール</td></tr>
<tr><td>fhir_search_duration_秒</td><td>検索クエリ時間</td><td>> 1秒</td></tr>
<tr><td>fhir_resource_count</td><td>総リソース</td><td>成長率</td></tr>
<tr><td>pg_stat_activity_count</td><td>PostgreSQL 接続</td><td>> 最大 90%</td></tr>
<tr><td>ディスク使用率_パーセント</td><td>ディスク容量</td><td>> 85%</td></tr>
</tbody>
</table>

<h3 id="health-check"><strong>ヘルスチェックエンドポイント</strong></h3>
...
<h2 id="6-alerting"><strong>6. アラートルール</strong></h2>
...
<ul>
<li><p><strong>データベースの最適化</strong> — PostgreSQL のインデックス作成、パーティショニング、接続プーリング</p></li>
<li><p><strong>キャッシング</strong> — メタデータ/ValueSet 用の Redis、Nginx リバース プロキシ キャッシュ</p></li>
<li><p><strong>水平スケーリング</strong> — 複数の FHIR インスタンス + ロード バランサー + PgPool</p></li>
<li><p><strong>モニタリング</strong> — Prometheus メトリクス、Grafana ダッシュボード、アラート ルール</p></li>
<li><p><strong>健康診断</strong> — カスタム HealthIndicator、readiness/liveness プローブ</p></li>
<li><p><strong>医療 SLA</strong> — 稼働時間 ≥ 99.9%、P95 < 500ms、監査証跡 7 年以上</p></li>
</ul>
