...
<text x="92" y="69" font-family="system-ui,-apple-system,sans-serif" font-size="13" font-weight="600" fill="#38bdf8">🔒 D​​evSecOps — レッスン 16</text>

  <!-- Title -->
  <text x="60" y="160" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
<tspan x="60" dy="0">レッスン 16: SSH シークレット エンジンと TOTP</tspan>
  </text>
...
<text x="60" y="222" font-family="system-ui,-apple-system,sans-serif" font-size="15" fill="#94a3b8" opacity="0.8">HashiCorp Vault の基本から上級まで</text>
  ...
  <!-- Section -->
<text x="60" y="246" font-family="system-ui,-apple-system,sans-serif" font-size="13" fill="#64748b" opacity="0.6">パート 4: 高度なシークレット エンジン</text>
...
<p><strong>SSH Secrets Engine</strong> は、企業における SSH アクセスの管理の問題を解決します。 SSH キーを手動で配布する (そして決して取り消さない) 代わりに、Vault は 2 つのメカニズムを提供します。</p>

<table>
<thead>
<tr><th>モード</th><th>仕組み</th><th>推奨</th></tr>
</thead>
<tbody>
<tr><td><strong>署名付き証明書 (CA)</strong></td><td>Vault 署名 SSH 公開キー → 短期証明書</td><td>✅ 推奨</td></tr>
<tr><td><strong>OTP</strong></td><td>Vault は SSH</td><td>Legacy へのワンタイム パスワードを生成します。vault-ssh-helper</td></tr> が必要です
</tbody>
</table>

<h2 id="2-ssh-ca-mode"><strong>2. SSH CA Mode (Signed Certificates)</strong></h2>

<h3 id="kien-truc-ssh-ca"><strong>アーキテクチャ</strong></h3>
...
<h3 id="cau-hinh-server"><strong>Vault CA を信頼するように SSH サーバーを構成する</strong></h3>
...
<h3 id="tao-role-ssh"><strong>役割の作成</strong></h3>
...
<p><strong>注:</strong> OTP モードでは、Vault で OTP を検証するためにターゲット サーバーに <code>vault-ssh-helper</code> をインストールする必要があります。 CA モードでは、サーバー上で追加の設定は必要ありません。</p>

<h2 id="4-totp-secrets-engine"><strong>4. TOTP Secrets Engine</strong></h2>

<p><strong>TOTP Secrets Engine</strong> により、Vault は RFC 6238 に従って TOTP (時間ベースのワンタイム パスワード) コードを生成および検証できます。</p>
...
<p>各従業員が各自の電話機で 2FA コードを管理する代わりに、Vault を TOTP キーの中央保管場所として使用できます。これにより、チームのローテーションが可能になり、電話機を変更するときにコードを失うことがなくなります。</p>

<h2 id="5-ldap-secrets-engine"><strong>5. LDAP Secrets Engine</strong></h2>

<p><strong>LDAP シークレット エンジン</strong> (LDAP 認証方法とは異なります) <strong>動的 LDAP 資格情報</strong> を生成します — LDAP サービス アカウントを自動的に作成および管理します。</p>
...
<p>Vault 1.21 では、メインフレーム環境に RACF パスフレーズのサポートが追加され、従来の 8 文字より長いパスフレーズが許可されます:</p>
...
<ul>
<li><p><strong>SSH CA モード</strong> — 推奨、短期署名証明書、サーバーにエージェントは不要</p></li>
<li><p><strong>SSH OTP モード</strong> — ワンタイム パスワード、vault-ssh-helper</p></li> が必要
...
<li><p><strong>RACF パスフレーズ</strong> (1.21) — メインフレーム環境サポート</p></li>
</ul>

<p>次の記事では、変換とトークン化 — PCI DSS、PII、およびコンプライアンス要件のデータ保護について説明します。</p>
