...
description: KMIP Secrets Engine、Consul Secrets Engine、Nomad Secrets Engine、Vault Plugin System — アーキテクチャ、カタログ、Go を使用したカスタム プラグインの開発。
...
<text x="92" y="69" font-family="system-ui,-apple-system,sans-serif" font-size="13" font-weight="600" fill="#f472b6">🔒 D​​evSecOps — レッスン 18</text>

  <!-- Title -->
  <text x="60" y="140" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
<tspan x="60" dy="0">レッスン 18: KMIP、領事、遊牧民の秘密</tspan>
<tspan x="60" dy="42">エンジンとカスタム プラグイン</tspan>
  </text>
...
<p><strong>KMIP (Key Management Interoperability Protocol)</strong> は、暗号化キー管理のための OASIS 標準プロトコルです。 KMIP Secrets Engine を使用すると、Vault が <strong>KMIP サーバー</strong> として機能し、KMIP 対応のデータベース、ストレージ システム、アプリケーションにキー管理を提供できます。</p>

<h3 id="use-cases-kmip"><strong>Use Cases</strong></h3>

<ul>
<li><p><strong>MongoDB Enterprise</strong> — KMIP</p></li> による保存時の暗号化
...
<h3 id="mongodb-kmip"><strong>MongoDB と KMIP ボールト</strong></h3>
...
<p><strong>Consul Secrets Engine</strong> は <strong>動的 Consul ACL トークン</strong> を生成し、アプリケーションが短期資格情報を使用して Consul サービス メッシュと KV ストアにアクセスできるようにします。</p>
...
<p>Vault はプラグイン ベースのアーキテクチャを採用しています。すべてのシークレット エンジンと認証メソッドはプラグインです。 Go.</p> を使用してカスタム プラグインを開発できます。
...
<p>プラグインの多重化 (Vault 1.12 以降) により、1 つのプラグイン プロセスが複数のマウントを処理できるようになり、リソースの使用量が削減されます:</p>
...
<h2 id="5-community-plugins"><strong>5。注目すべきコミュニティプラグイン</strong></h2>

<table>
<thead>
<tr><th>プラグイン</th><th>説明</th></tr>
</thead>
<tbody>
...
<ul>
<li><p><strong>KMIP</strong> — 標準キー管理、MongoDB/MySQL/VMware 統合</p></li>
<li><p><strong>Consul/Nomad Engines</strong> — dynamic ACL tokens cho HashiCorp ecosystem</p></li>
<li><p><strong>Plugin System</strong> — Go</p></li> を使用した拡張可能なアーキテクチャ、カスタム プラグイン
<li><p><strong>Plugin Multiplexing</strong> — performance improvement cho multiple mounts</p></li>
</ul>

<p>次のセクションでは、Vault Agent、Vault Proxy、Kubernetes の統合、つまりシークレットをアプリケーションに自動的に配信する方法について説明します。</p>
