...
<text x="92" y="69" font-family="system-ui,-apple-system,sans-serif" font-size="13" font-weight="600" fill="#818cf8">🔒 D​​evSecOps — レッスン 10</text>

  <!-- Title -->
  <text x="60" y="140" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
<tspan x="60" dy="0">レッスン 10: AWS、Azure、GCP、クラウドの秘密</tspan>
...
Cloud Secrets Engine を使用すると、Vault でクラウド プロバイダー (AWS、Azure、GCP) の **動的認証情報** を作成できます。 Vault は、有効期間の長いクラウド認証情報 (IAM アクセス キー、サービス プリンシパル シークレット、サービス アカウント キー) を管理する代わりに、オンデマンドで有効期間の短い認証情報を作成し、TTL の有効期限が切れると自動的に取り消します。

### 主な利点

- **長期間有効な認証情報は不要**: 認証情報が漏洩した場合のリスクを軽減します
- **最小権限**: 各リクエストは必要な権限を正確に受け取ります。
- **監査証跡**: 誰がいつクラウド認証情報を要求したかを正確に把握します。
- **集中アクセス**: 単一の場所からマルチクラウド アクセスを管理
- **自動ローテーション**: 手動でキーをローテーションする必要はありません

## PHẦN 1: AWS Secrets Engine

＃＃＃１．１． AWS Secrets Engine のアーキテクチャ
...
Vault は 3 種類の認証情報をサポートしています。

|タイプ |説明 | TTL |
|---|---|---|
| **IAM ユーザー** | IAM ユーザー + アクセスキーを作成 |ロング (キーローテーションあり) |
| **STS の役割を引き受ける** | AssumeRole → 一時的な認証情報 | 15分～12時間 |
| **STS フェデレーション トークン** |取得フェデレーショントークン | 15分～36時間 |
...
> **セキュリティ:** ルート認証情報をローテーションした後は、Vault のみが新しいアクセス キーを認識します。回転する前に構成がテストされていることを確認してください。

### 1.3. IAM User Credentials

アプリケーションがアクセス キーを要求すると、Vault はアクセス キーを持つ IAM ユーザーを作成します。
...
STS AssumeRole は、IAM ロールを引き受けることによって一時的な認証情報を作成します。**IAM ユーザーを作成する必要はありません**。
...
Vault は、指定されたロール割り当てを持つ一時的な Azure サービス プリンシパル (SP) を作成します。
...
静的ロールを使用すると、Vault で **既存のサービス プリンシパル**のパスワード ローテーションを管理できるようになります。
...
Vault は一時的なサービス アカウント キーを作成します。
...
Vault はキーを生成する代わりに、有効期間の短い **OAuth2 アクセス トークン**を返すことができます。
...
**既存のサービス アカウント**のキーのローテーションを管理します。
...
- ✅ IAM ユーザーではなく STS AssumeRole を優先します
- ✅ IAM ポリシーで条件キーを使用する
- ✅ CloudTrail で API 呼び出しを監査できるようにする
- ✅ Least privilege IAM policies cho Vault

**Azure:**

- ✅ Rotate root credentials ngay sau khi configure
- ✅ スコープのロール割り当ては最小限 (リソース グループ、サブスクリプションなし)
- ✅ 固定 SP を必要とするアプリケーションには Azure 静的ロールを使用します
- ✅ Monitor SP creation trong Azure AD audit logs

**GCP:**

- ✅ サービス アカウント キーの代わりに OAuth2 アクセス トークンを優先します。
- ✅ Vault SA には `iam.serviceAccountKeyAdmin` と `iam.serviceAccountAdmin` のみが必要です
- ✅ 可能な場合は Workload Identity Federation を使用します
...
このレッスンでは、Cloud Secrets Engine の使用方法をマスターしました。
...
5. **TTL ガイドライン** — ユースケースごとに適切な認証情報の種類と TTL を選択します
6. **トラブルシューティング** — 一般的な問題の処理

これは **パート 2: シークレット エンジン**の最後の記事です。 KV 静的シークレット、データベースの動的認証情報、PKI 証明書、トランジット暗号化からクラウド認証情報に至るまで、Vault のすべての最も重要なシークレット エンジンの強固な基盤がすでに構築されています。次のセクションでは、**認証方法**、つまり Vault で ID を認証および管理する方法について詳しく説明します。
