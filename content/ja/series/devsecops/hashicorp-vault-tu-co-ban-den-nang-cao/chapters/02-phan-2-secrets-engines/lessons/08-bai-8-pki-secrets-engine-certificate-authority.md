...
description: PKI deep dive, Root CA, Intermediate CA, Certificate roles, Issue/Sign, CRL, OCSP, Auto-rotation, ACME, PKI certificate counter (1.21), cert-manager integration, mTLS.
...
<text x="92" y="69" font-family="system-ui,-apple-system,sans-serif" font-size="13" font-weight="600" fill="#34d399">🔒 D​​evSecOps — レッスン 8</text>

  <!-- Title -->
  <text x="60" y="140" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
<tspan x="60" dy="0">レッスン 8: PKI シークレット エンジン - 証明書</tspan>
...
PKI (公開キー基盤) Secrets Engine は、Vault を完全な **認証局 (CA)** に変え、X.509 証明書を自動的に作成、署名、管理できるようにします。サードパーティから証明書を購入したり、別の CA サーバーを管理したりする代わりに、Vault はシンプルな API を備えた PKI-as-a-Service を提供します。

### なぜ PKI Secrets Engine が必要なのでしょうか?

- **完全な自動化**: API/CLI、CI/CD 統合を介して証明書を発行します。
- **有効期間の短い証明書**: 証明書が侵害されるリスクを軽減します。
- **mTLS**: 安全なサービス間通信
- **一元管理**: すべての証明書を 1 か所から管理します
- **ACME サポート**: Let's Encrypt プロトコルと互換性があります

## 1. PKI アーキテクチャ — ルート CA と中間 CA
...
- ルート CA 秘密キーの保護が強化されています (めったに使用されません)
- 中間 CA が侵害された場合は、中間 CA を取り消して新しい CA を作成するだけです
- ルート CA トラスト チェーンには影響しません。
...
ロールは、発行された証明書の **テンプレート**を定義します。各役割は、ドメイン パターン、TTL、キー タイプ、および許可される拡張子を指定します。
...
## 6. CSR への署名 — 外部からの証明書リクエストへの署名

アプリケーションが独自の秘密キーを生成し、署名のために CSR を送信すると、次のようになります。
...
OCSP を使用すると、クライアントは証明書失効ステータスをリアルタイムで確認できます。
...
Vault 1.14 以降、PKI エンジンは **ACME プロトコル** (Let's Encrypt が使用するものと同じプロトコル) をサポートします。これにより、certbot、cert-manager などの ACME クライアントとの統合が可能になります。
...
＃＃＃９．２． ACME Vault で certbot を使用する
...
Vault 1.21 には、発行された証明書をカウントする機能が追加されており、監視とコンプライアンスに役立ちます。
...
## 11. cert-manager の統合 (Kubernetes)

### 11.1。 cert-manager ボールト発行者のインストール
...
- ✅ ルート CA TTL: 10 ～ 20 年、オフライン ストレージ
- ✅ 中級 CA TTL: 3 ～ 5 年
- ✅ リーフ証明書 TTL: 30 日～1 年 (短いほど良い)
- ✅ 証明書の有効期限が 1/3 残ったら更新します
- ✅ リーフ証明書には ECDSA P-256 を使用します (RSA より高速)

### 15.2. Security

- ✅ ルート CA 秘密キーをエクスポートしないでください
- ✅ ルートと中間に個別の PKI マウント
- ✅ CRL と OCSP を有効にする
- ✅ Regular tidy operations
- ✅ 証明書の数と有効期限を監視します
- ✅ Restrict role permissions theo team/service
- ✅ 大量のリリースには `no_store=true` を使用します

＃＃ まとめ

このレッスンでは、Vault を使用して完全な **PKI インフラストラクチャ**をセットアップします。

1. **ルート CA + 中間 CA** — 標準の 2 層アーキテクチャ
2. **Certificate Roles** — templates cho web server, mTLS, internal services
3. **証明書の発行と署名** — CLI/API 経由で自動的に
4. **CRL および OCSP** — 証明書の失効
5. **ACME サポート** — Let's Encrypt プロトコルと互換性があります
6. **cert-manager の統合** — Kubernetes での PKI 自動化
7. **mTLS** — サービス間のセキュリティ
8. **自動ローテーション** — 証明書を自動的に更新します

次の記事では、**Transit Secrets Engine** — Encryption as a Service について説明します。Vault では、暗号化キーを管理せずにデータを暗号化できます。
