...
description: >-
  NGINXのセキュリティに関するレッスン——limit_req/limit_connによるレート制限、IPブロック、基本認証、ModSecurity WAF統合、DDoS対策、セキュアヘッダー（CSP、X-Frame-Options、CORS）。本番環境でサーバーを攻撃から守るためのベストプラクティスとセキュリティ強化ガイド。
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="92" y="69" font-family="system-ui,-apple-system,sans-serif" font-size="13" font-weight="600" fill="#a78bfa">🔒 DevSecOps — 第9課</text>

  <!-- Title -->
  <text x="60" y="160" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">第9課：NGINXのセキュリティ</tspan>
  </text>
...
<h2 id="1-rate-limiting-v%E1%BB%9Bi-limitreq-v%C3%A0-limitconn"><strong>1. limit_reqとlimit_connによるレート制限</strong></h2><p>レート制限は、1つのクライアントからのリクエスト数を制限することで、サーバーを乱用、ブルートフォース攻撃、DDoSから保護します。</p><h3 id="11-rate-limiting-basics-limitreq"><strong>1.1. Rate Limiting Basics (limit_req)</strong></h3><pre><code class="language-nginx">http {
