...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="92" y="69" font-family="system-ui,-apple-system,sans-serif" font-size="13" font-weight="600" fill="#f472b6">🔒 DevSecOps — レッスン 19</text>

  <!-- Title -->
  <text x="60" y="160" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">レッスン 19: ロギングとトラブルシューティング</tspan>
  </text>
...
</svg><h1 id="b%C3%A0i-19-logging-v%C3%A0-troubleshooting">レッスン 19: ロギングとトラブルシューティング</h1><h2 id="m%E1%BB%A5c-ti%C3%AAu">目標_</h2><p>このレッスンの後、次のことを学びます:</p><ul><li>PostgreSQL ログの理解と分析</li><li>Patroni の問題をデバッグするログ_</li><li>etcd クラスターの問題のトラブルシューティング</li><li>一般的な HA 問題の特定と修正</li><li>デバッグ ツールの効果的な使用</li><li>トラブルシューティング Runbook の作成</li></ul><h2 id="1-postgresql-logging">1。 PostgreSQL のログ</h2><h3 id="11-configure-logging">1.1。ロギング_</h3><pre><code class="language-sql">-- Essential logging settings
...
</code></pre><h3 id="12-log-file-location">1.2を構成します。ログ ファイルの場所_</h3><pre><code class="language-bash"># Default location
...
</code></pre><h3 id="13-common-log-patterns">1.3。一般的なログ パターン_</h3><h4 id="connection-issues">接続の問題</h4><pre><code class="language-text">LOG: connection received: host=10.0.1.50 port=54321
...
</code></pre><h4 id="slow-queries">遅いクエリ</h4><pre><code class="language-text">LOG: duration: 5432.123 ms  statement: SELECT * FROM large_table WHERE ...

→ Solution: Analyze query, add indexes, optimize
</code></pre><h4 id="deadlocks">デッドロック</h4><pre><code class="language-text">ERROR: deadlock detected
...
</code></pre><h4 id="checkpoint-warnings">チェックポイント警告_</h4><pre><code class="language-text">LOG: checkpoints are occurring too frequently (24 seconds apart)
HINT: Consider increasing the configuration parameter "max_wal_size".

→ Solution: Increase max_wal_size
</code></pre><h4 id="replication-lag">レプリケーションの遅延</h4><pre><code class="language-text">WARNING: could not send data to WAL receiver; retrying
DETAIL: Connection to replica timed out

→ Solution: Check network, replica health
</code></pre><h4 id="out-of-disk-space">ディスク容量不足</h4><pre><code class="language-text">FATAL: could not write to file "base/16384/12345": No space left on device

→ Solution: Free up disk space, increase storage
</code></pre><h3 id="14-analyze-logs-with-pgbadger">1.4。 pgBadger</h3><pre><code class="language-bash"># Install pgBadger
...
</code></pre><h2 id="2-patroni-logging">2 を使用してログを分析します。 Patroni のログ</h2><h3 id="21-patroni-log-levels">2.1。 Patroni ログ レベル_</h3><pre><code class="language-yaml"># In patroni.yml
log:
...
</code></pre><h3 id="22-view-patroni-logs">2.2。 Patroni のログを表示_</h3><pre><code class="language-bash"># Via journalctl (if systemd service)
...
</code></pre><h3 id="23-common-patroni-log-patterns">2.3。一般的な Patroni ログ パターン</h3><h4 id="successful-bootstrap">ブートストラップ成功</h4><pre><code class="language-text">2024-11-25 10:00:00 INFO: Selected new etcd server http://10.0.1.11:2379
...
</code></pre><h4 id="failed-to-acquire-lock-replica">ロック (レプリカ) の取得に失敗</h4><pre><code class="language-text">2024-11-25 10:05:00 INFO: Lock owner: node1; I am node2
...
</code></pre><h4 id="failover-detected">フェイルオーバー検出_</h4><pre><code class="language-text">2024-11-25 11:30:00 WARNING: Request failed to node1:8008: connection refused
...
</code></pre><h4 id="dcs-connection-issues">DCS 接続の問題</h4><pre><code class="language-text">2024-11-25 12:00:00 ERROR: Failed to get list of machines from etcd
...
</code></pre><h4 id="pgrewind-execution">_pg_rewind 実行</h4><pre><code class="language-text">2024-11-25 13:00:00 INFO: running pg_rewind from node2
...
</code></pre><h4 id="configuration-reload">構成リロード_</h4><pre><code class="language-text">2024-11-25 14:00:00 INFO: Reloading PostgreSQL configuration.
...
</code></pre><h3 id="24-enable-debug-logging">2.4。 DEBUG ログ_</h3><pre><code class="language-bash"># Temporarily enable debug logging
...
</code></pre><h2 id="3-etcd-logging">3 を有効にします。 etcd ログ</h2><h3 id="31-view-etcd-logs">3.1。 etcd ログを表示</h3><pre><code class="language-bash"># Via journalctl
...
</code></pre><h3 id="32-etcd-log-levels">3.2。 etcd ログ レベル</h3><pre><code class="language-bash"># Configure in etcd service file
...
ExecStart=/usr/local/bin/etcd \
  ...
  --logger zap \
  --log-outputs stderr
</code></pre><h3 id="33-common-etcd-log-patterns">3.3。一般的な etcd ログ パターン_</h3><h4 id="cluster-healthy">クラスターが正常</h4><pre><code class="language-text">2024-11-25 10:00:00 INFO: raft.node: elected leader at term 5
...
</code></pre><h4 id="leader-election">リーダー選挙_</h4><pre><code class="language-text">2024-11-25 11:00:00 WARN: rafthttp: lost TCP streaming connection with peer node1
...
</code></pre><h4 id="network-partition">ネットワークパーティション</h4><pre><code class="language-text">2024-11-25 12:00:00 WARN: rafthttp: health check for peer node2 could not connect: dial tcp: i/o timeout
...
</code></pre><h4 id="slow-operations">遅い操作</h4><pre><code class="language-text">2024-11-25 13:00:00 WARN: etcdserver: apply request took too long [1.234s] expected less than [100ms]
...
</code></pre><h3 id="34-etcd-debugging">3.4。 etcd のデバッグ</h3><pre><code class="language-bash"># Check cluster health
...
</code></pre><h2 id="4-common-issues-and-solutions">4。一般的な問題と解決策_</h2><h3 id="41-issue-patroni-wont-start">4.1。問題: Patroni が起動しない</h3><p><strong>症状</strong>:&nbsp;<code>systemctl status patroni</code>&nbsp;が表示されます失敗</p><p><strong>診断</strong>:</p><pre><code class="language-bash"># Check logs
...
</code></pre><h3 id="42-issue-no-leader-in-cluster">4.2。問題: クラスターにリーダーがいません_</h3><p><strong>症状</strong>:&nbsp;<code>patronictl リスト</code>&nbsp;には no が表示されますリーダー</p><p><strong>診断</strong>:</p><pre><code class="language-bash"># Check all Patroni instances
...
</code></pre><h3 id="43-issue-high-replication-lag">4.3。問題: レプリケーションの遅延が大きい_</h3><p><strong>症状</strong>：レプリカラグ＞ 100MB</p><p><strong>診断</strong>:</p><pre><code class="language-sql">-- Check lag
...
</code></pre><h3 id="44-issue-split-brain-detected">4.4。問題: スプリット ブレインが検出されました</h3><p><strong>症状</strong>: 複数のノードがリーダー</p><p><strong>診断</strong>:</p><pre><code class="language-bash"># Check cluster status
...
</code></pre><h3 id="45-issue-failover-not-happening">4.5。問題: フェイルオーバーが発生しない</h3><p><strong>症状</strong>: プライマリはダウンしているが、障害は発生していないプロモーション_</p><p><strong>診断</strong>:</p><pre><code class="language-bash"># Check Patroni status
patronictl list postgres
...
</code></pre><h3 id="46-issue-cannot-connect-to-postgresql">4.6。問題: PostgreSQL に接続できません</h3><p><strong>症状</strong>: 接続が拒否された、またはタイムアウト_</p><p><strong>診断</strong>:</p><pre><code class="language-bash"># 1. Check PostgreSQL running
...
</code></pre><h3 id="47-issue-pgrewind-failed">4.7。問題: pg_rewind が失敗しました</h3><p><strong>症状</strong>: フェイルオーバー後にノードが再参加できません</p><p><strong>エラー</strong>:&nbsp;<code>pg_rewind: エラー: できませんでした共通の祖先を検索_</code></p><p><strong>診断</strong>:_</p><pre><code class="language-bash"># Check logs
...
</code></pre><h2 id="5-debugging-tools">5。デバッグ ツール</h2><h3 id="51-patronictl-commands">5.1。 patronictl コマンド</h3><pre><code class="language-bash"># List cluster
patronictl list postgres
...
</code></pre><h3 id="52-etcdctl-commands">5.2。 etcdctl コマンド</h3><pre><code class="language-bash"># Cluster health
etcdctl endpoint health --cluster
...
</code></pre><h3 id="53-postgresql-diagnostic-queries">5.3。 PostgreSQL 診断クエリ_</h3><pre><code class="language-sql">-- Current activity
...
LIMIT 20;
</code></pre><h3 id="54-system-diagnostic-commands">5.4。システム診断コマンド</h3><pre><code class="language-bash"># CPU usage
top -bn1 | head -20
htop
...
</code></pre><h2 id="6-troubleshooting-runbook">6。トラブルシューティング ランブック</h2><h3 id="61-primary-node-down">6.1。プライマリ ノードがダウン_</h3><pre><code class="language-bash"># 1. Verify primary is down
patronictl list postgres
...
</code></pre><h3 id="62-replica-not-replicating">6.2。レプリカが複製されていません_</h3><pre><code class="language-bash"># 1. Check replica status
patronictl list postgres
...
</code></pre><h3 id="63-etcd-cluster-unhealthy">6.3。 etcd クラスターが異常</h3><pre><code class="language-bash"># 1. Check cluster health
etcdctl endpoint health --cluster
...
</code></pre><h2 id="7-best-practices">7。ベスト プラクティス</h2><h3 id="%E2%9C%85-do">✅ DO</h3><ol><li><strong>適切なログを有効にする</strong>&nbsp;- 詳細とボリュームのバランス</li><li><strong>一元化ログ</strong>&nbsp;- ELK/Grafana Loki を使用</li><li><strong>アラートを設定</strong>&nbsp;- プロアクティブな通知</li><li><strong>定期的なログの確認</strong>&nbsp;-週次分析</li><li><strong>ドキュメントの問題</strong>&nbsp;- ナレッジベースの構築</li><li><strong>テストシナリオ</strong>&nbsp;- 実践トラブルシューティング</li><li><strong>ランブックを更新し続ける</strong>&nbsp;- 生きているドキュメント</li><li><strong>ディスク容量を監視</strong>&nbsp;- ログがいっぱいになる可能性がありますディスク_</li></ol><h3 id="%E2%9D%8C-dont">❌ 禁止</h3><ol><li><strong>運用環境で DEBUG を有効にしないでください</strong>&nbsp;- 冗長すぎます</li><li><strong>_禁止してください警告を無視してください</strong>&nbsp;- 警告はエラーになります</li><li><strong>すぐにログを削除しないでください</strong>&nbsp;- 分析のために保存してください</li><li><strong>ログのローテーションをスキップしないでください</strong>&nbsp;- ディスクがいっぱいになるのを防ぎます</li><li><strong>盲目的なトラブルシューティングを行わないでください</strong>&nbsp;- ログを確認してください最初_</li><li><strong>ログなしで変更を加えない</strong>&nbsp;- アクションを文書化</li></ol><h2 id="8-lab-exercises">8。ラボ演習_</h2><h3 id="lab-1-log-analysis">ラボ 1: ログ分析</h3><p><strong>タスク_</strong>: 1. PostgreSQL の遅いクエリのログを構成する 2. 遅いクエリでワークロードを生成する 3. pgBadger でログを分析する 4. 上位の問題を特定する 5. 文書化する調査結果</p><h3 id="lab-2-simulate-and-debug-failover">ラボ 2: フェイルオーバーのシミュレーションとデバッグ</h3><p><strong>タスク</strong>: 1. プライマリ ノードの停止 2. フェイルオーバー中の Patroni ログの監視 3. イベントのタイムラインのトレース 4. ログからダウンタイムを計算 5. タイムラインの作成図_</p><h3 id="lab-3-debug-replication-lag">ラボ 3: レプリケーション ラグのデバッグ</h3><p><strong>タスク_</strong>: 1. プライマリでの高い書き込み負荷をシミュレートする 2. レプリカでラグの増加を観察する 3. 診断クエリを使用して原因を特定する 4. 問題を修正する 5. ラグを確認する削減_</p><h3 id="lab-4-troubleshoot-connection-issues">ラボ 4: 接続問題のトラブルシューティング</h3><p><strong>タスク</strong>: 1. pg_hba.conf の設定ミス 2. 接続を試みる (失敗します) 3. ログを使用して問題を特定する 4. 設定を修正する 5. 接続を確認する仕事</p><h2 id="9-t%E1%BB%95ng-k%E1%BA%BFt">9。概要</h2><h3 id="key-logging-locations">主要なログの場所</h3><pre><code class="language-text">PostgreSQL: /var/lib/postgresql/18/data/log/
Patroni: journalctl -u patroni
etcd: journalctl -u etcd
System: /var/log/syslog, dmesg
</code></pre><h3 id="essential-diagnostic-commands">重要な診断コマンド</h3><pre><code class="language-bash"># Cluster status
patronictl list postgres
...
sudo journalctl -u patroni -f
sudo journalctl -u etcd -f
</code></pre><h3 id="troubleshooting-workflow">トラブルシューティングワークフロー</h3><ol><li><strong>特定</strong>&nbsp;- 症状は何ですか?</li><li><strong>分離</strong>&nbsp;- どのコンポーネントが該当しますか?失敗しますか?</li><li><strong>調査</strong>&nbsp;- ログ、指標を確認</li><li><strong>診断</strong>&nbsp;- 原因は何ですか原因は何ですか?</li><li><strong>修正</strong>&nbsp;- 解決策を適用</li><li><strong>確認</strong>&nbsp;- 確認解決策</li><li><strong>ドキュメント</strong>&nbsp;- 今後の記録</li></ol><h3 id="next-steps">次のステップ</h3><p>レッスン 20 では、<strong>セキュリティ ベストについて説明します。実践_</strong>:</p><ul><li>SSL/TLS暗号化</li><li>認証方法</li><li>ネットワークセキュリティ</li><li>暗号化残り</li><li>監査ログ</li><li>セキュリティ強化_</li></ul>