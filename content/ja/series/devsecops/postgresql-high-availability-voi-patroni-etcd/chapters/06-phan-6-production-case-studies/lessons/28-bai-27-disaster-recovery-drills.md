...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="92" y="69" font-family="system-ui,-apple-system,sans-serif" font-size="13" font-weight="600" fill="#c084fc">🔒 DevSecOps — レッスン 27</text>

  <!-- Title -->
  <text x="60" y="160" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">レッスン 27: 災害復旧訓練</tspan>
  </text>
...
</svg><h2 id="m%E1%BB%A5c-ti%C3%AAu">目的_</h2><p>このレッスンの後、次のことを行います:_</p><ul><li>包括的な災害復旧手順を計画する_</li><li>科学的な DR 訓練を実行する</li><li>測定と最適化RTO/RPO</li><li>インシデント対応演習を実施</li><li>DR プロセスを文書化して改善</li></ul><h2 id="1-dr-planning-foundation">1。 DR プランニング財団</h2><h3 id="11-key-dr-metrics">1.1。主要な DR 指標_</h3><pre><code class="language-text">RTO (Recovery Time Objective):
...
- Goal: RPD &lt; RPO
</code></pre><h3 id="12-dr-scenarios-to-test">1.2。テストする DR シナリオ</h3><pre><code class="language-text">1. Single node failure
...
</code></pre><h2 id="2-dr-drill-preparation">2。 DR ドリルの準備_</h2><h3 id="21-pre-drill-checklist">2.1。ドリル前のチェックリスト</h3><pre><code class="language-text">☐ Review DR documentation
...
</code></pre><h3 id="22-dr-team-roles">2.2。 DR チームの役割</h3><pre><code class="language-text">Incident Commander:
...
</code></pre><h2 id="3-scenario-1-single-replica-failure">3。シナリオ 1: 単一レプリカの障害</h2><h3 id="31-drill-procedure">3.1。ドリル手順</h3><pre><code class="language-bash"># Step 1: Simulate replica failure (10:00:00)
...
</code></pre><h3 id="32-expected-results">3.2。期待される結果_</h3><pre><code class="language-text">Timeline:
...
</code></pre><h2 id="4-scenario-2-leader-failover">4。シナリオ 2: リーダーのフェイルオーバー</h2><h3 id="41-drill-procedure">4.1。ドリル手順</h3><pre><code class="language-bash"># Step 1: Record current leader (10:00:00)
...
</code></pre><h3 id="42-expected-results">4.2。期待される結果_</h3><pre><code class="language-text">Timeline:
...
</code></pre><h2 id="5-scenario-3-complete-datacenter-failure">5。シナリオ 3: データセンターの完全な障害</h2><h3 id="51-drill-procedure">5.1。ドリル手順</h3><pre><code class="language-bash"># Setup: Assume 2 datacenters
...
patronictl -c /etc/patroni/patroni.yml list
</code></pre><h3 id="52-expected-results">5.2。期待される結果_</h3><pre><code class="language-text">Timeline:
...
</code></pre><h2 id="6-scenario-4-point-in-time-recovery-data-corruption">6。シナリオ 4: ポイントインタイムリカバリ (データ破損)</h2><h3 id="61-drill-procedure">6.1。ドリル手順</h3><pre><code class="language-bash"># Setup: Simulate accidental table drop at 10:30:00
...
</code></pre><h3 id="62-expected-results">6.2。期待される結果_</h3><pre><code class="language-text">Timeline:
...
</code></pre><h2 id="7-dr-drill-metrics-and-reporting">7。 DR ドリルのメトリクスとレポート</h2><h3 id="71-drill-scorecard">7.1。ドリル スコアカード</h3><pre><code class="language-text">Scenario: Leader Failover Drill
Date: 2024-11-25
Duration: 30 minutes
...
☐ Re-test in 2 weeks
</code></pre><h3 id="72-post-drill-analysis">7.2。ドリル後の分析</h3><pre><code class="language-markdown"># DR Drill Post-Mortem: Leader Failover
...
</code></pre><h2 id="8-chaos-engineering-for-ha">8。 HA</h2><h3 id="81-chaos-monkey-for-postgresql">8.1 のカオス エンジニアリング。 PostgreSQL の Chaos Monkey_</h3><pre><code class="language-bash">#!/bin/bash
...
</code></pre><h3 id="82-automated-dr-testing">8.2。自動 DR テスト</h3><pre><code class="language-yaml"># automated_dr_test.yml
---
...
</code></pre><h2 id="9-best-practices">9。ベスト プラクティス</h2><h3 id="%E2%9C%85-do">✅ 実行</h3><ol><li><strong>定期的な訓練をスケジュールする</strong>&nbsp;- 四半期ごとの最小</li><li><strong>すべてテストするシナリオ</strong>&nbsp;- 簡単なシナリオだけではありません_</li><li><strong>役割をローテーション</strong>&nbsp;- 全員が一度はICになるべき</li><li><strong>すべてを文書化</strong>&nbsp;- タイムスタンプ付きメモ_</li><li><strong>RTO/RPO の測定</strong>&nbsp;- 改善の追跡</li><li><strong>すべての訓練の事後分析</strong>&nbsp;- 学習と改善_</li><li><strong>ランブックを更新</strong>&nbsp;- ドキュメントを最新の状態に保つ</li><li><strong>すべてのチームを参加させる</strong>&nbsp;- 部門横断型実践_</li><li><strong>バックアップのテスト</strong>&nbsp;- 復元の検証は必須</li><li><strong>可能な場合は自動化</strong>&nbsp;- 人員を削減エラー_</li></ol><h3 id="%E2%9D%8C-dont">❌ しないでください</h3><ol><li><strong>ドリルをスキップしないでください</strong>&nbsp;- 「忙しすぎる」は言い訳になりません</li><li><strong>簡単なシナリオだけをテストしないでください</strong>&nbsp;- 難しいシナリオも重要です最も</li><li><strong>アクションアイテムを無視しない</strong>&nbsp;- 改善点をフォローする_</li><li><strong>同じシナリオを再利用しない</strong>&nbsp;-ドリル</li><li><strong>1 人に依存しない</strong>&nbsp;- バス係数 = 1 は危険</li><li><strong>急いではいけません</strong>&nbsp;- 適切なテストが必要です時間</li><li><strong>事後分析をスキップしないでください</strong>&nbsp;- 学習の機会</li></ol><h2 id="10-lab-exercises">10。ラボ演習</h2><h3 id="lab-1-execute-failover-drill">ラボ 1: フェールオーバー ドリルの実行</h3><p><strong>タスク</strong>:</p><ol><li>ドリルの計画とスケジュール</li><li>チームの割り当て役割_</li><li>リーダーフェイルオーバーの実行</li><li>タイムラインの文書化_</li><li>RTO/RPOの計算</li><li>事後分析の作成</li></ol><h3 id="lab-2-pitr-recovery-drill">ラボ2: PITR 回復訓練</h3><p><strong>タスク</strong>:</p><ol><li>テストデータの作成_</li><li>データ破損のシミュレーション</li><li>特定PITR 目標時間</li><li>別のインスタンスへの復元</li><li>回復されたデータの検証_</li><li>手順の文書化</li></ol><h3 id="lab-3-multi-dc-failover">ラボ 3: マルチ DCフェイルオーバー</h3><p><strong>タスク</strong>:</p><ol><li>2-DC クラスターのセットアップ</li><li>DC1 の全体的な障害をシミュレート</li><li>手動で昇格DC2</li><li>アプリケーション構成の更新</li><li>ダウンタイムの測定</li><li>学んだ教訓の文書化</li></ol><h3 id="lab-4-chaos-engineering">ラボ 4: カオスエンジニアリング_</h3><p><strong>タスク_</strong>:_</p><ol><li>カオス モンキー スクリプトの実装</li><li>24 時間実行</li><li>クラスターの監視動作_</li><li>障害と回復の文書化_</li><li>_弱点の特定</li><li>HA 構成の改善</li></ol><h2 id="11-t%E1%BB%95ng-k%E1%BA%BFt">11。概要_</h2><h3 id="dr-drill-frequency">DR ドリル頻度</h3><pre><code class="language-text">Scenario Frequency:
- Single node failure: Monthly (automated)
- Leader failover: Quarterly
- DC failure: Semi-annually
- PITR recovery: Quarterly
- Full DR: Annually
</code></pre><h3 id="success-criteria">成功基準_</h3><pre><code class="language-text">A successful DR drill has:
✅ Met RTO/RPO targets
...
</code></pre><h3 id="key-metrics-to-track">追跡する主要な指標</h3><pre><code class="language-text">- Detection time (how fast we notice)
...
</code></pre><h3 id="next-steps">次へステップ</h3><p>レッスン 28 では、<strong>アーキテクチャ設計 HA</strong>:</p><ul><li>要件の収集</li><li>アーキテクチャ設計について説明しますドキュメント_</li><li>容量計画</li><li>コスト見積もり</li><li>設計レビュープロセス</li></ul>