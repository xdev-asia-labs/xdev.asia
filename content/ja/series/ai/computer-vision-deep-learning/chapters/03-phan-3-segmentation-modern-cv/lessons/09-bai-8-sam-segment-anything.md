...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="92" y="69" font-family="system-ui,-apple-system,sans-serif" font-size="13" font-weight="600" fill="#f87171">🧠 AI と ML — レッスン 7</text>

  <!-- Title -->
  <text x="60" y="140" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">レッスン 8: SAM (Segment Anything) — ゼロショット</tspan>
      <tspan x="60" dy="42">セグメンテーション</tspan>
  </text>
  ...
  <!-- Section -->
  <text x="60" y="268" font-family="system-ui,-apple-system,sans-serif" font-size="13" fill="#64748b" opacity="0.6">パート 3: セグメンテーションと最新の履歴書</text>
...
**SAM — Segment Anything Model** (Meta AI、2023) は、CV における画期的な技術です。**トレーニングなしで**、画像内の**あらゆるオブジェクト**をセグメント化します。 1クリック（ポイント）、1ボックス、または1テキスト文→すぐにSAMセグメント。

> 🎯 **SAM** は ChatGPT に似ていますが、セグメンテーション用です。ゼロショット、汎用で、あらゆるタイプの画像に機能します。
...
|モデル |エンコーダ |パラメータ |スピード |品質 |
|----------|----------|----------|----------|----------|
| SAM ViT-B | ViTベース | 91M | ⚡ 速い | ⭐⭐⭐ |
|サム・ヴィット・エル | ViT-ラージ | 308M | 🔥 中 | ⭐⭐⭐⭐ |
| **SAM ViT-H** | ViT-ヒュージ | **636M** | 🐌 遅い | **⭐⭐⭐⭐⭐** |
| **サム 2** |ヒエラ | **ライター** | **⚡ より速く** | **⭐⭐⭐⭐⭐** |
...
### 2.2 ポイント プロンプト — クリックしてセグメント化します
...
### 2.3 ボックスプロンプト — セグメントにボックスを描画します
...
|コンセプト |覚えておいてください |
|----------|----------|
| **サム** |何でもセグメント化 — ゼロショット、プロンプトベース |
| **ポイントプロンプト** | 1+ ポイント → セグメント | をクリックします。
| **ボックスプロンプト** |境界ボックスを描画 → セグメント |
| **自動生成** |すべてのオブジェクトを自動的にセグメント化する |
| **サム 2** | 6倍速、ビデオサポート |
| **ヨロ + サム** | YOLO 検出 (ボックス) → SAM セグメント (マスク) |

## 一般的な演習

1. **ポイント プロンプト:** SAM を使用して、ポイント プロンプトを使用して 1 つの画像内の 5 つの異なるオブジェクトをセグメント化します。
2. **自動セグメント:** 3 つの画像に対して自動マスク生成を実行します。セグメントはいくつですか?
3. **YOLO + SAM:** パイプライン YOLO 検出 → SAM セグメントを構築します。カラーマスクを使用して視覚化します。
4. **背景の削除:** SAM を使用してオブジェクトを背景から分離し、透明な PNG 画像を保存します。

> **次の記事:** 画像生成と安定拡散 — AI でテキストから画像を作成します。
