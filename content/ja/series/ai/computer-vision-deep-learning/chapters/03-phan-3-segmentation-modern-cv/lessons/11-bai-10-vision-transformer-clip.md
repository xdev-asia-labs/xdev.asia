...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="60" y="160" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">レッスン 10: ビジョン トランスフォーマー (ViT) と CLIP</tspan>
  </text>
...
トランスフォーマーは NLP (GPT、BERT) を変更しました。 **Computer Vision** が変わります。 **ViT** (Vision Transformer) は、Transformer が画像分類において **CNN** を超えることができることを証明します。 **CLIP** (OpenAI) は、同じ空間内のテキストと画像を接続 → ゼロショット分類、画像検索、マルチモーダル AI。

> 🎯 **ViT + CLIP** は、最新のマルチモーダル AI: GPT-4o Vision、Gemini、Claude Vision の基盤です。
...
| | CNN (ResNet) |トランス (ViT) |
|---|-------------|--------|
| **誘導バイアス** |翻訳不変性、局所性 |バイアスが少ない → より多くのデータが必要 |
| **データが必要です** |少し（<100万枚の写真） |たくさん (>1,000 万枚の写真) |
| **スケーラビリティ** |最大 150 層でピーク |適切なスケール: ViT-G (2B パラメータ) |
| **グローバルコンテキスト** |多くのレイヤーが必要 |今すぐ自分に注意を向けてください（すべてのトーキングパッチ） |
| **事前トレーニング済み** |イメージネット (1.3M) | JFT-300M、LAION-5B |

---

## 2. 事前トレーニング済みの ViT を使用する
...
|コンセプト |覚えておいてください |
|----------|----------|
| **ViT** |写真用トランスフォーマー: 分割パッチ → セルフアテンション |
| **パッチの埋め込み** |画像 224×224 → 196 パッチ (各 16×16) → 埋め込み |
| **クリップ** |テキストと画像を同じ埋め込みスペースに接続する |
| **ゼロショット** |画像の分類にはトレーニングは必要ありません。テキスト ラベルだけを必要とします。
| **画像検索** |画像 + テキストをエンコード → コサイン類似度 → 検索 |
| **DINOv2** |自己監視型 ViT、ユニバーサル機能 |

## 一般的な演習

1. **ViT 分類:** 事前トレーニングされた ViT を使用して 20 個の画像を分類します。 ResNet-50と精度を比較してください。
2. **クリップゼロショット:** 10 個のベトナム語カスタムラベルを作成します。 CLIP は画像を正しく分類しますか?
3. **画像検索:** 100 枚の個人写真用の画像検索エンジンを構築します。ベトナム語で検索してください。
4. **CLIP 類似度:** 任意の 2 つの画像 → CLIP 類似度スコア。どれが一番似ていますか?

> **次の記事:** OCR と文書の理解 — テキストを認識し、文書を理解します。
