...
description: X 線分類のための転移学習 ResNet/EfficientNet。 CheXpert データセット。マルチラベル分類。 Grad-CAM の説明可能性。
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="60" y="140" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">レッスン 4: 医用画像の CNN</tspan>
...
**Grad-CAM** は、モデルが意思決定を行うために「注目」する画像内の領域を強調表示します。これはあれば便利というわけではありません。これは医師が AI を信頼し、監査するために**必須**です。
...
**レッスン 5**: U-Net — 分類から **セグメンテーション**まで、腫瘍の境界を正確に描画します。
...
2. 不確実性戦略を「0」から「1」に変更し、再トレーニングします。 AUC がどのように変化するかを比較します。どの病理が最も影響を受けますか?

3. 最適なしきい値の選択を実装します。固定の 0.5 を使用する代わりに、個々の病状の F1 スコアに基づいて検証セットで最適なしきい値を見つけます。
