...
description: >-
  テキスト埋め込み: OpenAI、ローカル モデル。 MongoDB Atlas Search または ChromaDB を使用したベクター
  ストアの抽象化。類似性検索、インデックス戦略、バッチ埋め込み。
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="60" y="160" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">レッスン 10: 埋め込みとベクター ストア</tspan>
  </text>
  ...
  <!-- Section -->
  <text x="60" y="246" font-family="system-ui,-apple-system,sans-serif" font-size="13" fill="#64748b" opacity="0.6">パート 3: RAG パイプライン</text>
...
ドキュメントをチャンク化した後、類似性検索のためにテキストをベクトル (埋め込み) に変換し、ベクトル ストアに保存する必要があります。この記事では、埋め込みパイプラインとベクター ストアの抽象化を実装します。
...
- **Embedding Provider** — OpenAI、Cohere、ローカル モデルの抽象化
- **Vector Store** — MongoDB Atlas Search または ChromaDB
- **バッチ処理** — レート制限を回避するためにチャンクをバッチで処理します。
- **コサイン類似度** — テキスト埋め込みの最も一般的な距離メトリック

**次の記事:** RAG エンジン — 取得をエージェント フローに統合します。
