...
title: 'レッスン 12: ワークフロー エンジン — ノードベースの自動化の設計'
slug: bai-12-workflow-engine
description: >-
  ワークフロー エンジンの設計: DAG (有向非巡回グラフ)、ノード タイプ
  (トリガー、アクション、条件、ループ)。テンプレートの解決、変数の補間。ワークフローのスキーマと検証。
duration_minutes: 210
is_free: true
video_url: null
sort_order: 11
section_title: 'パート 4: ワークフロー エンジン'
course:
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="92" y="69" font-family="system-ui,-apple-system,sans-serif" font-size="13" font-weight="600" fill="#a78bfa">🧠 AI と ML — レッスン 11</text>

  <!-- Title -->
  <text x="60" y="140" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">レッスン 12: ワークフロー エンジン — 設計</tspan>
      <tspan x="60" dy="42">ノードベースの自動化</tspan>
  </text>
  ...
  <!-- Section -->
  <text x="60" y="268" font-family="system-ui,-apple-system,sans-serif" font-size="13" fill="#64748b" opacity="0.6">パート 4: ワークフロー エンジン</text>
...
ワークフロー エンジンを使用すると、ユーザーはビジュアル ノードを使用してオートメーションを作成できます。ドラッグ アンド ドロップ、ノードの接続、コードなしで複雑なロジックの定義が可能です。この記事では、コアコンセプトに基づいてエンジンを設計します。
...
- **DAG モデル** — ノード + エッジ、サイクルは許可されません
- **16 のノード タイプ** — LLM 呼び出しから HTTP リクエストまで
- **テンプレートの解像度** — `{{variable}}` 補間
- **サンドボックス コード** — カスタム コード ノードの VM 分離
- **コンポーザブル** — 再利用可能なサブワークフロー

**次の記事:** ワークフローの検証と実行 — エンドツーエンドのワークフローを実行します。
