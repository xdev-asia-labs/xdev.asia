...
description: >-
  ビルド ツール レジストリ: ツール、検証、サンドボックス実行を登録します。 ToolHandler インターフェイス、パラメーター
  スキーマ、タイムアウトおよびエラー処理。セキュリティ: サンドボックス実行と信頼された実行。
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="60" y="140" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">レッスン 6: ツール レジストリ — 登録と実行</tspan>
...
- **ToolHandler** — 単純な関数インターフェイス: `(args, context) => Promise<result>`
- **ToolRegistry** — 登録/登録解除/実行のレジストリ パターン
- **並列実行** — `executeAll()` ツールを並行して実行する
- **サンドボックス** — 信頼できないコードに対する VM ベースの分離
