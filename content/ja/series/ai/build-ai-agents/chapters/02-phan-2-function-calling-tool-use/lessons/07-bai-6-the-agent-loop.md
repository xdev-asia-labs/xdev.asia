...
description: >-
  純粋な Python で完全なエージェント ループを最初から実装します。ReAct パターン、複数ステップの推論の処理、会話履歴管理、トークン
  バジェット、および停止条件を処理します。
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="92" y="69" font-family="system-ui,-apple-system,sans-serif" font-size="13" font-weight="600" fill="#38bdf8">🧠 AI と ML — レッスン 5</text>

  <!-- Title -->
  <text x="60" y="140" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">レッスン 6: エージェント ループ —</tspan>
      <tspan x="60" dy="42">思考-行動-観察のサイクル</tspan>
  </text>
...
これは、このシリーズの最も重要なレッスンです。フレームワークを使用せずに、**完全なエージェント ループを最初から実装**します。このループを深く理解すると、後でフレームワークを使用するときにエージェントをより効果的にデバッグおよび最適化するのに役立ちます。
...
1. **LLM は独自の判断で停止を決定します** (追加のツールは呼び出されません)
2. **最大歩数に達しました** — セーフティネット
3. **トークンの予算超過** — 予期せぬコストを回避
4. **エラーしきい値** - ツールの連続失敗が多すぎる
5. **ユーザー割り込み** — 人間参加型
...
1. 5 つ以上のツールを備えた SimpleAgent クラスを実装する
2. トークン予算管理の追加
3. 会話履歴の永続化を追加（ファイルの保存/読み込み）
4. 5 つ以上のステップを必要とする複雑なタスクでテストする
