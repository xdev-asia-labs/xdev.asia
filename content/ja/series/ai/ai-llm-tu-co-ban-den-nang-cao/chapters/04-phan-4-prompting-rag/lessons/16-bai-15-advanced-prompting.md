...
description: '高度なプロンプト手法: 思考連鎖、思考ツリー、自己一貫性、ReAct パターン。 LLM を使用して複雑な問題を解決します。'
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="92" y="69" font-family="system-ui,-apple-system,sans-serif" font-size="13" font-weight="600" fill="#818cf8">🧠 AI と ML — レッスン 14</text>

  <!-- Title -->
  <text x="60" y="140" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">レッスン 15: 高度なプロンプト — CoT、ToT、</tspan>
      <tspan x="60" dy="42">反応する</tspan>
  </text>
...
タスクが複雑である場合 (複数ステップの数学、論理的推論、コーディング)、標準的なプロンプトでは十分ではありません。高度なプロンプト技術は、LLM が推測ではなく「段階的に考える」のに役立ちます。
...
魏ら。 (2022) 発見: *「段階的に考えてみましょう」* を追加するか、推論を伴う例を示すだけで、モデルは問題をより適切に解決します。
...
**どのような場合に使用するか:** 重要な問題です。待ち時間とコストの増加を受け入れます。

---

## 3. 思考の木 (ToT)

CoT は単一のパスをたどります。 ToT **推論の複数の分岐を探索**し、各分岐を評価します。
...
ReAct は、**推論** と **アクション** (ツールの呼び出し) をループ内で組み合わせます。
...
|エンジニアリング |コスト (トークン) |レイテンシ |良いこと |
|----------|------|----------|----------|
|標準 |最低 |最速 |簡単なタスク |
|ゼロショットCoT |低い |速い |数学、基本的な論理 |
|少数ショットCoT |平均 |平均 |タスクには特定のパターンがあります |
|自己一貫性 |高 (×N) |遅い (×N) |重要な精度 |
|へ|非常に高い |非常に遅い |複雑な計画 |
|反応する |平均 |バリエーション |ツールの使い方、検索 |
|反射 |高 (×3) |遅い |品質重視の出力 |
...
**次の記事:** RAG (Retrieval Augmented Generation) — LLM が新しい情報や個人データを知らないという問題の解決策。
