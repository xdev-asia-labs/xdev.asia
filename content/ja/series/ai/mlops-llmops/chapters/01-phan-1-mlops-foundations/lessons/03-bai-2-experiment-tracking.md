...
description: >-
  実験的な追跡の詳細: MLflow のセットアップ、パラメータ/メトリクス/アーティファクトのログ記録。
  W&B統合。ハイパーパラメータスイープ。モデルの比較。再現性のためのベストプラクティス。
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="60" y="140" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">レッスン 2: 実験の追跡 — MLflow と</tspan>
      <tspan x="60" dy="42">重みとバイアス</tspan>
  </text>
...
次のようなことはありますか?
- モデルをトレーニングした後、**どのハイパーパラメータが最良の結果をもたらすかを忘れました**?
- **ファイルに名前を付けて 20 件の実験を比較** `model_v1_lr001_final_FINAL_v2.pt`?
- 同僚が「このモデルはどのようなデータでトレーニングされていますか?」と尋ねます。 — それで、あなたは**覚えていない**のですか？

**実験追跡** は、上記の問題をすべて解決します。

> 🎯 **目標:** ML 実験に関連するすべてを追跡します: パラメータ、メトリクス、アーティファクト、コード バージョン。
...
|特長 | MLフロー | W&B |
|--------|--------|-----|
| **価格** |無料＆オープンソース |無料（個人）、有料（チーム） |
| **セルフホスト** | ✅簡単 | ⚠️ はい、しかし複雑です |
| **UI** | ⭐⭐⭐ OK | ⭐⭐⭐⭐⭐ とても美しい |
| **コラボレーション** | ⭐⭐⭐ | ⭐⭐⭐⭐⭐ レポート、コメント |
| **モデル レジストリ** | ✅ 内蔵 | ✅ アーティファクト |
| **スイープ** | ❌ Optuna が必要 | ✅ 内蔵 |
| **自動ログ** | ✅ 多くのフレームワーク | ✅ 多くのフレームワーク |
| **視覚化** | ⭐⭐⭐ | ⭐⭐⭐⭐⭐ インタラクティブ |
| **学習曲線** |簡単 |簡単 |
...
|コンセプト |覚えておいてください |
|----------|----------|
| **実験の追跡** |実行ごとにパラメータ、メトリクス、アーティファクト、コードを追跡 |
| **MLflow** |オープンソース、自己ホスト型のモデル レジストリ |
| **W&B** |クラウドベースの美しい UI、組み込みのスイープ |
| **自動ログ** |コードなしの自動追跡 |
| **スイープ** |自動ハイパーパラメータ検索 |
| **再現性** | git ハッシュ + データ ハッシュ + 環境をログに記録します。

## 演習

1. **MLflow セットアップ:** MLflow をインストールし、3 つの異なるモデル (RF、XGBoost、LightGBM) をトレーニングし、すべてを MLflow に記録します。 UIで比較してみます。
2. **W&B 統合:** W&B アカウントを作成し、PyTorch モデルのトレーニング曲線を記録します。レポートのリンクを共有します。
3. **スイープ:** 1 つのモデルに対してハイパーパラメータ スイープ (50 試行) を実行します。最適なパラメータを見つけます。
4. **再現性:** ログに記録された情報を使用して、演習 1 の最良の実行を再現します。

> **次の記事:** データのバージョニングと機能ストア — DVC、LakeFS、Feast。
