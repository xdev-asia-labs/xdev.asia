...
description: >-
  AI の安全性: ガードレール フレームワーク、入出力検証、PII 検出、コンテンツ フィルタリング、幻覚検出。コンプライアンス:
  GDPR、データ保持。責任ある AI の実践。 NeMo ガードレール、ガードレール AI。
duration_minutes: 120
is_free: true
video_url: null
sort_order: 10
section_title: 'パート 4: 生産とガバナンス'
course:
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="60" y="160" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">レッスン 11: ガードレール、安全性、コンプライアンス</tspan>
  </text>
  ...
  <!-- Section -->
  <text x="60" y="246" font-family="system-ui,-apple-system,sans-serif" font-size="13" fill="#64748b" opacity="0.6">パート 4: 生産とガバナンス</text>
...
- 🤥 **幻覚** → 間違った情報を与える
- 🔐 **PII の漏洩** → 個人情報の漏洩
- 💸 **悪用されている** → API マネーを無駄に浪費する

**ガードレール** = LLM アプリケーションの保護バリア。本番環境では必須です。

> 🎯 **目標:** LLM アプリの入力検証、出力フィルタリング、安全性チェックを実装します。
...
|コンセプト |覚えておいてください |
|----------|----------|
| **入力ガード** |インジェクション検出、PII マスキング、レート制限 |
| **出力ガード** |毒性チェック、幻覚検知、PII漏洩 |
| **NeMo ガードレール** | NVIDIA フレームワーク、構成主導 |
| **ガードレール AI** |バリデータによる出力検証 |
| **GDPR/コンプライアンス** |データ保持、消去の権利、監査ログ |
| **完全なパイプライン** |レート制限 → 入力チェック → PII マスク → LLM → 出力チェック |

## 演習

1. **入力ガード:** プロンプト挿入検出器 (正規表現 + LLM ベース) を実装します。 20 個の攻撃プロンプトを使用してテストします。
2. **PII:** ベトナム語用の PII 検出器とマスカーを構築します (電子メール、電話、CCCD)。テストの精度。
3. **出力の安全性:** 出力検証パイプラインを実装します (毒性 + 幻覚チェック)。
4. **監査:** 監査ログ システムを構築し、100 件のやり取りをログに記録し、コンプライアンス レポートを生成します。

> **次の記事:** Capstone — ML プラットフォームをゼロから構築する。
