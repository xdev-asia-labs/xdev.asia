...
description: >-
  ML インフラストラクチャ: ML 用の Docker、マルチステージ ビルド。 ML ワークロード、GPU スケジューリング用の
  Kubernetes。クラウド ML プラットフォーム: AWS SageMaker、GCP Vertex AI、Azure ML。 Terraform
  を使用したコードとしてのインフラストラクチャ。
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="60" y="140" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">レッスン 6: インフラストラクチャ — Docker、Kubernetes</tspan>
      <tspan x="60" dy="42">&クラウドML</tspan>
  </text>
...
ML モデルはラップトップで実行され、サーバーに転送され、失敗します。 「自分のマシンで動作します」症候群ですが、ML バージョン: CUDA とは異なり、PyTorch とは異なり、numpy バージョンとは異なります...

> 🎯 **この記事:** Docker による ML のコンテナ化、Kubernetes オーケストレーション、Cloud ML プラットフォームの拡張。

---

## 1. ML 用の Docker

### 1.1 基本的な ML Dockerfile
...
### 2.3 スケジュールされた再トレーニングのための CronJob
...
### 3.3 クラウド プラットフォームの比較

|特長 | AWS SageMaker | GCP バーテックス AI |アジュールML |
|----------|--------------|---------------|----------|
| **使いやすさ** | ⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐ |
| **GPU の可用性** | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐⭐ |
| **AutoML** | ✅ オートパイロット | ✅ AutoML | ✅ AutoML |
| **実験の追跡** | ✅ 内蔵 | ✅ 内蔵 | ✅ 内蔵 |
| **モデル登録** | ✅ | ✅ | ✅ |
| **特集ストア** | ✅ | ✅ | ✅ |
| **価格** | 💰💰💰 | 💰💰 | 💰💰💰 |
| **こんな用途に最適** |エンタープライズ |研究 + 製品 |マイクロソフト ショップ |
...
|コンセプト |覚えておいてください |
|----------|----------|
| **ドッカー** | ML のコンテナ化 → 再現可能な環境 |
| **マルチステージ** |ビルドとランタイムを分離 → より小さいイメージ |
| **GPU ドッカー** | nvidia/cuda ベース イメージ + NVIDIA コンテナ ツールキット |
| **Kubernetes** |オーケストレーション: ジョブ (トレーニング)、デプロイメント (サービス)、Cron ジョブ (再トレーニング) |
| **HPA** |サービングポッドの自動スケール |
| **SageMaker** | AWS が管理する ML、エンドツーエンド |
| **Vertex AI** | GCP マネージド ML、研究に最適 |
| **テラフォーム** |コードとしてのインフラストラクチャ |

## 演習

1. **Docker:** ML サービス用の Dockerfile を作成します (マルチステージ、 < 500MB image size).
2. **Compose:** API + MLflow + Prometheus + Grafana を使用して docker-compose を作成します。
3. **K8s:** K8s クラスター (minikube/kind) にサービスを提供するモデルをデプロイします。 HPAを追加します。
4. **クラウド:** モデルを SageMaker または Vertex AI (無料枠) にデプロイします。

> **次の記事:** LLMOps と MLOps — パラダイム シフト。
