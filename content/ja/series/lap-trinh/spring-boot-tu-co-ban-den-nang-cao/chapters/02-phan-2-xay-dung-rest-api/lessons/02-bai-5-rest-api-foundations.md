...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="60" y="140" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">レッスン 5: REST API の基礎 —</tspan>
      <tspan x="60" dy="42">@RestController とリクエストのマッピング</tspan>
  </text>
...
1. エンティティの CRUD REST API を作成する `Book` 完全な HTTP メソッドを使用した (ID、タイトル、著者、isbn、価格、発行年)
2. 検索エンドポイントを実装します。 `GET /api/books?author=X&minPrice=Y&maxPrice=Z` すべてのオプションのパラメータを使用して
3. ResponseEntity を使用して正しいステータス コードを返します (201 Created with Location ヘッダー、204 No Content for DELETE、404 for resource doesn't know)
