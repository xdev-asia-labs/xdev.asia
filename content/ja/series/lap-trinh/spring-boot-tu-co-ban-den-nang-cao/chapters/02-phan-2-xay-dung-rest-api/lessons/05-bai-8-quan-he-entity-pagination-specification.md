...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="60" y="140" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">レッスン 8: エンティティの関係、ページネーションと</tspan>
      <tspan x="60" dy="42">仕様</tspan>
  </text>
...
実際のアプリケーションでは、エンティティは相互に複雑な関係を持っています。リレーションシップをマッピングする方法を理解し、N+1 問題を回避し、ページネーションを実装することは、バックエンド開発者に必要なスキルです。この記事では、JPA リレーションシップと動的クエリについて詳しく説明します。

---

## 1. エンティティの関係

### 1.1 @ManyToOne および @OneToMany
...
**ベスト プラクティス**: 常に使用します `FetchType.LAZY` すべての関係について、必要に応じて JOIN FETCH を使用してフェッチします。
...
API にオプションの複数のフィルター パラメーターがある場合:

```
GET /api/products?category=electronics&minPrice=100&maxPrice=500&status=ACTIVE&keyword=phone
```

すべての組み合わせに対して派生クエリ メソッドを記述することは現実的ではありません。この仕様により、クエリを動的に構築できます。
...
- JPA 関係 (@OneToMany、@ManyToOne、@ManyToMany) はエンティティ間の関係をマップし、常に FetchType.LAZY を使用します。
- JOIN FETCH、@EntityGraph、@BatchSize、または DTO プロジェクションによって解決される N+1 問題
- Spring Data ページネーション (Page、Slice、Pageable) は自動ページネーションとソートをサポートします
- JPA 仕様では、多くのオプションのフィルター パラメーターを使用して動的クエリを構築できます

## 演習

1. 3 つのエンティティを作成します: 著者 (1-N) 書籍 (N-N) タグ。便利なメソッドを使用してリレーションシップを完全に実装する
2. N+1 を使用せずに著者のすべての書籍を取得するクエリを作成します (@EntityGraph または JOIN FETCH を使用)。ログ内のクエリ数で検証する
3. JPA 仕様を使用して書籍の検索 API を実装します。著者、タグ、価格帯、タイトルのキーワードでフィルターします。ページネーションと組み合わせる
