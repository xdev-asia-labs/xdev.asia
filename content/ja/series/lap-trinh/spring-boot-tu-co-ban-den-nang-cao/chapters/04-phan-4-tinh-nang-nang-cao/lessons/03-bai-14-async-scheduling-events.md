...
title: 'レッスン 14: 非同期処理、スケジューリング、およびアプリケーション イベント'
slug: bai-14-async-scheduling-events
description: >-
  @Async と仮想スレッド、CompletableFuture。 @バックグラウンドタスク用にスケジュールされています。 Spring アプリケーション
  イベント — パブリッシュ/サブスクライブ、ドメイン イベント、@TransactionalEventListener。
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="92" y="69" font-family="system-ui,-apple-system,sans-serif" font-size="13" font-weight="600" fill="#2dd4bf">💻 プログラミング — レッスン 13</text>

  <!-- Title -->
  <text x="60" y="140" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">レッスン 14: 非同期処理、スケジュール設定、および</tspan>
      <tspan x="60" dy="42">アプリケーションイベント</tspan>
  </text>
...
非同期処理、タスク スケジューリング、およびイベント駆動型アーキテクチャは、エンタープライズ アプリケーションで広く使用されている 3 つのパターンです。 Spring Boot 4 は仮想スレッド (Java 21+) をサポートしており、非同期処理がこれまでより効率的になります。
...
### 1.3 仮想スレッド (Spring Boot 4 + Java 21+)
...
- @Async と仮想スレッド (Spring Boot 4) を組み合わせて効率的な同時処理を実現し、手動のスレッド プール管理が不要
- @Scheduled は、バックグラウンド タスクの固定レート、固定遅延、cron 式をサポートします
- アプリケーション イベント (パブリッシュ/サブスクライブ) は、ビジネス ロジックの分離に役立ちます。@TransactionalEventListener は、トランザクションのコミット後にのみイベントが処理されるようにします。

## 演習

1. 非同期電子メール サービスを実装する: ユーザーを作成した後、非同期のウェルカム電子メールを送信します。 AsyncUncaughtExceptionHandler でエラーを処理する
2. 毎日午前 2 時に実行するスケジュールされたタスクを作成し、期限切れのセッションと一時ファイルを削除します
3. イベント駆動型の注文処理を実装します: @TransactionalEventListener を使用して、OrderCreatedEvent → NoticeListener + InventoryListener + AnalyticsListener
