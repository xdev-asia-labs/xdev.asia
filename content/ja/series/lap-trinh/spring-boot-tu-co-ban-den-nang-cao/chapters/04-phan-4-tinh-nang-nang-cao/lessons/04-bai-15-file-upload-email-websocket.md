...
description: >-
  検証付きのマルチパート ファイルのアップロード。ファイルダウンロードストリーミング。 Spring Mail および Thymeleaf
  テンプレートを使用して電子メールを送信します。 WebSocketとSTOMPによるリアルタイム通信。
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="60" y="140" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">レッスン 15: ファイルのアップロード/ダウンロード、メール送信、</tspan>
...
ファイルのアップロード/ダウンロード、電子メール通知、WebSocket を介したリアルタイム通信という 3 つの機能は、すべてのエンタープライズ アプリケーションに共通しています。この記事では、各機能の本番環境に対応した実装について説明します。
...
- 検証（タイプ、サイズ）、パストラバーサル防止、および一意のファイル名の生成を伴うファイルアップロード
- Spring Mail はテキスト、HTML、および添付ファイルの電子メールをサポートします — @Async と組み合わせてノンブロッキング送信を実現します
- WebSocket + STOMP によるリアルタイム通信: ブロードキャスト (トピック)、プライベート メッセージ (キュー)、サーバー プッシュ通知

## 演習

1. ファイルアップロード API の実装: 画像 (JPEG、PNG、WebP)、最大 5MB をサポートします。コンテンツタイプを検証し、サムネイルを作成する
2. ユーザー登録時に HTML テンプレートを使用してウェルカム電子メールを送信する電子メール サービスを作成します。 @Async を使用してブロックしないようにする
3. WebSocket を使用してシンプルなチャット ルームを実装します。ユーザーがルームに参加し、メッセージを送信し、リアルタイム メッセージを受信します。
