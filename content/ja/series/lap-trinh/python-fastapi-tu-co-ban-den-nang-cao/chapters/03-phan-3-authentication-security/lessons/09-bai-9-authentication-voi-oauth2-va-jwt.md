...
description: >-
  FastAPI、JWT アクセス トークン、およびリフレッシュ トークンでの OAuth2 パスワード フロー。 Passlib/Bcrypt
  によるパスワードのハッシュ化。トークンのローテーション、ブラックリストへの登録。 OpenAPI のセキュリティ スキーム。
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="92" y="69" font-family="system-ui,-apple-system,sans-serif" font-size="13" font-weight="600" fill="#2dd4bf">💻 プログラミング — レッスン 9</text>

  <!-- Title -->
  <text x="60" y="160" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">レッスン 9: OAuth2 と JWT による認証</tspan>
  </text>
...
<h2 id="1-auth-overview"><strong>1. FastAPIでの認証</strong></h2>

<p>FastAPI は、OpenAPI/OAuth2 標準に準拠した多くの認証スキームをサポートしています。この記事では、REST API で最も一般的な方法である JWT ベースの認証を実装します。</p>

<h3 id="cai-dat"><strong>依存関係をインストールする</strong></h3>

<pre><code class="language-bash">uv add python-jose[cryptography] passlib[bcrypt] python-multipart
</code></pre>

<h2 id="2-password-hashing"><strong>2. パスワードのハッシュ化</strong></h2>
...
<h2 id="3-auth-schemas"><strong>3. 認証スキーマ</strong></h2>
...
<h2 id="4-auth-dependencies"><strong>4. 認証の依存関係</strong></h2>
...
<h2 id="5-auth-routes"><strong>5. 認証ルート</strong></h2>
...
<h2 id="6-protected-routes"><strong>6. ルートで認証を使用する</strong></h2>
...
<h2 id="7-token-blacklist"><strong>7. トークンブラックリスト（上級）</strong></h2>
...
<p>この記事では、完全な認証を実装しました。</p>

<ul>
<li><strong>パスワードのハッシュ化</strong>: Passlib 経由の Bcrypt</li>
<li><strong>JWTトークン</strong>：アクセストークン＋リフレッシュトークン</li>
<li><strong>OAuth2</strong>: OpenAPI標準に従ったパスワードフロー</li>
<li><strong>認証の依存関係</strong>: get_current_user、get_optional_user</li>
<li><strong>トークンローテーション</strong>: リフレッシュトークンの仕組み</li>
<li><strong>トークンブラックリスト</strong>: Redis ベースのログアウト</li>
</ul>

<p>次の記事では、RBAC を使用した認可を実装します。</p>
