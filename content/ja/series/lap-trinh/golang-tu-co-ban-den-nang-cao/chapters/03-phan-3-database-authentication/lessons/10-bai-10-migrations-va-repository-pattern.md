...
description: >-
  golang-migrate、Atlas の移行。リポジトリ パターン、サービス レイヤー、Wire/Fx による依存関係の注入。クリーン アーキテクチャ
  プロジェクトの構造。
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="92" y="69" font-family="system-ui,-apple-system,sans-serif" font-size="13" font-weight="600" fill="#2dd4bf">💻 プログラミング — レッスン 10</text>

  <!-- Title -->
  <text x="60" y="160" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">レッスン 10: 移行とリポジトリ パターン</tspan>
  </text>
...
<h2 id="1-database-migrations"><strong>1. データベースの移行</strong></h2>

<p>GORM AutoMigrateは開発には便利ですが、 <strong>生産には適さない</strong> ロールバックをサポートしておらず、バージョンを追跡せず、安全なスキーマ変更を管理しないためです。専用の移行ツールを使用します。</p>

<h3 id="1-1-golang-migrate"><strong>1.1. golang-移行</strong></h3>
...
<h3 id="1-2-migrate-in-code"><strong>1.2.コードで移行を実行する</strong></h3>
...
<h3 id="1-3-makefile"><strong>1.3.移行用のメイクファイル</strong></h3>
...
<h2 id="2-repository-pattern"><strong>2. リポジトリパターン</strong></h2>

<p>リポジトリ パターンはデータ アクセス ロジックをビジネス ロジックから分離し、コードのテストと保守を容易にします。</p>

<h3 id="2-1-interface"><strong>2.1.リポジトリインターフェース</strong></h3>
...
<h3 id="2-2-gorm-implementation"><strong>2.2. GORMの実装</strong></h3>
...
<h2 id="3-service-layer"><strong>3. サービス層</strong></h2>
...
<h2 id="4-dependency-injection"><strong>4. 依存性の注入</strong></h2>

<h3 id="4-1-manual-di"><strong>4.1.手動 DI (コンストラクター インジェクション)</strong></h3>
...
<h3 id="4-2-wire"><strong>4.2. Google Wire (コード生成 DI)</strong></h3>
...
<h2 id="5-clean-architecture"><strong>5. クリーンなアーキテクチャ</strong></h2>
...
<ul>
<li><strong>移行</strong>: バージョン管理されたスキーマ変更のための golang-merge</li>
<li><strong>リポジトリパターン</strong>: インターフェース + 実装、個別のデータアクセス</li>
<li><strong>サービス層</strong>: ビジネスロジック、検証、オーケストレーション</li>
<li><strong>依存関係の注入</strong>：マニュアル（小）、ワイヤー/FX（大）</li>
<li><strong>クリーンなアーキテクチャ</strong>: ハンドラー → サービス → リポジトリ → モデル</li>
</ul>

<p>次の記事: <strong>認証 - JWT および OAuth2</strong> — JWT トークンを使用した安全な API。</p>
