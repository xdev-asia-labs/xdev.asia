...
title: 'レッスン 18: キャッシュ、ヘルスチェック、API ゲートウェイ'
slug: bai-18-caching-health-checks-api-gateway
description: '@CacheResult を使用した Redis キャッシュ、ヘルスチェック (稼働状況/準備完了)、API ゲートウェイ パターン、レート制限、集約。'
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="92" y="69" font-family="system-ui,-apple-system,sans-serif" font-size="13" font-weight="600" fill="#a78bfa">💻 プログラミング — レッスン 17</text>

  <!-- Title -->
  <text x="60" y="140" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">レッスン 18: キャッシュ、ヘルスチェック、API</tspan>
      <tspan x="60" dy="42">ゲートウェイ</tspan>
  </text>
...
この記事では、**Redis キャッシュ**によるデータベース負荷の軽減、**ヘルス チェック** (Kubernetes の準備/活性プローブ用)、および **API ゲートウェイ** (フロントエンドの単一のエントリ ポイントとして) という 3 つの横断的な懸念事項を備えたマイクロサービス プラットフォームを完成させます。
...
Quarkus は以下を自動的に追加します。
- **データベース** (データソース): データベース接続はOKですか?
- **Kafka**: ブローカーが接続されていますか?
- **Redis**: キャッシュが接続されていますか?
...
1. 製品サービス用の Redis キャッシュを追加します (`getById`、 `listByCategory`）
2.プロダクト作成・更新・削除時のキャッシュ無効化を実装
3. カスタム ヘルス チェックの作成: データベース、外部サービス
4. サービスへの Nginx API Gateway ルーティングを構成する
5. Nginx にレート制限を追加する
6. テスト: 起動 → 準備完了 → 活性プローブ

## 概要

- **Redis キャッシュ** (`@CacheResult`、 `@CacheInvalidate`) DBの負荷を軽減します
- **健康診断** (`/q/health/live`、 `/q/health/ready`) Kubernetes プローブの場合
- **カスタムヘルス**が拡張されます `HealthCheck` — データベース、外部サービスを確認する
- **API ゲートウェイ** (Nginx) — シングル エントリ ポイント、ルーティング、レート制限、TLS 終端
- **レート制限**によりバックエンド サービスを悪用から保護します

次の記事: テスト — @QuarkusTest および Testcontainers。
