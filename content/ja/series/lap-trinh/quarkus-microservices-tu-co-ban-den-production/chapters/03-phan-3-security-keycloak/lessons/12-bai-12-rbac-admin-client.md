...
title: 'レッスン 12: 高度な RBAC と Keycloak 管理クライアント'
slug: bai-12-rbac-nang-cao-keycloak-admin-client
description: 高度なロールベースのアクセス制御、きめ細かい認可、バックエンドからユーザー/ロールを管理するためのKeycloak管理クライアントAPI。
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="92" y="69" font-family="system-ui,-apple-system,sans-serif" font-size="13" font-weight="600" fill="#fbbf24">💻 プログラミング — レッスン 11</text>

  <!-- Title -->
  <text x="60" y="140" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">レッスン 12: 高度な RBAC と Keycloak 管理者</tspan>
      <tspan x="60" dy="42">クライアント</tspan>
  </text>
...
レッスン 11 では基本認証について説明しました `@Authenticated` そして `@RolesAllowed`。この記事では、**きめ細かい RBAC** (リソースレベルの権限) について詳しく説明し、**レルムロールとクライアントロールと複合ロール**、**グループベースのアクセス制御**、ユーザー/ロールをプログラムで管理するための**Keycloak管理クライアント**、**マルチテナンシー**パターン、および実稼働用の**監査ログ**を区別します。
...
各クライアント (サービス) のスコープ — **サービス固有**の権限に使用されます。
...
> **Keycloak でのサービス アカウントのセットアップ**: クライアント `ecommerce-admin` → 設定 → サービスアカウントロール → 割り当て `realm-admin` 役割。役割。
...
1. KeycloakレルムJSONでレルムロール、クライアントロール、複合ロールを設定する
2.実装する `@RequiresOwnership` Order および Product リソースのアノテーション + インターセプター
3. Keycloak管理クライアントの統合: CRUDユーザー、ロールの割り当て/削除、グループの管理
4. Swagger ドキュメントを使用して Admin API エンドポイントを作成する
5. 電子メール検証を備えた自己登録エンドポイントを実装する
6. グループベースのアクセスを構成します: 製品割り当てのある販売者 (プレミアム/ベーシック)
7. 別個のログ ファイルで構造化された監査ログを追加します。
8. テストを作成します。 `@TestSecurity` 管理者、顧客、販売者、リソース所有者向け
9. (上級) Hibernate フィルターまたはレルムごとのアプローチを使用したマルチテナントの実装
