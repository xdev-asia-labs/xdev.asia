...
description: >-
  MLflow 模型登錄：階段轉換、模型版本控制。模型打包：MLflow Models 格式、BentoML、Docker。模型服務模式。 A/B
  測試和金絲雀部署。模型治理。
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="60" y="140" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">第 4 課：模型註冊表、版本控制和</tspan>
      <tspan x="60" dy="42">包裝</tspan>
  </text>
...
模型訓練完成→儲存 `model.pt` → 複製到伺服器 → 祈禱它能運作。聽起來很熟悉嗎？

**模型註冊表**解決模型生命週期管理問題：版本控制、暫存、批准、回滾。

> 🎯 **目標：** 以系統化的方式管理從實驗→分階段→生產的模型。
...
1. **註冊：** 訓練 3 個模型變體，全部註冊，推廣最佳 → 生產。
2. **自訂模型：** 建立內建預處理的 MLflow PythonModel。
3. **Docker：** 將模型打包到Docker容器中，使用FastAPI服務。
4. **A/B 測試：** 在 2 個模型版本之間實施 A/B 測試，記錄預測。

> **下一篇文章：** ML 的 CI/CD — 測試和驗證管道。
