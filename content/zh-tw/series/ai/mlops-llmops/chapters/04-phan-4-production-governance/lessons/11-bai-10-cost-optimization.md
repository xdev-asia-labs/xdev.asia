...
description: LLM成本最佳化：語意快取、提示壓縮、模型路由、令牌最佳化。自託管模型：vLLM、Ollama。用於推理的量化。成本監控和預算。
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="60" y="140" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">第 10 課：成本最佳化 — 快取、</tspan>
      <tspan x="60" dy="42">路由和量化</tspan>
  </text>
...
LLM API 呼叫非常昂貴。每天為 10K 用戶提供服務的 RAG 聊天機器人應用程式的成本僅為 **500-5000 美元/月** 僅 API 成本。本課程教授在不犧牲品質的情況下降低成本的技術。

> 🎯 **目標：** 在維持品質的同時將 LLM 成本降低 50-80%。
...
|型號|輸入（$/1M）|產出（$/1M）|速度|
|--------|-------------|----------------|--------|
| GPT-4o | 2.50 美元 | 10.00 美元 |中|
| GPT-4o-迷你 | 0.15 美元 | 0.60 美元 |快|
|克勞德 3.5 十四行詩 | $3.00 | $15.00 |中|
|克勞德 3.5 俳句 | 0.25 美元 | 1.25 美元 |快|
|雙子座1.5快閃記憶體| 0.075 美元 | 0.30 美元 |非常快|
|駱駝 3.1 70B（自備）| ~$0.00 | ~$0.00 |取決於 GPU |
...
1. **語意緩存：** 實作語意緩存，比較緩存命中率與精確緩存。
2. **路由器：** 建立路由器模型（簡單→迷你，複雜→4o）。對 100 個查詢進行了測試。
3. **成本儀表板：** 建置成本監視器，運行 1000 個 LLM 調用，產生報告。
4. **自架：** 透過 Ollama 部署 Llama 3.1 8B，與 GPT-4o-mini 進行基準測試。
