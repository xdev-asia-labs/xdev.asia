...
description: AI安全：護欄框架、輸入/輸出驗證、PII偵測、內容過濾、幻覺偵測。合規性：GDPR、資料保留。負責任的人工智慧實踐。 NeMo 護欄、護欄 AI。
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="60" y="160" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">第 11 課：護欄、安全與合規性</tspan>
  </text>
  ...
  <!-- Section -->
  <text x="60" y="246" font-family="system-ui,-apple-system,sans-serif" font-size="13" fill="#64748b" opacity="0.6">第 4 部分：生產與治理</text>
...
**護欄** = LLM 應用程式的保護屏障。生產中需要。

> 🎯 **目標：** 為 LLM 應用程式實現輸入驗證、輸出過濾和安全檢查。
...
|概念 |記住|
|--------|--------|
| **輸入防護** |注入偵測、PII 遮蔽、速率限制 |
| **輸出防護** |毒性檢查、幻覺偵測、PII 洩漏 |
| **NeMo 護欄** | NVIDIA 框架，配置驅動 |
| **護欄人工智慧** |使用驗證器進行輸出驗證 |
| **GDPR/合規性** |資料保留、刪除權、審核日誌 |
| **完整管道** |速率限制 → 輸入檢查 → PII 遮罩 → LLM → 輸出檢查 |

## 練習

1. **輸入防護：** 實現提示注入偵測器（基於正規表示式 + LLM）。使用 20 個攻擊提示進行測試。
2. **PII：** 為越南語建立 PII 偵測器和屏蔽器（電子郵件、電話、CCCD）。測試準確度。
3. **輸出安全：**實施輸出驗證管道（毒性+幻覺檢查）。
4. **審核：** 建立審核日誌系統，記錄 100 次交互，產生合規報告。

> **下一篇文章：** Capstone — 從頭開始建立機器學習平台。
