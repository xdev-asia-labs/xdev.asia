...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="60" y="140" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">第 7 課：影像分割－語意，</tspan>
      <tspan x="60" dy="42">實例和全景</tspan>
  </text>
...
物件偵測為我們提供了一個**邊界框**－圍繞物件的矩形。但許多應用需要**更精確**：醫學需要知道腫瘤的**正確**區域，自動駕駛汽車需要知道**正確的像素**哪些是道路，哪些是人行道。這就是**圖像分割**。

> 🎯 **分割** = 將影像中的**每個像素**分類。比檢測更詳細。
...
|概念 |記住|
|--------|--------|
| **語意** |將每個像素分類→類別（不論實例為何）|
| **實例** |區分單一物件 |
| **全景** |結合語意+實例|
| **優網** |編碼器-解碼器 + 跳過連接（醫療）|
| **SegFormer** |基於變壓器，SOTA |
| **YOLO-Seg** |即時實例分割 |
| **骰子損失** |適合分割的損失（類別不平衡）|

## 一般練習

1. **SegFormer 示範：** 在 5 張街道照片上執行預先訓練的 SegFormer。可視化顏色蒙版。
2. **YOLO-Seg：** 短視頻上的實例分割。計算唯一實例。
3. **U-Net Mini：** 在二進位資料集（前景/背景）上訓練簡單的 U-Net。
4. **Mask Overlay：** 在原圖上寫一個函數疊加分割mask（半透明）。

> **下一篇文章：** SAM（分割任何東西）－分割任何物件**無需訓練**。
