...
title: 第 12 課：Capstone — 為企業建造即時圖書館
slug: bai-12-capstone
description: 一般專案：為企業建立提示庫－註冊、版本控制、測試、A/B、部署。應用前 11 課中的所有技巧。
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="60" y="140" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">第 12 課：Capstone — 建置即時函式庫</tspan>
      <tspan x="60" dy="42">企業</tspan>
  </text>
...
這是一堂頂點課程——前 11 課中**所有**技巧的總結。您將建置**企業提示庫**：一個管理、測試、部署提示系統，為多個團隊服務。

最終產品：團隊可以在其中**瀏覽提示**、**編輯**、**測試**、**A/B 測試**和**部署** - 全部通過 UI 或 API。
...
|模組 |應用技術|參考文章|
|--------|--------------------|--------------|
| **註冊表** | YAML + Pydantic + 快取 |第 11 課 |
| **渲染** |模板變量，系統+使用者 |第 1-3 課 |
| **測試運行者** |黃金資料集，斷言|第 10 課 |
| **A/B 測試** |流量分割，t 檢定 |第 11 課 |
| **API** | FastAPI，結構化輸出 |第 6 課 |
| **儀表板** | Streamlit 視覺化 |第 8 課 |
| **CI/CD** | GitHub Actions 管道 |第 11 課 |

## 總結練習（Capstone 專案）

1. **MVP（2-3小時）：** 註冊表+ API + 3個提示+黃金測試。運行 `uvicorn api.main:app` 並使用curl/httpx進行測試。
2. **完整（1-2 天）：** 新增 A/B 測試、Streamlit 儀表板、CI/CD 管道。部署在本地或 VPS 上。
3. **生產（1週）：** 新增身分驗證、成本追蹤、團隊管理、金絲雀部署。在雲端部署（fly.io 或 Railway）。
4. **示範：** 錄製 5 分鐘示範影片：建立提示 → 編輯 → 測試 → A/B 測試 → 部署。與團隊/社群分享。

> **恭喜！ ** 您已完成即時工程大師班。現在您可以將所有這些技術應用到實際專案中。
