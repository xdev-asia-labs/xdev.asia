...
> Detecting the disease is the first step.放射治疗师需要知道肿瘤的**精确边界**才能发射射线。外科医生需要知道肿瘤附着在哪些血管上。 This is a segmentation problem.
...
标准 U-Net 问题：跳过连接传输**所有**特征，包括背景噪声。 **注意力门**学习过滤——只保留分割任务的重要特征：
...
3. 实现滑动窗口推理：不是将整个 CT 大小调整为 512x512，而是将其划分为 50% 重叠的 256x256 块，预测每个块，然后再次缝合。与调整大小方法相比，计算推理时间和内存使用量。
