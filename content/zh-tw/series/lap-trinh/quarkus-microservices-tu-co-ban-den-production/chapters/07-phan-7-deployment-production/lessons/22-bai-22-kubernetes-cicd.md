...
title: 第 22 課：Kubernetes 部署和 CI/CD
slug: bai-22-kubernetes-cicd
description: >-
  將微服務部署到 Kubernetes、Helm 圖表、ConfigMaps/Secrets、HPA 自動縮放、GitHub Actions CI/CD、在
  K8s 上使用 Prometheus + Grafana 進行監控。
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="92" y="69" font-family="system-ui,-apple-system,sans-serif" font-size="13" font-weight="600" fill="#f472b6">💻 程式設計 — 第 21 課</text>

  <!-- Title -->
  <text x="60" y="160" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">第 22 課：Kubernetes 部署和 CI/CD</tspan>
  </text>
...
> **生產**：使用**密封的秘密**或**外部秘密操作員**而不是普通的秘密。
...
1. 產生 Kubernetes 清單 `quarkus-kubernetes` 延伸
2. 為電商平台創建Helm圖表
3. 設定 GitHub Actions CI/CD 管道
4.部署到本地Kubernetes（minikube/kind）
5.配置HPA並檢查自動縮放
6.設定Prometheus + Grafana監控

## 系列總結

透過 22 節課程，您已經完全建立了：

1. **Quarkus 平台** — REST API、PostgreSQL、驗證
2. **微服務設計** - DDD，每個服務資料庫，4個實用服務
3. **安全性** — Keycloak OIDC、RBAC、代幣傳播
4. **通訊** — REST 用戶端、gRPC、Kafka 事件驅動
5. **彈性與可觀察性** — 容錯、OpenTelemetry、緩存
6. **測試** — @QuarkusTest，合約測試
7. **生產** — GraalVM Native、Kubernetes、CI/CD
