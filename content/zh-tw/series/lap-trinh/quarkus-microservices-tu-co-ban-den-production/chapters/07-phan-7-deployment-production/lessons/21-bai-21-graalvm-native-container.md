...
title: 第 21 課：GraalVM 本機鏡像與容器
slug: bai-21-graalvm-native-container
description: 使用 GraalVM、多階段 Dockerfile 建置本機執行檔、比較 JVM 與本機效能、本機測試、反射配置、容器最佳化。
duration_minutes: 100
is_free: false
video_url: null
sort_order: 20
section_title: 第 7 部分：部署與生產
course:
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="92" y="69" font-family="system-ui,-apple-system,sans-serif" font-size="13" font-weight="600" fill="#818cf8">💻 程式設計 — 第 20 課</text>

  <!-- Title -->
  <text x="60" y="160" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">第 21 課：GraalVM 本機鏡像與容器</tspan>
  </text>
  ...
  <!-- Section -->
  <text x="60" y="246" font-family="system-ui,-apple-system,sans-serif" font-size="13" fill="#64748b" opacity="0.6">第 7 部分：部署與生產</text>
...
Quarkus 是從頭開始為 **GraalVM Native Image** 設計的。本機可執行檔在 **~20ms** 內啟動（相對於 2-5s JVM），使用 **~30MB RSS** （相對於 150-300MB JVM）。這是 Kubernetes 上微服務的一大優點：快速擴充、節省資源。
...
|指標| JVM 模式 |本機模式 |
|--------|----------|-------------|
| **啟動時間** | 2-5秒| 0.015-0.05秒|
| **記憶體 (RSS)** | 150-300 MB | 30-80 MB |
| **Docker 映像** | 〜300 MB | 〜80 MB |
| **建置時間** | 10-30 秒 | 3-10 分鐘 |
| **峰值吞吐量** |更高（JIT）|降低約 10-20% |
| **熱身** |需要熱身|不需要|
| **反思** |自由|需要配置|
...
### Native 的 application.properties
...
1. 為產品服務建置本機可執行檔： `./mvnw package -Dnative`
2. 比較啟動時間與記憶體：JVM vs Native
3. 為原生映像檔編寫多階段Dockerfile
4. 跑步 `@QuarkusIntegrationTest` 在本機二進位檔案上
5. Docker Compose啟動整個系統（5個服務+基礎設施）

## 總結

- **GraalVM Native Image**：啟動 ~20ms，RSS ~30MB — 非常適合微服務
- **多階段 Dockerfile**：建置階段 (Mandrel) → 執行階段 (quarkus-micro-image ~80MB)
- **`@RegisterForReflection`** 對於使用動態反射的類
- **`@QuarkusIntegrationTest`** 在實際的本機二進位檔案上進行測試
- Docker Compose 用於開發/登台，Kubernetes 用於生產（第 22 課）

下一篇文章：Kubernetes 部署和 CI/CD — 部署到 Kubernetes 叢集。
