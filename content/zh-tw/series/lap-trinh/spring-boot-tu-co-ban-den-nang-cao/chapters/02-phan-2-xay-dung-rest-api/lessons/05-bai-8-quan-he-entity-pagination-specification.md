...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="60" y="140" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">第 8 課：實體關係、分頁和</tspan>
      <tspan x="60" dy="42">規格</tspan>
  </text>
...
在實際應用中，實體之間存在著複雜的關係。了解如何映射關係、避免N+1問題以及實現分頁是後端開發人員所需的技能。本文深入探討 JPA 關係和動態查詢。

---

## 1. 實體關係

### 1.1 @ManyToOne 和 @OneToMany
...
**最佳實務**：永遠使用 `FetchType.LAZY` 對於所有關係，然後根據需要使用 JOIN FETCH 進行取得。
...
- JPA 關係（@OneToMany、@ManyToOne、@ManyToMany）映射實體之間的關係，並始終使用 FetchType.LAZY
- 透過 JOIN FETCH、@EntityGraph、@BatchSize 或 DTO 投影解決 N+1 問題
- Spring Data Pagination（Page、Slice、Pageable）支援自動分頁和排序
- JPA 規範允許使用許多可選的過濾器參數來建立動態查詢

## 練習

1. 建立 3 個實體：作者 (1-N) 圖書 (N-N) 標籤。用便捷的方法充分實現關係
2. 寫一個查詢來取得所有有作者的書籍，無需 N+1（使用 @EntityGraph 或 JOIN FETCH）。透過日誌中的查詢數量進行驗證
3. 實作具有 JPA 規範的書籍搜尋 API：按作者、標籤、價格範圍、標題中的關鍵字過濾。與分頁結合
