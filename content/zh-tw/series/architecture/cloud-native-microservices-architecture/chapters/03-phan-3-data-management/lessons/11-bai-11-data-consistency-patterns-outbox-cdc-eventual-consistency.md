...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="60" y="140" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">第 11 課：資料一致性模式 —</tspan>
      <tspan x="60" dy="42">寄件匣、CDC 和最終一致性</tspan>
  </text>
...
![第 11 課：資料一致性模式 — 寄件匣、CDC 與最終一致性](/storage/uploads/2026/03/cn-bai-11-diagram.png)

## 簡介

當每個微服務都有自己的資料庫時，最大的挑戰是確保服務之間的**資料一致性**。本課程深入探討解決雙重寫入、可靠事件發布和最終一致性的模式。
...
當服務需要**更新資料庫**和**發布事件**時，如果其中任何一個失敗→不一致：
...
將事件記錄到與業務資料**相同的資料庫事務**中的**寄件箱表**。一個單獨的進程（Relay/Poller）讀取發件箱並將其發佈到訊息代理程式：
...
CDC 從**事務日誌**（PostgreSQL 中的 WAL）捕獲**資料庫變更**（INSERT、UPDATE、DELETE）並直接串流到訊息代理程式：
...
- **雙寫問題**是微服務中資料不一致的根本原因
- **寄件匣模式**透過將事件寫入相同資料庫交易來解決
- **CDC (Debezium)** 讀取 WAL 流以實現近乎即時的事件傳遞
- **冪等消費者**確保精確一次處理，儘管至少一次交付
