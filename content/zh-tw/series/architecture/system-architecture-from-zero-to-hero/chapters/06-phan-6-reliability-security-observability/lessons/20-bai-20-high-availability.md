...
description: 可用性指標（九）。冗餘模式：主動-主動、主動-被動。故障轉移策略。健康檢查和心跳。混沌工程原理。優雅的降級。為失敗心態而設計。
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="92" y="69" font-family="system-ui,-apple-system,sans-serif" font-size="13" font-weight="600" fill="#a78bfa">🏗️ 建築 — 第 20 課</text>

  <!-- Title -->
  <text x="60" y="140" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">第 20 課：高可用性和故障</tspan>
      <tspan x="60" dy="42">公差</tspan>
  </text>
...
「一切都會失敗，一直如此。」— 沃納‧沃格爾斯，亞馬遜首席技術長。高可用性 (HA) 並不是為了防止故障，而是為了在發生故障時系統**繼續運作**。
...
|戰略| MTBF 影響 |平均修復時間 (MTTR) 影響複雜性 |
|----------|-------------|-------------|------------|
| 冗餘 |中性| ⬇️ 快速故障轉移 |中等|
|健康檢查| ⬆️ 及早發現 | ⬇️ 自動恢復 |低|
|優雅的惡化|中性| ⬇️ 部分服務 |中等|
|混沌工程| ⬆️ 尋找弱點 | ⬇️ 更好的操作手冊 |高|
| 自動縮放| ⬆️ 處理尖刺 |中性|中等|
...
1. **可用性計算：** 系統包括：LB（99.99%）→ 3台App Server（各99.9%，雙活）→ DB Primary（99.95%）+ DB Replica（99.95%，熱備）。計算總體可用性。

2. **故障轉移設計：** PostgreSQL 叢集：1 個主節點 + 2 個副本節點。主要崩潰。編寫詳細的故障轉移流程：偵測、升級、重連、驗證。

3. **混沌計畫：** 為電商系統設計5個混沌實驗（API、資料庫、Redis、S3、支付網關）。對於每個實驗：行動、假設、預期行為、回溯計畫。
