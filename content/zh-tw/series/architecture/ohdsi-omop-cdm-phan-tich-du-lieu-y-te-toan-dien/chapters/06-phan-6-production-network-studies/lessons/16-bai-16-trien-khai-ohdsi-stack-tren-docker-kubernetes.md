...
description: >-
  Docker Compose for OHDSI 堆疊（PostgreSQL、WebAPI、ATLAS）、Kubernetes Helm 圖表、備份/還原
  CDM、使用 Prometheus/Grafana 進行監控、效能調整 PostgreSQL。
duration_minutes: 130
is_free: true
video_url: null
sort_order: 16
section_title: 第 6 部分：生產與網絡研究
course:
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="60" y="140" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">第 16 課：在 Docker 上部署 OHDSI 堆疊</tspan>
      <tspan x="60" dy="42">& 庫伯內特斯</tspan>
  </text>
  ...
  <!-- Section -->
  <text x="60" y="268" font-family="system-ui,-apple-system,sans-serif" font-size="13" fill="#64748b" opacity="0.6">第 6 部分：生產與網絡研究</text>
...
本文將指導您為生產環境部署 **整個 OHDSI 堆疊** (PostgreSQL + WebAPI + ATLAS + ACHILLES)，包括用於單節點的 Docker Compose 和用於叢集的 Kubernetes Helm。
...
|組件|碼頭工人 |庫伯內特 |
|------------|--------|------------|
| PostgreSQL |容器+體積| StatefulSet+PVC|
|網頁API |容器+環境 |部署（副本）|
|阿特拉斯 |容器 + config-local.js |部署+ConfigMap |
| R 伺服器 |容器+掛載腳本|作業/CronJob |
|秘密 | Docker 的秘密 | K8s 的秘密 |
|網路| docker-compose 網路 |服務+入口|
|備份|排程任務 + pg_dump |排程任務 |
|監控| postgres 導出器 + Grafana |服務監控 |

**下一篇文章**：網路研究與最佳實務 - 多中心研究
