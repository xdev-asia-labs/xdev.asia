...
title: 'Bài 24: High Availability, Clustering và Multi-site'
slug: bai-24-high-availability-clustering-va-multi-site
description: >-
  Keycloak High Availability concepts, Infinispan distributed caches (embedded
  vs external), cache stack configuration (kubernetes, jdbc-ping, dns-ping),
  session replication, external Infinispan server setup, multi-site/cross-datacenter
  deployment, active-passive vs active-active patterns, database replication
  (PostgreSQL Patroni, PgBouncer), load balancer sticky sessions, split-brain
  handling và disaster recovery strategies.
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="92" y="69" font-family="system-ui,-apple-system,sans-serif" font-size="13" font-weight="600" fill="#34d399">🔒 DevSecOps — Bài 24</text>

  <!-- Title -->
  <text x="60" y="140" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">Bài 24: High Availability, Clustering và</tspan>
...
<p>Keycloak High Availability đảm bảo hệ thống authentication/authorization hoạt động liên tục, ngay cả khi một hoặc nhiều nodes gặp sự cố. HA dựa trên ba trụ cột: <strong>clustering (Infinispan)</strong>, <strong>database replication</strong>, và <strong>load balancing</strong>.</p>
...
<p>Keycloak sử dụng hai loại cache Infinispan với mục đích khác nhau:</p>

<table>
<thead>
<tr><th>Cache Type</th><th>Mô tả</th><th>Keycloak Caches</th><th>Cluster Mode</th></tr>
</thead>
<tbody>
<tr><td><strong>Local Cache</strong></td><td>Lưu trữ data trên node hiện tại, dùng cho metadata có thể đọc lại từ DB</td><td><code>realms</code>, <code>users</code>, <code>authorization</code>, <code>keys</code></td><td>Local + invalidation messages</td></tr>
<tr><td><strong>Distributed Cache</strong></td><td>Data phân tán trên N owners trong cluster, dùng cho session data</td><td><code>sessions</code>, <code>authenticationSessions</code>, <code>offlineSessions</code>, <code>clientSessions</code>, <code>offlineClientSessions</code></td><td>Distributed (2 owners default)</td></tr>
<tr><td><strong>Replicated Cache</strong></td><td>Data replicated trên tất cả nodes</td><td><code>work</code> (cluster communication)</td><td>Replicated</td></tr>
</tbody>
</table>

<h3 id="22-keycloak-caching-architecture-chi-tiet"><strong>2.2 Keycloak Caching Architecture Chi tiết</strong></h3>
...
<p>Mặc định, Keycloak nhúng Infinispan trong JVM process. Các node phát hiện nhau qua discovery protocol và tự động form cluster:</p>
...
<p>Cho multi-site deployment hoặc khi cần quản lý cache layer riêng biệt, sử dụng External Infinispan server:</p>
...
<tbody>
<tr><td>Setup complexity</td><td>Đơn giản, zero config</td><td>Cần deploy Infinispan cluster riêng</td></tr>
<tr><td>Multi-site</td><td>❌ Không hỗ trợ</td><td>✅ Cross-datacenter replication</td></tr>
<tr><td>Scalability</td><td>Tốt cho ≤ 10 nodes</td><td>Tốt cho large-scale deployments</td></tr>
<tr><td>Management</td><td>Tự động</td><td>Cần monitor/manage Infinispan riêng</td></tr>
...
<p>Trên Kubernetes, sử dụng <strong>KUBE_PING</strong> (dns.DNS_PING) để các Keycloak pods tự phát hiện nhau qua Kubernetes API hoặc DNS:</p>
...
<p>Keycloak Kubernetes cần một <strong>headless Service</strong> để DNS discovery hoạt động:</p>
...
<p>Khi deploy trên VMs (không có Kubernetes), sử dụng <strong>JDBC_PING</strong> để các node phát hiện nhau qua shared database:</p>
...
<p>Cấu hình Keycloak kết nối đến External Infinispan:</p>
...
<p>Trong active-passive, chỉ <strong>primary site</strong> phục vụ traffic. Backup site ở trạng thái standby, sẵn sàng take over khi primary gặp sự cố:</p>
...
<p>Trong active-active, <strong>cả hai sites</strong> đều phục vụ traffic đồng thời. Phức tạp hơn nhưng tận dụng tối đa resources:</p>
...
<tbody>
<tr><td>Traffic handling</td><td>1 site tại một thời điểm</td><td>Cả 2 sites đồng thời</td></tr>
<tr><td>Resource utilization</td><td>50% (passive site idle)</td><td>100%</td></tr>
<tr><td>Complexity</td><td>Thấp hơn</td><td>Cao hơn (conflict resolution)</td></tr>
<tr><td>Failover time</td><td>Cần DNS switch (seconds-minutes)</td><td>Tự động (GSLB)</td></tr>
<tr><td>Data consistency</td><td>Strong (synchronous repl)</td><td>Eventual (async cross-site)</td></tr>
<tr><td>DB requirement</td><td>Primary-Standby</td><td>Multi-master hoặc shared DB</td></tr>
</tbody>
</table>
...
<p><strong>Patroni</strong> quản lý PostgreSQL HA (automatic failover), <strong>PgBouncer</strong> cung cấp connection pooling:</p>
...
<p>Keycloak <strong>yêu cầu sticky sessions</strong> cho authentication flows (login, registration). Session affinity dựa trên cookie <code>KC_ROUTE</code> (trước đây <code>KEYCLOAK_SESSION</code>):</p>
...
<p>Split-brain xảy ra khi network partition chia cluster thành 2+ partitions, mỗi partition nghĩ phía còn lại đã chết:</p>
...
<tbody>
<tr><td><code>PREFERRED_NON_NULL</code></td><td>Giữ entry non-null khi merge</td><td>Sessions - tốt hơn keep session hơn mất</td></tr>
<tr><td><code>REMOVE_ALL</code></td><td>Xóa tất cả conflicting entries</td><td>Khi data consistency quan trọng nhất</td></tr>
<tr><td><code>PREFERRED_ALWAYS</code></td><td>Giữ entry từ partition lớn hơn</td><td>General purpose</td></tr>
</tbody>
</table>
...
<p>Ví dụ hoàn chỉnh với 2 Keycloak nodes + External Infinispan + PostgreSQL:</p>
...
<h2 id="12-infinispan-cli-va-monitoring"><strong>12. Infinispan CLI và Monitoring</strong></h2>
