...
description: >-
  GitHub Actions CI/CD pipeline. Deploy lên AWS/GCP với Kubernetes. Database migration
  với Flyway. Zero-downtime deployment. Production checklist & best practices.
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="92" y="69" font-family="system-ui,-apple-system,sans-serif" font-size="13" font-weight="600" fill="#fb923c">💻 Lập trình — Bài 23</text>

  <!-- Title -->
  <text x="60" y="140" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">Bài 24: CI/CD, Cloud Deployment &amp;</tspan>
...
Bài cuối cùng của series tổng hợp mọi thứ cần thiết để đưa Spring Boot application lên production: CI/CD pipeline, database migration, Kubernetes deployment, zero-downtime updates, và production checklist.

---

## 1. Database Migration với Flyway
...
> **Quy tắc**: Không bao giờ sửa migration file đã chạy. Tạo migration mới để thay đổi schema.
...
### 3.3 Kubernetes Probes trong Spring Boot
...
| JWT với expiration ngắn (15-30min) | ☐ |
| Rate Limiting | ☐ |
| Input Validation tất cả endpoints | ☐ |
| SQL Injection protection (JPA) | ☐ |
| Secrets trong env vars / vault | ☐ |
...
| @Transactional(readOnly=true) cho read queries | ☐ |
...
- Flyway quản lý database migration: version control cho schema, không bao giờ sửa migration đã chạy
- GitHub Actions CI/CD: test → build Docker image → push to registry → deploy to Kubernetes
- Kubernetes: rolling update với readiness/liveness probes, graceful shutdown cho zero-downtime deployment
- Production checklist: security headers, connection pooling, caching, monitoring — đảm bảo ứng dụng production-ready

## Bài tập

1. Setup Flyway: tạo migration files cho tất cả entities trong project, test với `./gradlew flywayMigrate`
2. Viết GitHub Actions CI/CD: chạy tests với PostgreSQL service, build Docker image, push to GHCR
3. Tạo Kubernetes manifests: Deployment (3 replicas), Service, Ingress. Cấu hình probes và graceful shutdown. Test rolling update
