...
title: 'Bài 12: RBAC nâng cao & Keycloak Admin Client'
slug: bai-12-rbac-nang-cao-keycloak-admin-client
description: >-
  Role-Based Access Control nâng cao, Fine-grained Authorization,
  Keycloak Admin Client API để quản lý users/roles từ backend.
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="92" y="69" font-family="system-ui,-apple-system,sans-serif" font-size="13" font-weight="600" fill="#fbbf24">💻 Lập trình — Bài 11</text>

  <!-- Title -->
  <text x="60" y="140" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">Bài 12: RBAC nâng cao &amp; Keycloak Admin</tspan>
...
Bài 11 đã cover authentication cơ bản với `@Authenticated` và `@RolesAllowed`. Bài này đi sâu vào **Fine-grained RBAC** (resource-level permissions), phân biệt **Realm Roles vs Client Roles vs Composite Roles**, **Group-based Access Control**, **Keycloak Admin Client** để quản lý users/roles programmatically, **Multi-tenancy** pattern, và **Audit Logging** cho production.
...
Scope toàn realm — dùng cho **cross-service** permissions:
...
Scope từng client (service) — dùng cho **service-specific** permissions:
...
Gom nhiều roles thành 1 role cha:
...
### Sử dụng Client Roles trong Quarkus

Để `@RolesAllowed` nhận client roles, cần map claim trong token:
...
### Cấu hình Groups trong Keycloak
...
### Map Groups vào Token

Trong Keycloak Admin Console → Client Scopes → tạo mapper:
...
### Cách 2: Custom `@ResourceOwner` Annotation (tái sử dụng)
...
Thêm custom attributes vào token qua Keycloak Admin → Client Scopes → Mappers:
...
Hoặc dùng **Script Mapper** cho logic phức tạp:
...
### Đọc Custom Claims trong Quarkus
...
> **Service Account setup trong Keycloak**: Client `ecommerce-admin` → Settings → Service Account Roles → assign `realm-admin` role.
...
Cho SaaS — mỗi tenant là 1 realm hoặc 1 group + tenant attribute:
...
### Audit Log trong application.properties
...
1. Cấu hình Realm Roles, Client Roles và Composite Roles trong Keycloak realm JSON
2. Implement `@RequiresOwnership` annotation + interceptor cho Order và Product resources
3. Tích hợp Keycloak Admin Client: CRUD users, assign/remove roles, manage groups
4. Tạo Admin API endpoints với Swagger documentation
5. Implement Self-Registration endpoint với email verification
6. Cấu hình Group-based access: sellers (premium/basic) với product quota
7. Thêm Audit Logging structured với separate log file
8. Viết tests: `@TestSecurity` cho admin, customer, seller, resource ownership
9. (Nâng cao) Implement Multi-tenancy với Hibernate Filter hoặc per-realm approach
...
| **Composite Roles** | Gom roles: seller = product:create + product:view + ... |
| **Groups** | Tổ chức users: /sellers/premium, /staff/support |
| **@RequiresOwnership** | Resource-level authorization, admin bypass |
| **Custom Claims** | Token enrichment qua Keycloak Protocol Mapper |
| **Admin Client** | CRUD users, roles, groups, sessions programmatically |
| **Multi-tenancy** | Per-group hoặc per-realm tenant isolation |
| **Audit Logging** | Structured logs cho security events, separate file |
| **@TestSecurity** | Mock authentication/authorization trong tests |

Bài tiếp theo: REST Client — Giao tiếp đồng bộ giữa Microservices.
