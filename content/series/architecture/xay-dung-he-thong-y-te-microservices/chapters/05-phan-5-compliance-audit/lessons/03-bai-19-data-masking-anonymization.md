...
description: >-
  ...
  k-anonymity/l-diversity/t-closeness cho datasets, tokenization
  cho sensitive fields, synthetic data generation cho testing,
  và Quarkus implementation cho data de-identification pipeline.
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="92" y="69" font-family="system-ui,-apple-system,sans-serif" font-size="13" font-weight="600" fill="#38bdf8">🏗️ Kiến trúc — Bài 19</text>

  <!-- Title -->
  <text x="60" y="140" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">Bài 19: Data Masking, Anonymization &amp;</tspan>
...
### 2.1. Danh sách 18 Identifiers phải xóa

| # | Identifier | Ví dụ | Implementation |
|---|-----------|-------|----------------|
| 1 | Names | Nguyễn Văn A | Remove hoặc replace với pseudonym |
| 2 | Geographic data (< state) | 123 Nguyễn Huệ, Q1, HCM | Remove address; keep state/province only |
| 3 | Dates (trừ year) | 15/03/1990 → 1990 | Generalize to year only |
...
| 12 | Vehicle identifiers | 51A-12345 | Remove |
...
Expert Determination method cho phép linh hoạt hơn Safe Harbor nhưng yêu cầu:

1. Một chuyên gia thống kê hoặc khoa học đánh giá dữ liệu
2. Chuyên gia xác nhận rủi ro re-identification là **"very small"**
...
```sql
-- Dynamic Data Masking cho PostgreSQL
-- Tạo masked views dựa trên user role

-- Base table (chứa PHI đầy đủ)
-- healthcare.patients (đã tạo từ bài trước)

-- === Masked View cho Clinical Staff ===
-- Nhân viên y tế: thấy tên, giới tính, tuổi nhưng không thấy SSN, full address
CREATE OR REPLACE VIEW healthcare.patients_clinical_view AS
SELECT
    id,
    mrn,
    full_name,  -- Clinical cần biết tên BN
    CASE
    ...
    CASE
        ...
        THEN address
        ELSE regexp_replace(address, '^[^,]+,\s*', '', 'g')  -- Chỉ giữ city/province
    END AS address,
    diagnosis_codes,
    department,
    hospital_id,
...
-- === Masked View cho Research/Analytics ===
-- Researcher: chỉ thấy de-identified data
CREATE OR REPLACE VIEW healthcare.patients_research_view AS
SELECT
    gen_random_uuid() AS research_id,  -- Random ID mỗi query
    NULL AS mrn,
    NULL AS full_name,
    NULL AS ssn,
    ...
    CASE
        WHEN address ILIKE '%hồ chí minh%' OR address ILIKE '%hcm%' THEN 'Hồ Chí Minh'
    ...
    diagnosis_codes,
    department,
    created_at::DATE AS created_date  -- Bỏ time component
FROM healthcare.patients;

-- === Masked View cho Billing ===
-- Billing staff: thấy thông tin thanh toán, không thấy clinical data
...
SELECT
    ...
    email,
    address,
    NULL AS diagnosis_codes,  -- Billing không cần biết diagnoses
    department,
    hospital_id,
    created_at
FROM healthcare.patients;
...
-- === Row-Level Security ===
-- Đảm bảo mỗi department chỉ thấy patients của mình
...
-- KHÔNG grant trực tiếp trên base table
...
/**
 * K-Anonymity implementation cho healthcare datasets.
 */
@ApplicationScoped
public class KAnonymityService {
    ...
    /**
     * Áp dụng k-anonymity cho dataset.
     * @param records Dataset gốc
     * @param k Minimum group size (recommended: k ≥ 5)
     * @param quasiIdentifiers Danh sách quasi-identifier fields
     * @return Dataset đã k-anonymized
     */
    public List<Map<String, Object>> anonymize(
        ...
        if (!valid) {
            // Apply suppression cho groups < k
    ...
    /**
     * Generalize values dựa trên field type.
     */
    private Object generalizeValue(String fieldName, Object value) {
        ...
        return switch (fieldName) {
            ...
            case "gender" -> value; // Giữ nguyên hoặc suppress nếu cần
            default -> value;
        };
    }

    /**
     * Generalize age thành 5-year ranges.
    ...
    /**
     * Verify dataset đạt k-anonymity.
     */
    ...
    /**
     * Suppress (remove) records trong groups nhỏ hơn k.
     */
...
|--------|-----------------|----------|
| K-Anonymity | Identity disclosure | Homogeneity attack (cùng sensitive value) |
| L-Diversity | Attribute disclosure | Skewness attack (phân phối không đều) |
| T-Closeness | Distribution disclosure | Phức tạp hơn, khó implement |

## 7. Tokenization với Format-Preserving Encryption
...
### 8.1. Synthetic Data Generator cho Testing
...
| Encryption | Yes (with key) | No | Storage, transport | Still PHI |

## Tổng kết

Trong bài học này, chúng ta đã triển khai comprehensive **Data Masking, Anonymization & De-identification** cho healthcare:

1. **HIPAA Safe Harbor Method**: Implementation xóa tất cả 18 identifiers, text scrubbing cho clinical notes, date generalization (year only, suppress if age > 89)
...
5. **K-Anonymity**: Implementation với generalization và suppression, verify k ≥ 5 cho healthcare datasets
6. **L-Diversity & T-Closeness**: Concepts và so sánh — bảo vệ chống homogeneity và skewness attacks
7. **Tokenization (FPE)**: Format-preserving tokenization cho SSN, phone, MRN — giữ format, thay giá trị
8. **Synthetic Data Generation**: Vietnamese patient data generator cho dev/test — zero real PHI
9. **De-identification REST API**: Quarkus endpoints cho Safe Harbor, tokenization, batch processing, synthetic data
10. **Pipeline Architecture**: End-to-end masking pipeline từ production → staging → masked → dev/test

## Bài tập

1. **Safe Harbor Implementation**: Implement SafeHarborDeidentifier cho project của bạn. Tạo 10 sample PatientRecord objects. De-identify tất cả 10 records. Verify: không có name, SSN, phone, email, address chi tiết trong output. Verify: diagnosis codes, medications, gender được giữ lại. Test deidentifyText() với một đoạn clinical note chứa tên, SSN, ngày tháng.

2. **Dynamic Masking với PostgreSQL**: Tạo 3 masked views (clinical, research, billing) trong PostgreSQL. Tạo 3 database roles tương ứng. Set session variable `app.user_role` trước khi query. Verify: research_view không hiển thị name, SSN, full address. Verify: clinical_view hiển thị name nhưng mask SSN. Test Row-Level Security: user cardiology không thấy patients của surgery.

3. **K-Anonymity**: Tạo dataset 100 records với quasi-identifiers (age, zip, gender). Implement k-anonymity với k=5. Verify: mỗi equivalence class có ≥ 5 records. Đo data utility loss (bao nhiêu records bị suppress). So sánh k=3 vs k=5 vs k=10 về utility loss.

4. **Synthetic Data Pipeline**: Implement SyntheticDataGenerator. Generate 1000 synthetic patients. Export sang CSV và import vào test database. Run application tests với synthetic data. Verify: không có real PHI trong test database bằng regex scan script.
...
|:---|---:|
| [Bài 18: Centralized Audit Trail với OpenTelemetry & ELK Stack](/series/bao-mat-du-lieu-y-te-cho-microservices/bai-18-centralized-audit-trail-opentelemetry-elk) | [Bài 20: Backup, Disaster Recovery & Business Continuity cho Dữ liệu Y Tế](/series/bao-mat-du-lieu-y-te-cho-microservices/bai-20-backup-dr-business-continuity-du-lieu-y-te) |
<!-- SERIES-NAV:END -->
