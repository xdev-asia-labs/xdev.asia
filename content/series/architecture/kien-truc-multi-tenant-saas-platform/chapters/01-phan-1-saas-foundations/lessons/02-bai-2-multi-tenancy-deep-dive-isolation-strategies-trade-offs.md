...
## 1. Ba chiến lược tenant isolation: Silo (DB per tenant), Pool (shared DB), Bridge (schema per tenant)

### 1.1 Khái niệm cơ bản

Ba chiến lược tenant isolation: Silo (DB per tenant), Pool (shared DB), Bridge (schema per tenant) là một trong những chủ đề quan trọng nhất trong lĩnh vực này. Hiểu rõ các concepts cốt lõi sẽ giúp bạn thiết kế hệ thống đúng đắn ngay từ đầu.
...
## 2. Trade-offs về cost, security, performance
...
Trong bài học này, chúng ta đã tìm hiểu về Multi-tenancy Deep Dive - Isolation Strategies & Trade-offs. Các key takeaways:
