...
| Knowledge Bases | RAG với S3 data sources |
| Guardrails | Content filtering, PII detection |
| Fine-tuning | Custom fine-tune trên managed infra |
| Agents | Build AI agents với tool use |
...
| Item | Mô tả | Priority |
|---|---|---|
| VPC Private Subnets | AI services không expose ra internet | Critical |
| VPC Endpoints | Traffic không đi qua public internet | High |
| Secrets Manager | Không hardcode API keys | Critical |
| IAM Least Privilege | Chỉ grant quyền cần thiết | Critical |
| Encryption at rest | S3, EBS, RDS encrypted | High |
| Encryption in transit | TLS 1.2+ mọi connections | High |
| WAF | Protect API từ attacks | Medium |
...
Document toàn bộ steps, screenshots, và cost breakdown.
