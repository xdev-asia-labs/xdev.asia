...
description: >-
  Capstone project: xây dựng AI Agent system hoàn chỉnh. Requirements analysis, architecture design. RAG + Tool Calling + Multi-Agent. Conversation management, streaming responses. Error handling, fallback strategies. Testing AI Agents.
...
- `GET /api/v1/documents` — list tất cả documents đã upload (trích từ ChromaDB metadata)
- `DELETE /api/v1/documents/{name}` — xóa document khỏi knowledge base
- Filter cho RAG search: chỉ search trong specific documents
