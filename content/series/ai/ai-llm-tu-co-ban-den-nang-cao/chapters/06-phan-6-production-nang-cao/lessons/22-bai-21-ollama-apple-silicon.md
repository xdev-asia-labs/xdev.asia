...
description: >-
  Deep dive chạy LLM local trên Apple Silicon: hiểu kiến trúc Unified Memory & Metal GPU,
  cài đặt và cấu hình Ollama chuyên sâu, Modelfile tuning, quantization GGUF,
  benchmark tokens/s thực tế trên M1/M2/M3/M4, so sánh với MLX framework,
  multi-model serving, xây dựng local AI development stack hoàn chỉnh.
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="92" y="69" font-family="system-ui,-apple-system,sans-serif" font-size="13" font-weight="600" fill="#2dd4bf">🧠 AI &amp; ML — Bài 20</text>

  <!-- Title -->
  <text x="60" y="140" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">Bài 21: Chạy AI Local với Ollama trên</tspan>
...
Bài 20 đã giới thiệu cách deploy LLM với Ollama, vLLM, TGI ở mức tổng quan. Bài này **đi sâu hoàn toàn** vào Apple Silicon — hiểu rõ **tại sao** nó chạy LLM tốt hơn hầu hết laptop thông thường, cách khai thác tối đa phần cứng, và xây dựng một local AI stack production-grade trên Mac.

**Bạn sẽ học:**

1. Kiến trúc Apple Silicon — Unified Memory, Metal GPU, Neural Engine
2. Cách Ollama tận dụng Metal Performance Shaders cho inference
3. Quantization GGUF — chọn đúng format cho từng chip
4. Modelfile — tuning parameters, system prompt, template
5. Benchmark chi tiết tokens/s trên M1 → M4
6. Apple MLX framework — so sánh với Ollama
7. Multi-model serving và concurrent inference
8. Xây dựng local AI dev stack hoàn chỉnh (RAG + Code Assistant + Embedding)

---

## 1. Tại sao Apple Silicon chạy LLM tốt?

### 1.1 Unified Memory Architecture (UMA)

Trên PC/laptop thông thường, CPU và GPU có RAM riêng biệt. Khi chạy LLM trên GPU, model weights phải được **copy từ System RAM sang VRAM** — quá trình này chậm và bị giới hạn bởi PCIe bandwidth (~32 GB/s PCIe 4.0 x16).

Apple Silicon thay đổi hoàn toàn bằng **Unified Memory**:
...
- Mac 16GB RAM = 16GB cho model (không bị chia CPU/GPU riêng)
- Không có overhead copy data qua PCIe
- Memory bandwidth rất cao — yếu tố quyết định tốc độ inference

### 1.2 Memory Bandwidth — Yếu tố quyết định tốc độ

LLM inference (đặc biệt autoregressive decoding) bị giới hạn bởi **memory bandwidth**, không phải compute. Lý do: mỗi token sinh ra cần đọc toàn bộ model weights một lần.
...
**Bảng Memory Bandwidth Apple Silicon:**

| Chip | Memory BW | So với RTX 4090 (1008 GB/s) |
|---|---|---|
| **M1** | 68.25 GB/s | 6.8% |
...
> RTX 4090 có bandwidth cao hơn nhiều, nhưng chỉ có 24GB VRAM. Mac M4 Max có thể có đến 128GB unified memory — đủ chạy model 70B mà GPU desktop không thể.

### 1.3 Metal GPU & Compute Pipeline

Ollama dùng **llama.cpp** bên dưới, và llama.cpp hỗ trợ Apple **Metal** — framework GPU compute của Apple. Khi chạy trên Apple Silicon:
...
Tất cả tensor operations (matrix multiplication, attention, normalization) chạy trên **GPU cores qua Metal** — không phải CPU. Đây là lý do Mac chạy LLM nhanh hơn nhiều so với laptop x86 thuần CPU.

Kiểm tra Ollama có đang dùng Metal:
...
### 1.4 Neural Engine (ANE) — Chưa được khai thác

Apple Silicon có **Neural Engine** (ANE) — 16-38 TOPS, thiết kế cho ML inference. Tuy nhiên, hiện tại hầu hết LLM frameworks (Ollama, llama.cpp, MLX) **chưa sử dụng ANE** cho text generation vì:

- ANE tối ưu cho batch processing (image classification, vision)
- LLM autoregressive decoding sinh 1 token/step — không phù hợp ANE
- Thiếu operator support cho các kiến trúc Transformer mới

> Đây là lý do bạn thấy GPU usage cao khi chạy Ollama, nhưng ANE gần như idle.

---

## 2. Cài đặt Ollama trên macOS — Cấu hình chuyên sâu
...
### 2.2 Cấu hình qua Environment Variables

Ollama có nhiều biến môi trường để tuning — hầu hết người dùng bỏ qua:
...
Hoặc đơn giản hơn — set trong shell:
...
|---|---|---|
| `OLLAMA_NUM_PARALLEL` | 1 | Số request xử lý song song trên 1 model. Tăng = dùng nhiều RAM hơn cho KV cache |
| `OLLAMA_MAX_LOADED_MODELS` | 1 | Số model load đồng thời. M1 8GB nên để 1, M4 Max 64GB có thể để 3-4 |
| `OLLAMA_KEEP_ALIVE` | 5m | Sau bao lâu không dùng thì unload model. Set "0" = unload ngay, "-1" = giữ mãi |
| `OLLAMA_FLASH_ATTENTION` | 0 | Flash Attention giảm memory usage cho KV cache ~50%. **Nên bật trên Apple Silicon** |

### 2.3 Kiểm tra Metal GPU đang hoạt động
...
Nếu bạn thấy `offloaded X/Y layers to GPU` với X = Y hoặc gần bằng Y → model đang chạy hoàn toàn trên Metal GPU.

---

## 3. Quantization GGUF — Chọn format đúng cho Apple Silicon

### 3.1 GGUF là gì?

GGUF (GPT-Generated Unified Format) là định dạng model của llama.cpp — runtime mà Ollama dùng bên dưới. Khi bạn `ollama pull llama3.2:3b`, Ollama tải file GGUF đã được quantize sẵn.

### 3.2 Các mức quantization
...
### 3.3 K-quants vs I-quants trên Apple Silicon

**K-quants** (Q4_K_M, Q5_K_M, ...): dùng phương pháp k-means clustering. Nhanh trên Metal GPU.

**I-quants** (IQ4_XS, IQ3_XXS, ...): importance-weighted quantization. Chất lượng tốt hơn ở cùng kích thước nhưng **chậm hơn trên Metal** vì cần lookup tables phức tạp.
...
> **Kết luận:** Trên Apple Silicon, **Q4_K_M** là lựa chọn tối ưu nhất. Nếu có dư RAM, nâng lên Q5_K_M hoặc Q6_K.

### 3.4 Tải model với quantization cụ thể
...
## 4. Modelfile — Tuning và Customization

Modelfile là cách Ollama cho phép bạn tùy chỉnh model — tương tự `Dockerfile` cho container.

### 4.1 Cấu trúc Modelfile
...
### 4.2 Giải thích Parameters cho Apple Silicon
...
### 4.4 Import GGUF từ HuggingFace

Khi muốn dùng model/quantization không có sẵn trên Ollama registry:
...
## 5. Benchmark chi tiết trên Apple Silicon

### 5.1 Script benchmark tokens/s

Ollama trả về metrics trong response khi `stream: false`:
...
### 5.2 Kết quả benchmark tham khảo

Dưới đây là kết quả thực tế trên các chip Apple Silicon (Q4_K_M, num_ctx=4096):
...
### 5.3 Ma trận chọn model theo máy
...
## 6. Apple MLX vs Ollama — Khi nào dùng gì?

### 6.1 MLX là gì?

**MLX** là ML framework do Apple phát triển, tối ưu riêng cho Apple Silicon. Khác với Ollama/llama.cpp dùng Metal qua compute shaders, MLX được **thiết kế từ đầu** cho UMA.
...
- **Ollama**: Dùng cho hầu hết trường hợp. Dễ setup, có sẵn model library, API server production-ready, multi-model serving.
- **MLX**: Khi cần tốc độ tối đa trên Mac, khi muốn fine-tune LoRA local, hoặc cần tích hợp sâu vào Python pipeline.

### 6.4 Fine-tune LoRA với MLX (chỉ trên Mac)
...
### 7.2 Router Pattern — Chọn model theo task
...
## 8. Xây dựng Local AI Dev Stack hoàn chỉnh
...
### 8.2 Bước 1: Pull các model cần thiết
...
### 8.3 Bước 2: Cài Continue.dev cho VS Code

**Continue** là extension cho VS Code/JetBrains, kết nối trực tiếp với Ollama để có AI coding assistant **hoàn toàn offline**.
...
Sau khi cài: mở VS Code → `Cmd+L` để chat, `Tab` cho autocomplete — tất cả chạy local.

### 8.4 Bước 3: Open WebUI (giao diện ChatGPT-like local)
...
### 9.2 Kiểm tra model có fit trong RAM
...
## 10. Troubleshooting nâng cao

### 10.1 Model load nhưng chạy cực chậm

**Triệu chứng:** `ollama ps` hiển thị `PROCESSOR: 50% GPU / 50% CPU` hoặc `100% CPU`.

**Nguyên nhân:** Model không fit hoàn toàn trong GPU memory → một phần layers chạy trên CPU.
...
### 10.2 Lỗi `signal: killed` hoặc app bị crash

macOS sẽ kill process khi hết memory. Kiểm tra Console.app → tìm `jetsam` events.
...
### 10.3 Ollama không nhận model mới sau update
...
### 10.4 Performance drop khi dùng external monitor

Một số Mac (đặc biệt M1/M2 base) bị throttle GPU khi kết nối external display vì GPU phải render cả display.

**Fix:** Đóng apps nặng GPU (games, video editing) khi chạy LLM intensive tasks.

---

## 11. Bảo mật khi chạy Local AI
...
Nếu cần remote access (ví dụ từ máy khác trong mạng nội bộ):
...
- Mọi prompt và response **ở trên máy bạn**, không gửi đi đâu
- Ollama **không collect telemetry** (open-source, kiểm tra được)
- Log files ở `~/.ollama/logs/` — cần ẩn nếu chứa data nhạy cảm

---

## Tóm tắt

| Khía cạnh | Key takeaway |
|---|---|
| **Tại sao Apple Silicon tốt** | Unified Memory = toàn bộ RAM dùng cho model, Memory Bandwidth cao, Metal GPU acceleration |
| **Quantization** | Q4_K_M là sweet spot. Tăng RAM → tăng lên Q5_K_M/Q6_K |
| **Modelfile** | Tuning num_ctx, temperature, system prompt cho từng use case |
| **Benchmark** | M1: 35-95 t/s (3B), M4 Max: 110 t/s (3B), 70 t/s (7B) |
| **MLX vs Ollama** | Ollama cho daily use, MLX cho max speed + fine-tuning |
| **Multi-model** | Router pattern route task → đúng model. OLLAMA_MAX_LOADED_MODELS > 1 |
| **Dev Stack** | Ollama + Continue.dev + Open WebUI = offline AI IDE hoàn chỉnh |
| **Flash Attention** | Bật OLLAMA_FLASH_ATTENTION=1 → giảm 50% KV cache memory |
...
1. Cài Ollama, pull `llama3.2:3b` và `qwen2.5-coder:7b`. Chạy `ollama ps` để xác nhận model load 100% GPU.
2. Chạy script `benchmark_ollama.py` ở trên — ghi lại tokens/s trên máy của bạn.
3. Tạo Modelfile cho coding assistant tiếng Việt, build và test.

### Trung bình

4. Setup Continue.dev extension trong VS Code kết nối Ollama. Thử autocomplete và chat.
5. Chạy Open WebUI bằng Docker, kết nối Ollama. So sánh trải nghiệm với ChatGPT.
6. Viết model router (như phần 7.2) phân loại ít nhất 4 task types.

### Nâng cao

7. Xây dựng local RAG pipeline (phần 7.3) index 20+ documents, đo accuracy.
8. So sánh tốc độ Ollama vs MLX trên cùng 1 model (Qwen2.5 7B Q4). Báo cáo prompt processing và generation tokens/s.
9. Fine-tune LoRA với MLX trên dataset tự tạo (50+ samples). So sánh base model vs fine-tuned model.
10. Đo RAM usage thực tế khi chạy 2-3 models đồng thời. Tìm cấu hình OLLAMA_MAX_LOADED_MODELS tối ưu cho máy bạn.
