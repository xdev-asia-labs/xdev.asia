...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="92" y="69" font-family="system-ui,-apple-system,sans-serif" font-size="13" font-weight="600" fill="#818cf8">🔒 DevSecOps — Lesson 2</text>

  <!-- Title -->
  <text x="60" y="140" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">LESSON 2: PLANNING HARDWARE AND NETWORK</tspan>
...
<ul>
<li>✅ Calculate accurate sizing for each type of node (Control Plane, Worker, Storage)</li>
<li>✅ Design a production-grade network topology with VLAN separation__HTMLTAG_75___
<li>✅ Configure NIC bonding for HA networking</li>
<li>✅ Understand and choose the appropriate MTU for each network segment</li>
<li>✅ Create a complete hardware plan for a real project</li>
</ul>

<hr>

<h2 id="phan-1-sizing-control-plane-nodes">PART 1: SIZING CONTROL PLANE NODES</h2>

<h3 id="11-thanh-phan-chay-tren-control-plane">1.1. Components running on Control Plane</h3>
...
<h3 id="12-tinh-toan-resources-cho-control-plane">1.2. Calculate Resources for Control Plane</h3>

<h4 id="etcd-la-thanh-phan-critical-nhat">etcd is the most critical element__HTMLTAG_91___
<p>etcd performance depends mainly on disk I/O. Here are the sizing guidelines from etcd documentation:</p><!--kg-card-begin: html-->
<table>
<thead>
<tr>
<th>Cluster Size</th>
...
<p>⚠️ <strong>Critical:</strong> etcd requires disk latency p99 < 10ms. Dùng NVMe SSD chuyên dụng cho etcd.</p>
...
<h4 id="tong-hop-control-plane-node">Synthesis of Control Plane Node Sizing</h4><!--kg-card-begin: html-->
<table>
<thead>
<tr>
<th>Component</th>
...
<h2 id="phan-2-sizing-worker-nodes">PART 2: SIZING WORKER NODES</h2>

<h3 id="21-tinh-toan-tu-workload">2.1. Calculated from Workload Requirements</h3>

<p>Formula to calculate Worker nodes:</p>
<pre><code class="language-text">
...
<h3 id="22-sizing-guidelines-theo-quy-mo">2.2. Sizing Guidelines by Size</h3><!--kg-card-begin: html-->
<table>
<thead>
<tr>
<th>Scale</th>
...
<p>Kubernetes needs to reserve resources for system components on each node:</p>
...
<h2 id="phan-3-sizing-storage-nodes">PART 3: SIZING STORAGE NODES (CEPH)</h2>

<h3 id="31-ceph-components-tren-storage-nodes">3.1. Ceph Components on Storage Nodes</h3>
<pre><code>
Storage Node
...
<h3 id="32-tinh-toan-storage-capacity">3.2. Calculate Storage Capacity</h3>
<pre><code class="language-text">
...
</code></pre><p>⚠️ <strong>Important decision:</strong> Dedicated storage nodes vs Converged (worker + storage)?</p>
<pre><code class="language-text">
...
<h2 id="phan-4-network-topology-design">PART 4: NETWORK TOPOLOGY DESIGN</h2>

<h3 id="41-4-networks-cho-production">4.1. 4 Networks for Production</h3>
...
<h3 id="42-ip-planning">4.2. IP Planning details</h3><!--kg-card-begin: html-->
<table>
<thead>
<tr>
<th>Node</th>
...
<!--kg-card-end: html--><h3 id="43-nic-bonding-configuration">4.3. NIC Bonding Configuration</h3>
<p>NIC bonding (LACP) provides link redundancy and bandwidth aggregation:</p>
...
<td>Management</td>
<td>1500</td>
<td>Standard, compatible with all devices</td>
</tr>
<tr>
<td>Cluster (K8s)</td>
<td>9000</td>
<td>Jumbo frames reduce CPU overhead, increase throughput__HTMLTAG_688___
</tr>
<tr>
<td>Storage (Ceph)</td>
<td>9000</td>
<td>Critical for Ceph OSD replication performance__HTMLTAG_696___
</tr>
<tr>
<td>External</td>
<td>1500</td>
<td>Standard for internet-facing traffic__HTMLTAG_704___
</tr>
<tr>
...
<p>⚠️ <strong>Jumbo Frames requirement:</strong> All switch ports on the path must support and enable MTU 9000. Check with switch admin before deploying.</p>

<hr>

<h2 id="phan-5-switch-va-firewall">PART 5: SWITCH AND FIREWALL REQUIREMENTS</h2>
...
<h3 id="52-firewall-rules">5.2. Firewall Rules between Networks</h3><!--kg-card-begin: html-->
<table>
<thead>
<tr>
<th>Source</th>
...
<td>6443</td>
<td>TCP</td>
<td>K8s API (if needed external)</td>
</tr>
<tr>
<td>Cluster</td>
<td>Cluster</td>
...
<h2 id="phan-6-disk-layout">PART 6: DISK LAYOUT AND PARTITIONING__HTMLTAG_918___
...
<h2 id="phan-7-bill-of-materials">PART 7: BILL OF MATERIALS (BOM)</h2>

<h3 id="71-bom-cho-production-medium">7.1. BOM for Production (Medium - 20 microservices)</h3>
...
<p><strong>Total:</strong> 13 servers + 2 switches + power infrastructure</p>

<hr><h2 id="key-takeaways">💡 KEY TAKEAWAYS</h2>
<ol>
<li><strong>Control Plane</strong> needs NVMe SSD for etcd, 8+ cores, 16-32GB RAM per node</li>
<li><strong>Worker sizing</strong> calculated from total pod requests + 30% buffer + system reserved</li>
<li><strong>Ceph storage</strong>needs 5GB RAM per OSD, raw disks not formatted</li>
<li><strong>4 separate networks</strong> by VLAN: Management, Cluster, Storage, External</li>
<li><strong>NIC bonding</strong> (LACP) for link redundancy, Jumbo Frames 9000 for cluster/storage</li>
<li><strong>Disk layout:</strong> etcd needs separate partition, Ceph needs raw block devices</li>
</ol>

<hr>

<h2 id="bai-tap">🎯 EXERCISES__HTMLTAG_1069___

<h3 id="bt1">Exercise 1: Calculating Sizing</h3>
<p>For systems including:</p>
<ul>
<li>30 microservices, each service 2 replicas, avg 1 CPU/2GB RAM</li>
<li>PostgreSQL 3 nodes (4 CPU/8GB RAM each)</li>
<li>Kafka 3 brokers (4 CPU/8GB RAM each)</li>
<li>Full observability stack</li>
<li>Data retention: 90 days logs, 1 year metrics</li>
</ul>
<p>Calculate: Number of worker nodes, storage nodes, total disk capacity needed.</p>

<h3 id="bt2">Exercise 2: Network Design__HTMLTAG_1089___
<p>Draw a detailed network topology diagram for the above system, including:</p>
<ul>
<li>VLAN assignment for each network segment__HTMLTAG_1094___
<li>IP planning table for all nodes__HTMLTAG_1096___
<li>NIC bonding topology</li>
<li>Firewall rules matrix</li>
</ul>

<h3 id="bt3">Exercise 3: Lab Setup__HTMLTAG_1103___
<ul>
<li>Using Vagrantfile from Lesson 1, add NIC for storage network</li>
<li>Configure VLANs on VMs (if hypervisor supports)</li>
<li>Test connectivity: ping, iperf3 between all networks</li>
<li>Verify MTU 9000 works on cluster network</li>
</ul>

<hr>

<h2 id="bai-tiep-theo">📚 NEXT POST</h2>
<p>In <strong>Lesson 3: Preparing Linux OS and System Tuning</strong>, we will configure kernel parameters, turn off swap, setup NTP, SSH hardening and prepare all nodes before installing Kubernetes.</p>