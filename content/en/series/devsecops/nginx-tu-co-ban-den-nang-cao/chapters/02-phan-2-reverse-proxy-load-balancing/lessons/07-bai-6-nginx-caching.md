...
title: 'Lesson 6: Nginx Caching'
slug: bai-6-nginx-caching
description: >-
  A lesson on Caching in Nginx — browser caching with expires and Cache-Control
  headers, proxy caching, FastCGI caching for PHP, cache keys and zones. Guide
  to cache purging, bypass strategies, and optimization techniques. Includes
  real-world examples to improve performance and reduce load.
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="60" y="160" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">Lesson 6: Nginx Caching</tspan>
  </text>
...
<h2 id="tags"><strong>TAGS</strong></h2><p><code>#Nginx</code> <code>#Caching</code> <code>#ProxyCache</code> <code>#FastCGI</code> <code>#BrowserCache</code> <code>#CacheControl</code> <code>#Expires</code> <code>#Performance</code> <code>#Optimization</code> <code>#CDN</code> <code>#CachePurging</code> <code>#CacheBypass</code> <code>#WebPerformance</code> <code>#StaticContent</code> <code>#DynamicContent</code> <code>#Tutorial</code> <code>#Production</code> <code>#BestPractices</code> <code>#DevOps</code> <code>#Infrastructure</code></p><hr><h2 id="1-browser-caching-with-expires-and-cache-control-headers"><strong>1. Browser Caching with Expires and Cache-Control Headers</strong></h2><p>Browser caching stores files at the client browser, reducing the number of requests to the server and speeding up load times.</p><h3 id="11-expires-header"><strong>1.1. Expires Header</strong></h3><p>The Expires header specifies a specific point in time when the cache expires.</p><p><strong>Basic syntax:</strong></p><pre><code class="language-nginx">location ~* \.(jpg|jpeg|png|gif|ico)$ {
    expires 30d;  # Cache for 30 days
}
</code></pre><p><strong>Expires values:</strong></p><pre><code class="language-nginx">server {
    listen 80;
...
expires off;   # Do not set Expires header
    ...
    location ~* \.(jpg|jpeg|png|gif|ico|css|js|woff|woff2|ttf)$ {
        # Only apply to files with version in name
        # e.g., style.v1.2.3.css, app.20240101.js
        if ($uri ~ "\.v[0-9]+\.|\.v?\d{8,}\.") {
            expires max;
            add_header Cache-Control "public, immutable";
        }
    ...
    # Videos - long cache but can revalidate
...
</code></pre><h3 id="12-cache-control-header"><strong>1.2. Cache-Control Header</strong></h3><p>Cache-Control is the modern alternative to Expires and is more flexible.</p><p><strong>Cache-Control directives:</strong></p><pre><code class="language-nginx"># public: Can be cached by browser and intermediate caches
add_header Cache-Control "public";

# private: Browser-only cache, not cached by proxies
...
# s-maxage: Max-age for shared caches (CDN, proxies)
...
server {
...
</code></pre><p><strong>Cache based on file extension:</strong></p><pre><code class="language-nginx">server {
    listen 80;
    root /var/www/html;
...
</code></pre><h3 id="14-etag-and-last-modified"><strong>1.4. ETag and Last-Modified</strong></h3><pre><code class="language-nginx">server {
    listen 80;
    server_name example.com;
    ...
    location / {
        ...
        # Browser will send If-None-Match (ETag) or If-Modified-Since
        # Nginx returns 304 Not Modified if file hasn't changed
    }
}
</code></pre><p><strong>Example with conditional requests:</strong></p><pre><code class="language-nginx">server {
    listen 80;
...
</code></pre><hr><h2 id="2-proxy-caching-basics"><strong>2. Proxy Caching Basics</strong></h2><p>Proxy caching stores responses from backend servers, reducing load and response time.</p><h3 id="21-proxy-cache-zone-configuration"><strong>2.1. Proxy Cache Zone Configuration</strong></h3><pre><code class="language-nginx"># Define cache path and settings (in http context)
http {
...
</code></pre><p><strong>Parameter explanation:</strong></p><pre><code class="language-nginx">proxy_cache_path /var/cache/nginx/proxy    # Path to store cache
    levels=1:2                              # Directory structure (optimizes I/O)
    keys_zone=my_cache:10m                  # Zone name and shared memory size
    max_size=1g                             # Max cache size on disk
    inactive=60m                            # Remove unused cache after 60 minutes
    use_temp_path=off;                      # Write directly to cache path
</code></pre><p><strong>Levels explained:</strong></p><pre><code>levels=1:2 creates structure:
...
</code></pre><h3 id="22-cache-status-values"><strong>2.2. Cache Status Values</strong></h3><pre><code class="language-nginx">$upstream_cache_status can be:
...
- STALE      : Serving stale content (if configured)
- UPDATING   : Cache is updating from backend
- REVALIDATED: Cache revalidated with backend (304)
- BYPASS     : Cache bypassed (per config)
...
</code></pre><h3 id="23-cache-key"><strong>2.3. Cache Key</strong></h3><p>The cache key determines the uniqueness of cached items.</p><p><strong>Default cache key:</strong></p><pre><code class="language-nginx">proxy_cache_key $scheme$proxy_host$request_uri;
...
</code></pre><p><strong>Example with user-specific caching:</strong></p><pre><code class="language-nginx">map $cookie_user_id $cache_user {
...
server {
...
</code></pre><h3 id="24-cache-methods-and-conditions"><strong>2.4. Cache Methods and Conditions</strong></h3><pre><code class="language-nginx">server {
    listen 80;
    ...
    location / {
        ...
        # Cache durations based on status code
        ...
        # Cache even when backend errors
...
# - updating: Cache is being updated (prevents stampede)
# - http_500/502/503/504: Backend returns error codes
...
</code></pre><hr><h2 id="3-fastcgi-caching"><strong>3. FastCGI Caching</strong></h2><p>FastCGI caching is used for dynamic content such as PHP applications.</p><h3 id="31-fastcgi-cache-configuration"><strong>3.1. FastCGI Cache Configuration</strong></h3><pre><code class="language-nginx">http {
...
</code></pre><h3 id="32-wordpress-with-fastcgi-cache"><strong>3.2. WordPress with FastCGI Cache</strong></h3><pre><code class="language-nginx">http {
...
</code></pre><h3 id="33-drupal-with-fastcgi-cache"><strong>3.3. Drupal with FastCGI Cache</strong></h3><pre><code class="language-nginx">http {
...
</code></pre><h3 id="34-custom-php-application-with-cache"><strong>3.4. Custom PHP Application with Cache</strong></h3><pre><code class="language-nginx">http {
...
</code></pre><hr><h2 id="4-cache-keys-and-cache-zones"><strong>4. Cache Keys and Cache Zones</strong></h2><h3 id="41-cache-zones-configuration"><strong>4.1. Cache Zones Configuration</strong></h3><pre><code class="language-nginx">http {
...
</code></pre><hr><h2 id="5-cache-purging-and-bypass"><strong>5. Cache Purging and Bypass</strong></h2><h3 id="51-cache-bypass"><strong>5.1. Cache Bypass</strong></h3><pre><code class="language-nginx">server {
    listen 80;
    
    # Bypass cache with special header
    location / {
        proxy_pass http://backend;
        proxy_cache my_cache;
...
</code></pre><p><strong>Bypass with query parameter:</strong></p><pre><code class="language-nginx">server {
    location / {
        proxy_pass http://backend;
        proxy_cache my_cache;
...
</code></pre><p><strong>Cache purge with open source module:</strong></p><pre><code class="language-bash"># Install ngx_cache_purge module
# Ubuntu/Debian
...
</code></pre><h3 id="53-selective-cache-invalidation"><strong>5.3. Selective Cache Invalidation</strong></h3><pre><code class="language-nginx"># API to invalidate cache
server {
    listen 80;
    server_name example.com;
...
</code></pre><hr><h2 id="6-practice-exercises"><strong>6. Practice Exercises</strong></h2><h3 id="exercise-1-browser-caching"><strong>Exercise 1: Browser Caching</strong></h3><ol><li>Set up a static file server</li><li>Configure expires headers:<ul><li>Images: 1 year</li><li>CSS/JS: 1 month</li><li>HTML: 1 hour</li></ul></li><li>Test with browser DevTools (Network tab)</li><li>Verify cache headers</li></ol><h3 id="exercise-2-proxy-cache"><strong>Exercise 2: Proxy Cache</strong></h3><ol><li>Set up backend server (Node.js/Python)</li><li>Configure Nginx proxy cache</li><li>Generate traffic and monitor cache hits/misses</li><li>Check cache files in <code>/var/cache/nginx</code></li></ol><h3 id="exercise-3-fastcgi-cache-for-wordpress"><strong>Exercise 3: FastCGI Cache for WordPress</strong></h3><ol><li>Install WordPress</li><li>Configure FastCGI cache</li><li>Test cache bypass for:<ul><li>Logged-in users</li><li>Admin pages</li><li>POST requests</li></ul></li><li>Measure performance improvement</li></ol><h3 id="exercise-4-custom-cache-keys"><strong>Exercise 4: Custom Cache Keys</strong></h3><ol><li>Set up cache with custom key (include device type)</li><li>Test from mobile and desktop</li><li>Verify different cached versions</li></ol><h3 id="exercise-5-cache-purging"><strong>Exercise 5: Cache Purging</strong></h3><ol><li>Set up proxy cache</li><li>Create purge endpoint</li><li>Test cache purge:<ul><li>Manual purge script</li><li>API endpoint</li></ul></li><li>Verify cache cleared</li></ol><h3 id="exercise-6-cache-performance-testing"><strong>Exercise 6: Cache Performance Testing</strong></h3><ol><li>Set up with and without cache</li><li>Use Apache Bench to benchmark:</li></ol><pre><code class="language-bash"># Without cache
...
</code></pre><h3 id="73-stale-content-being-served"><strong>7.3. Stale Content Being Served</strong></h3><p><strong>Problem:</strong> Old content is served despite updates</p><p><strong>Solutions:</strong></p><pre><code class="language-nginx"># Clear cache manually
