...
description: 'Keycloak Operator (CRDs: Keycloak, KeycloakRealmImport), install Operator via OLM/kubectl, custom resource configuration, realm import automation, Operator upgrade strategies. Monitoring with Prometheus + Grafana (Keycloak metrics, JVM metrics, Infinispan metrics, custom dashboards). Admin CLI (kcadm.sh) mastery - all operations (realms, users, clients, roles, groups, identity providers, flows, components). Backup & restore strategies for Kubernetes.'
...
<tspan x="60" dy="0">Lesson 25: Kubernetes Operator, Monitoring and</tspan>
...
<p>Keycloak Operator is the official Kubernetes Operator, managing Keycloak deployment according to <strong>declarative approach</strong>. Operators use Custom Resource Definitions (CRDs) to define the desired state, then automatically reconcile to ensure the actual state matches.</p>
...
<table>
<thead>
<tr><th>CRD</th><th>API Group</th><th>Purpose</th></tr>
</thead>
<tbody>
<tr><td><code>Keycloak</code></td><td><code>k8s.keycloak.org/v2alpha1</code></td><td>Define Keycloak deployment (instances, DB, hostname, TLS, features)</td></tr>
<tr><td><code>KeycloakRealmImport</code></td><td><code>k8s.keycloak.org/v2alpha1</code></td><td>Import realm configuration from JSON</td></tr>
</tbody>
</table>
...
<h3 id="42-realm-import-tu-file-json"><strong>4.2 Realm Import from JSON</strong></h3> file
...
<h2 id="6-monitoring-voi-prometheus-grafana"><strong>6. Monitoring with Prometheus + Grafana</strong></h2>
...
<h3 id="710-export-import-va-scripting"><strong>7.10 Export/Import and Scripting</strong></h3>
...
<p>Below are all the necessary manifests to fully deploy Keycloak on Kubernetes:</p>
