...
title: 'Lesson 20: Integrating Spring Boot and Quarkus'
slug: bai-20-tich-hop-spring-boot-va-quarkus
description: Integrate Keycloak with Spring Boot 3 using Spring Security OAuth2 Resource Server (spring-boot-starter-oauth2-resource-server), configure JWT validation, custom JwtAuthenticationConverter, role mapping from realm_access/resource_access claims, Spring Security method-level authorization (@PreAuthorize), integrate Quarkus with quarkus-oidc extension, multi-tenant configuration and testing with Testcontainers.
...
<text x="92" y="69" font-family="system-ui,-apple-system,sans-serif" font-size="13" font-weight="600" fill="#f472b6">🔒 DevSecOps — Lesson 20</text>

  <!-- Title -->
  <text x="60" y="160" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
<tspan x="60" dy="0">Lesson 20: Integrating Spring Boot and Quarkus</tspan>
  </text>
...
<h2 id="1-tong-quan-tich-hop-backend"><strong>1. Backend integration overview</strong></h2>

<p>Keycloak provides a standard OAuth2/OIDC mechanism, allowing integration with any backend framework that supports JWT verification. In this article, we will integrate with the two most popular frameworks in the Java ecosystem: <strong>Spring Boot 3</strong> and <strong>Quarkus</strong>.</p>
...
<p><strong>Important note:</strong> From Keycloak 20+, the dedicated adapter for Spring Boot (<code>keycloak-spring-boot-starter</code>) has been <strong>deprecated</strong>. The current standard method is to use Spring Security's <code>spring-boot-starter-oauth2-resource-server</code>.</p>
...
<tbody>
<tr><td><code>issuer-uri</code></td><td>URI of Keycloak realm, used to validate <code>iss</code> claim in JWT</td></tr>
<tr><td><code>jwk-set-uri</code></td><td>Endpoint contains public keys (JWKS) to verify JWT signature</td></tr>
</tbody>
</table>
...
<p>Keycloak stores roles in JWT claims in a special structure. Need custom converter to extract correct roles:</p>
...
<h3 id="25-rest-controller-voi-rbac"><strong>2.5 REST Controller with RBAC</strong></h3>
...
<p>When the frontend (React/Angular) calls the backend API, CORS configuration is required:</p>
...
<p>Add CORS to <code>SecurityFilterChain</code>:</p>
...
<h3 id="27-xu-ly-token-expiration"><strong>2.7 Handling Token Expiration</strong></h3>

<p>Spring Security automatically validates <code>exp</code> claim. When the token expires, the server returns HTTP 401:</p>
...
<p>Register in <code>SecurityFilterChain</code>:</p>
...
<p>Quarkus provides the <code>quarkus-oidc</code> extension that integrates with Keycloak:</p>
...
<h3 id="33-rest-resource-voi-rolesallowed"><strong>3.3 REST Resource with @RolesAllowed</strong></h3>
...
<p>Quarkus supports multi-tenant OIDC for SaaS systems that need to connect multiple Keycloak realms:</p>
...
<p>Use <code>quarkus-keycloak-authorization</code> to enforce Keycloak Authorization Services policies:</p>
...
<h2 id="4-testing-voi-testcontainers"><strong>4. Testing with Testcontainers</strong></h2>

<h3 id="41-spring-boot-testcontainers-keycloak"><strong>4.1 Spring Boot + Testcontainers Keycloak</strong></h3>

<p>Use <code>testcontainers-keycloak</code> to run real Keycloak in integration tests:</p>
...
<p>Create file <code>src/test/resources/test-realm.json</code> to import realm for test:</p>
...
<h3 id="43-unit-test-voi-mock-jwt"><strong>4.3 Unit Test with Mock JWT</strong></h3>

<p>For unit tests without real Keycloak, use <code>@WithMockUser</code> or custom JWT:</p>
...
<h2 id="5-best-practices-va-troubleshooting"><strong>5. Best Practices and Troubleshooting</strong></h2>
...
<tbody>
<tr><td>1</td><td>Use Realm Roles</td><td>Priority <code>realm_access.roles</code> for simple authorization</td></tr>
<tr><td>2</td><td>Stateless sessions</td><td>Always use <code>SessionCreationPolicy.STATELESS</code> for REST API</td></tr>
<tr><td>3</td><td>Disable CSRF for API</td><td>CSRF is not needed when using JWT Bearer token</td></tr>
<tr><td>4</td><td>Token validation caching</td><td>Spring Security automatically caches JWK Set, no need to call Keycloak for each request</td></tr>
<tr><td>5</td><td>Claim-based authorization</td><td>Use <code>@PreAuthorize</code> with SpEL for complex logic</td></tr>
<tr><td>6</td><td>Error handling</td><td>Custom <code>AuthenticationEntryPoint</code> for uniform response format</td></tr>
<tr><td>7</td><td>Test coverage</td><td>Combining unit tests (mock JWT) and integration tests (Testcontainers)</td></tr>
</tbody>
</table>
...
<table>
<thead>
<tr><th>Criteria</th><th>Spring Boot 3</th><th>Quarkus</th></tr>
</thead>
<tbody>
...
<p>In the next article, we will learn how to integrate Keycloak with frontend frameworks (React, Angular) and Node.js backend.</p>
