---
id: cka-d5-l10
title: 'Lesson 10: Troubleshooting Nodes'
...
<blockquote><p><strong>Exam tip:</strong> NotReady debug procedure: <code>kubectl describe node</code> → SSH → <code>systemctl status kubelet</code> → <code>journalctl -u kubelet</code>. Most common issues: kubelet stopped, wrong API server address, or certificate expired.</p></blockquote>
...
<tr><td>MemoryPressure: True</td><td>Low memory</td><td>Evict pods, scale node</td></tr>
<tr><td>DiskPressure: True</td><td>Disk full</td><td>Clean /var/log, /tmp, unused images</td></tr>
...
<p><em>Explanation: Ready: Unknown means the API server hasn't received a heartbeat from the kubelet recently. This typically indicates the node is unreachable (network partition, node powered off). Ready: False means kubelet is reachable but reports a problem.</em></p>
