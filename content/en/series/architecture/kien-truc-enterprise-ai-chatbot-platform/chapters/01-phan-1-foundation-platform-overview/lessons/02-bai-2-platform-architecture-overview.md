...
title: 'Lesson 2: Platform Architecture Overview — Microservices, Event-Driven & DDD'
...
<ul>
<li><strong>Modularity</strong> — Each capability is an independent bounded context that can be replaced/upgraded separately</li>
<li><strong>Event-Driven</strong> — Services communicate via events, reduce coupling, support audit trail</li>
<li><strong>AI-First</strong> — Architecture optimization for AI workloads: streaming, long-running inference, GPU-aware scaling</li>
</ul>

<h2 id="2-bounded-contexts"><strong>2. Bounded Contexts — DDD for AI Chatbots</strong></h2>
...
<ul>
<li><strong>Audit Trail</strong> — All conversation events are logged, crucial for compliance</li>
<li><strong>Async Processing</strong> — Knowledge ingestion, analytics, billing run async</li>
<li><strong>Decoupling</strong> — AI Engine does not need to know about Billing; only emit token usage events</li>
<li><strong>Replay</strong> — Replay events to debug conversation flow, retrain models</li>
</ul>
...
<h2 id="7-request-flow"><strong>7. Request Flow — From User Message to Response</strong></h2>
...
<li>Request flow through <strong>7 stages</strong>: Channel → Input Guard → Context Build → RAG + Memory → Prompt → LLM → Output Guard</li>
...
<p><strong>Next article:</strong> Multi-Model Gateway — how to route requests between OpenAI/Claude/Gemini/self-hosted models, fallback chain, cost optimization, and token budget management.</p>
