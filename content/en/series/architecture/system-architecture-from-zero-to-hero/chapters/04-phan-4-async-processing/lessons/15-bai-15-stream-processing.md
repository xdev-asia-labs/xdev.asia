...
description: >-
  Batch vs Stream Processing. Apache Kafka Streams, basic Apache Flink.
  Windowing strategies. Change Data Capture (CDC). Real-time analytics pipeline.
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="92" y="69" font-family="system-ui,-apple-system,sans-serif" font-size="13" font-weight="600" fill="#2dd4bf">🏗️ Architecture — Lesson 15</text>

  <!-- Title -->
  <text x="60" y="140" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">Lesson 15: Stream Processing & Real-time Data</tspan>
  ...
  <!-- Section -->
  <text x="60" y="268" font-family="system-ui,-apple-system,sans-serif" font-size="13" fill="#64748b" opacity="0.6">Part 4: Asynchronous Processing & Communication</text>
...
Batch processing processes data in batches (hourly, daily). But in the real-time era, users want to see results immediately. **Stream Processing** processes data as it appears.
...
|----------|-------|-------|
| Latency | Minutes → hours | Ms → sec |
| Data | Bounded (with an end point) | Unbounded (ongoing) |
| Processing | Entire dataset | Each event/micro-batch |
...
### 3.2 Example: Real-time Order Stats
...
| Kappa | Stream only, replays | Kafka + Flink |

---

## Exercises

1. **Pipeline Design:** Social media needs real-time trending topics. Design stream processing pipeline: input (posts, likes, shares) → processing → output (trending list updated every minute).

2. **CDC Implementation:** You have PostgreSQL (orders) and Elasticsearch (search). Design CDC pipeline to sync. Case handling: Elasticsearch downed for 2 hours then recovered.

3. **Window Strategy:** Fraud detection: alert if a credit card has > 5 transactions in 10 minutes. What type of window to use? How to handle late events?
