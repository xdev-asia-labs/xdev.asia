...
description: >-
  ...
  monitoring stack. Centralized logging (ELK/EFK). Distributed tracing (Jaeger,
  OpenTelemetry). Alerting strategies. SLI/SLO/SLA revisited. Observability for
  microservices.
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="60" y="140" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">Lesson 23: Observability - Monitoring,</tspan>
...
Monitoring says **WHAT** is going wrong. Observability tells **WHY** it is wrong. In distributed systems, observability is critical — you can't fix what you can't see.
...
1. **Monitoring Setup:** Microservices: API Gateway, User, Order, Payment, Notification. For each service, list the 5 most important metrics to monitor. Use the RED method.

2. **Tracing Analysis:** Trace shows: API Gateway (2s) → Order (1.8s) → DB (50ms) → Payment (1.7s) → External API (1.5s). Identify bottlenecks. Suggest 3 ways to optimize.

3. **Alert Design:** Design alerting system for e-commerce. Definition of 3 P1, 3 P2, 3 P3 alerts. For each alert: condition, threshold, escalation, runbook summary.
