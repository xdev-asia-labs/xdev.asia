...
description: >-
  Fan-out pattern, Producer-Consumer pattern, Priority Queue pattern. Circuit
  Breaker, Bulkhead, Outbox pattern, Saga pattern for notification workflows.
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="60" y="140" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">Lesson 3: Design Patterns for Email System</tspan>
      <tspan x="60" dy="42">large scale</tspan>
  </text>
...
Designing a large-scale email system isn't just about writing code — it's about applying the right **design patterns** to make the system reliable, scalable, and maintainable. This article will cover the 6 most important patterns.
...
A campaign sent to 10 million recipients → needs to **fan-out** into 10 million individual messages.
...
- **Stream, do not load all**: Use cursor/pagination to stream recipients
- **Batch publish**: Send batch goods to the queue, not individual publish
- **Progress tracking**: Update fan-out progress
...
ESP (Email Service Provider) may downgrade or rate limit you → need to temporarily disconnect.
...
Need to ensure: database write + queue publish is **atomic**. If publish is successful but DB crashes → inconsistency.
...
Marketing email flood → affects transactional emails (OTP delay) → users cannot login.
...
| Pattern | Resolve | When to use |
|--------|-----------|--------------|
| Fan-Out | 1 campaign → N messages | Campaign broadcast |
| Producer-Consumer | Decouple sent from process | Anytime |
...
**Next article:** Deep dive into Message Queue — the backbone of the notification system.
