...
description: >-
  Implement Row-Level Security (RLS) in PostgreSQL for medical data: RLS
  policies for patient data isolation, department-based access control,
  doctor-patient relationship policies, column-level encryption for sensitive
  fields (SSN, diagnosis, lab results), dynamic data masking, and integrating
  RLS with Keycloak JWT claims in Quarkus.
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="60" y="140" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">Lesson 11: Row-Level Security & Column</tspan>
      <tspan x="60" dy="42">Encryption for PHI</tspan>
  </text>
...
- **Doctor A** only sees his patient
- **Internal Medicine nurses** only see Internal Medicine patients
- **Hospital Admin** sees all patients of that hospital
...
- Developer forgot WHERE clause → data leak
- SQL injection bypass WHERE clause
- Direct DB access bypass application logic
- Reporting tools bypass application

**Row-Level Security (SAFETY):**

- `SELECT * FROM patients;` — Returns ONLY rows allowed
- Database enforceable, cannot be bypassed
- Transparent for applications
- Works with all tools (psql, reporting, BI)
...
### 2.1. Schema for Healthcare Database
...
## 3. RLS Policies for Patient Data Isolation
...
## 4. Integrate RLS with Keycloak JWT in Quarkus
...
## 5. Column-Level Encryption for PHI Fields
...
### 5.2. INSERT/SELECT with Encryption
...
## 6. Dynamic Data Masking with Views
...
|--------|--------|-----------|
| Forgot `FORCE ROW LEVEL SECURITY` | Table owner bypass RLS | Always use `FORCE` |
| `current_setting()` no `true` param | Error when variable has not been set | Use `current_setting('var', true)` |
| RLS on JOIN tables | Performance issue | Index on foreign keys |
| Encryption key in source code | Key leaks | Use Vault or environment variables |
| Forgot to RESET session variables | Connection pool leak | Reset on connection return |
...
1. **Row-Level Security (RLS)**: Enable, force, and create policies for hospital isolation, doctor-patient relationships, department-based access, and patient portal
2. **Role-Based Policies**: Different for doctor, nurse, admin, lab_tech, patient
3. **Emergency Access**: Break-the-glass mechanism with time-limited access and audit logging
4. **JWT-to-PostgreSQL Integration**: Quarkus interceptor passes JWT claims through `SET LOCAL` session variables
5. **Column Encryption**: pgcrypto helper functions for encrypt/decrypt PHI fields
6. **Dynamic Data Masking**: Views display masked/full data depending on the role
7. **Testing**: Comprehensive test script verifies all access patterns
8. **Performance**: Indexes support RLS policy evaluation

RLS combined with Column Encryption creates powerful **two layers of protection** for medical data — even if one layer is bypassed, the other layer still protects PHI.

## Exercises

1. **Implement RLS Policies**: Create a database with the above schema, enable RLS, and implement policies for all roles (doctor, nurse, admin, patient). Insert test data and verify access isolation via test script.

2. **JWT Integration**: Write a Quarkus interceptor that passes JWT claims into PostgreSQL session variables. Test with 3 different users (doctor, nurse, patient) - verify each user sees correct data.

3. **Emergency Access**: Implement break-the-glass mechanism: log emergency access, grant temporary 4-hour access, revoke after expiration. Test verify emergency access works properly.

4. **Dynamic Masking**: Create masking views for 3 different roles. Test verify doctor sees full data, nurse sees masked data, reporting sees anonymized data.
...
|:---|---:|
| [Lesson 10: Encrypting At-Rest & In-Transit Data with PostgreSQL](/series/bao-mat-du-lieu-y-te-cho-microservices/bai-10-ma-hoa-du-lieu-at-rest-in-transit-postgresql) | [Lesson 12: Audit Logging & Change Data Capture with pgAudit](/series/bao-mat-du-lieu-y-te-cho-microservices/bai-12-audit-logging-cdc-pgaudit) |
<!-- SERIES-NAV:END -->
