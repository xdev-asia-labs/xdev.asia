...
description: >-
  Build a centralized audit trail system for healthcare: OpenTelemetry
  instrumentation for Quarkus, distributed tracing for patient data flow,
  structured logging with correlation IDs, ELK Stack (Elasticsearch, Logstash,
  Kibana) deployment, audit log dashboards, alert rules for suspicious access
  patterns, and log retention policies according to HIPAA.
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="60" y="140" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">Lesson 18: Centralized Audit Trail —</tspan>
      <tspan x="60" dy="42">OpenTelemetry & ELK Stack</tspan>
  </text>
...
| Request | HIPAA Reference | Implementation |
|----------|----------------|----------------|
| Record all PHI access | §164.312(b) | OpenTelemetry spans + structured logs |
| Unique user identification | §164.312(a)(2)(i) | Correlation IDs in traces |
| Review audit logs periodically | §164.308(a)(1)(ii)(D) | Kibana dashboards + scheduled reports |
| Detect security incidents | §164.308(a)(6)(i) | Elasticsearch alerts + anomaly detection |
| 6 years archived | §164.530(j)(2) | Elasticsearch ILM policies |
| Protect audit logs | §164.312(c)(1) | Immutable indices, RBAC |

## 2. OpenTelemetry Setup for Quarkus
...
### 2.3. Custom Spans for PHI Access Tracking
...
### 2.4. Use PhiAccessTracer in Service
...
- **Client Request** with `X-Request-ID` + `traceparent` headers
...
- **Trace ID** links ALL spans in the flow
...
With the configuration above, each log entry will have the format:
...
### 4.1. Docker Compose for Observability Stack
...
## 5. Logstash Pipeline for Healthcare Audit
...
### 6.1. Index Template for Audit Logs
...
## 7. Kibana Dashboards for Audit Visualization
...
Elasticsearch query for suspicious patterns:
...
| Alert | Triggers | Severity | Action |
|-------|---------|----------|-------|
...
In this lesson, we built a complete **Centralized Audit Trail** for healthcare microservices:

1. **OpenTelemetry Instrumentation**: Custom spans for PHI access (PhiAccessTracer), automatic distributed tracing, OTLP export
2. **Correlation IDs**: MDC integration — trace_id, span_id, request_id, user_id in every log entry
3. **Structured Logging**: JSON format, PHI masking filter, consistent log schema
4. **ELK Stack Deployment**: Docker Compose stack with Elasticsearch, Logstash, Kibana, Jaeger, Prometheus
5. **Logstash Pipeline**: Healthcare-specific parsing, suspicious pattern detection, audit event routing
6. **Index Templates & ILM**: Elasticsearch templates for audit data, 6-year retention policy (Hot → Warm → Cold → Delete)
...
1. **OpenTelemetry Setup**: Add `quarkus-opentelemetry` Go to Quarkus project. Configure OTLP exporter to local Jaeger (`docker run jaegertracing/all-in-one`). Implement PhiAccessTracer and wrap Patient CRUD operations. Verify traces display in Jaeger UI with custom attributes (user_id, patient_id, action).

2. **ELK Stack Deployment**: Run Docker Compose observability stack. Create Elasticsearch index template for `healthcare-audit-*`. Configure ILM policy with 4 phases (hot/warm/cold/delete). Verify ILM policy with `GET _ilm/policy/hipaa-retention-policy`. Send sample audit events via Logstash TCP input.

3. **Kibana Dashboard**: Import audit index pattern into Kibana. Create 3 visualizations: (a) PHI access timeline, (b) Top users by access count, (c) Access by department pie chart. Combine into 1 dashboard. Create saved search for suspicious patterns.

4. **Alert Configuration**: Create Elasticsearch Watcher for bulk PHI access alert (>50 records in 5 min). Create after-hours access alert (22:00-06:00). Test by sending sample events via API. Verify alert fires and notifications are sent (using webhook.site for testing).
...
|:---|---:|
| [Lesson 17: HIPAA Technical Safeguards - Full Implementation Checklist](/series/bao-mat-du-lieu-y-te-cho-microservices/bai-17-hipaa-technical-safeguards-implementation) | [Lesson 19: Data Masking, Anonymization & De-identification for PHI](/series/bao-mat-du-lieu-y-te-cho-microservices/bai-19-data-masking-anonymization-de-identification) |
<!-- SERIES-NAV:END -->
