...
description: >-
  Circuit Breaker states (Closed, Open, Half-Open), Retry with Exponential
  Backoff & Jitter, Timeout pattern, fallback strategies, implementation with
  Resilience4j/Polly, cascade failure prevention.
duration_minutes: 90
is_free: true
video_url: null
sort_order: 18
section_title: 'Part 6: Resiliency Patterns'
course:
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="60" y="160" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">Lesson 18: Circuit Breaker & Retry Patterns</tspan>
  </text>
  ...
  <!-- Section -->
  <text x="60" y="246" font-family="system-ui,-apple-system,sans-serif" font-size="13" fill="#64748b" opacity="0.6">Part 6: Resiliency Patterns</text>

  <!-- xDev watermark -->
  <text x="1140" y="320" font-family="system-ui,-apple-system,sans-serif" font-size="12" fill="#475569" text-anchor="end" opacity="0.4">xdev.asia</text>
</svg>

![Lesson 18: Circuit Breaker & Retry Patterns](/storage/uploads/2026/03/cn-bai-18-diagram.png)

## Introduction

In microservices, **failure is normal** — services will time out, crash, be overloaded. The question is not "will there be failure?" but "how is failure handled?".

Without a protection mechanism, a failed service can bring down the entire system via **cascade failure**. This article presents the essential patterns for building resilient systems.

---

## 1. Cascade Failure — The core problem

### 1.1 Typical cascade failure scenario
...
**Cascade failure occurs because**: Resources (threads, connections, memory) held by calls to the failed downstream service.
...
### 2.1 Timeout on all network calls

**Rule**: Every outbound network call must have a timeout.
...
### 2.3 How much timeout should I set?
...
### 3.4 Deployment with Resilience4j (Java)
...
### 6.1 Unit Testing with Resilience4j
...
Expose circuit breaker state via Prometheus:
...
|--------|-------------------|-------------|
| Timeouts | Thread starvation | All network calls |
| Retry + Backoff + Jitter | Transient failures | Idempotent operations |
| Circuit Breaker | Cascade failure | External service calls |
| Fallback | Degraded experience | When failure cannot be avoided |

**Order of application**: Timeout → Retry → Circuit Breaker → Fallback

**Next article**: Bulkhead, Rate Limiting & Health Check Patterns
