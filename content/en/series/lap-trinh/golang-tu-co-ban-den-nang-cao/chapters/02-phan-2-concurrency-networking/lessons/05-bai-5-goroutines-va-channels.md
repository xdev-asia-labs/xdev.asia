...
description: >-
  Goroutines, WaitGroup, buffered/unbuffered channels. Select statement, channel
  directions, fan-in/fan-out patterns. Concurrency vs parallelism.
duration_minutes: 150
is_free: true
video_url: null
sort_order: 5
section_title: 'Part 2: Concurrency & Networking'
course:
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="60" y="160" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">Lesson 5: Goroutines & Channels</tspan>
  </text>
  ...
  <!-- Section -->
  <text x="60" y="246" font-family="system-ui,-apple-system,sans-serif" font-size="13" fill="#64748b" opacity="0.6">Part 2: Concurrency & Networking</text>
...
<p>Before starting, it is necessary to distinguish two concepts that are often confused:</p>

<ul>
<li><strong>Concurrency</strong>: Handle multiple tasks at the same time by <em>alternately</em> (1 CPU core runs alternately). Like a cook who prepares many dishes.</li>
<li><strong>Parallelism</strong>: Really <em>run simultaneously</em> on multiple CPU cores. Like many people, each person cooks one dish.</li>
</ul>

<p>Go is designed for <strong>concurrency</strong> — helps write programs that handle multiple tasks efficiently. Go runtime automatically schedules goroutines onto OS threads, taking advantage of parallelism when there are multiple CPU cores.</p>
...
<p>Goroutines are lightweight threads managed by the Go runtime. Initialized with only ~2KB stack (compared to OS thread ~1-8MB), can create millions of goroutines.</p>

<h3 id="2-1-basic-goroutine"><strong>2.1. Create Goroutine</strong></h3>
...
<p>Channels are the communication mechanism between goroutines. Go proverb: <em>"Don't communicate by sharing memory, share memory by communicating."</em></p>
...
<p><code>select</code> Allows waiting on multiple channel operations — like a switch but for channels.</p>
...
<h3 id="5-3-worker-pool"><strong>5.3. Workers Pool</strong></h3>
...
<ul>
<li><strong>Goroutines</strong>: Lightweight threads (~2KB), created with <code>go. go</code> keyword. keyword</li>
<li><strong>WaitGroup</strong>: Wait for group goroutines to complete</li>
<li><strong>Channels</strong>: Communication between goroutines (unbuffered = sync, buffered = async)</li>
<li><strong>Select</strong>: Multiplexing on multiple channels</li>
<li><strong>Patterns</strong>: Fan-Out/Fan-In, Pipeline, Worker Pool, Done Channel</li>
<li><strong>Rules</strong>: Sender close channel, use context/done for cancellation, avoid goroutine leaks</li>
</ul>

<p>Next article: <strong>Context, Sync & Concurrency Patterns</strong> — context package, mutex, and advanced patterns.</p>
