...
title: 'Lesson 8: Order Service — Order management'
slug: bai-8-order-service-quan-ly-don-hang
description: >-
  Build Order Service: Order Aggregate, order status (State Machine), basic Saga
  pattern, Product Service integration.
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="92" y="69" font-family="system-ui,-apple-system,sans-serif" font-size="13" font-weight="600" fill="#fb923c">💻 Programming — Lesson 7</text>

  <!-- Title -->
  <text x="60" y="160" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">Lesson 8: Order Service — Order management</tspan>
  </text>
  ...
  <!-- Section -->
  <text x="60" y="246" font-family="system-ui,-apple-system,sans-serif" font-size="13" fill="#64748b" opacity="0.6">Part 2: Designing Microservices Architecture</text>
...
Order Service is the heart of E-Commerce — where Product Service (check stock), Payment Service (payment), and Notification Service (send notifications) are coordinated. This article builds an Order Aggregate with a State Machine to manage the order lifecycle.
...
## REST Client — Call Product Service
...
1. Create Order Service project with Order Aggregate (Order + OrderItem)
2. Implement State Machine for OrderStatus with validation transitions
3. Create a REST Client to call Product Service to check stock
4. Implement endpoint to create order, confirm, cancel
5. Write Flyway migration for order schema
6. Test flow: Create → Confirm → (mock) Pay → Ship → Deliver

## Summary

- **Order Aggregate** manages OrderItems, ensuring total calculation consistency
- **State Machine** (`canTransitionTo()`) control the order lifecycle
- **REST Client** (`@RegisterRestClient`) call Product Service to reserve/release stock
- **Domain Methods** (`confirm()`, `markPaid()`, `cancel()`) contains business logic
- Service layer coordinates domain logic and external service calls

Next article: Payment Service & Notification Service.
