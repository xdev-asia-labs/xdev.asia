...
title: 'Lesson 5: Validation, Error Handling & Config Profiles'
slug: bai-5-validation-error-handling-config-profiles
description: >-
  Bean Validation with Hibernate Validator, Exception Mappers, Problem Details
  (RFC 9457), Config Profiles for dev/staging/prod environments.
duration_minutes: 80
is_free: false
video_url: null
sort_order: 4
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="60" y="140" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">Lesson 5: Validation, Error Handling & Config</tspan>
...
1. Add Bean Validation for `CreateProductRequest` and `UpdateProductRequest`
2. Create custom `@ValidSlug` annotation validator
3. Implement cross-field validation `@ValidDateRange` for promotions
4. Implement `ProblemDetail` response according to RFC 9457
5. Create `GlobalExceptionHandler` Handle all exceptions with pattern matching
6. Add Validation Groups (`OnCreate`, `OnUpdate`) for ProductRequest
7. Create `ValidationMessages.properties` for i18n (Vietnamese & English)
8. Configuration `%dev`, `%test`, `%prod` profiles in `application.properties`
9. Create `ProductConfig` with `@ConfigMapping` to manage typed config
10. Write `@QuarkusTest` test validation errors returns the correct Problem Details format
...
- **Bean Validation** (`@NotBlank`, `@Size`, `@Min`...) automatically validates when used `@Valid`
- **ExceptionMapper** converts exceptions into standard JSON response
- **RFC 9457 Problem Details** — standard error format with `application/problem+json`
- **Config Profiles** (`%dev`, `%test`, `%prod`) — separate configuration for each environment
- **`@ConfigMapping`** — typesafe configuration alternative `@ConfigProperty`
