...
title: 'Lesson 16: Fault Tolerance — Circuit Breaker, Retry, Bulkhead'
slug: bai-16-fault-tolerance-circuit-breaker-retry-bulkhead
description: >-
  SmallRye Fault Tolerance: @Retry, @Timeout, @CircuitBreaker, @Bulkhead,
  @Fallback — protects services from cascading failures.
duration_minutes: 90
is_free: false
video_url: null
sort_order: 15
section_title: 'Part 5: Resilience & Observability'
course:
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="60" y="140" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">Lesson 16: Fault Tolerance — Circuit Breaker,</tspan>
  ...
  <!-- Section -->
  <text x="60" y="268" font-family="system-ui,-apple-system,sans-serif" font-size="13" fill="#64748b" opacity="0.6">Part 5: Resilience & Observability</text>
...
1. Add `@Retry` + `@Timeout` for Product Service REST Client calls
2. Implement `@CircuitBreaker` with fallback returns cached data
3. Add `@Bulkhead` Limit concurrent calls to Product Service
4. Create `ResilientProductClient` wrapper with all patterns
5. Create `ResilientPaymentClient` with fallback queue into Kafka
6. Override config via `application.properties`
7. Add Micrometer metrics — verify metrics at `/q/metrics`
8. Test: mock Product Service down → verify retry → circuit open → fallback
9. (Advanced) Create Grafana dashboard for Fault Tolerance metrics

## Summary

- **`@Retry`** — automatic retry, exponential backoff support
- **`@Timeout`** — limits processing time, prevents thread from being blocked
- **`@CircuitBreaker`** — circuit break when service fails too much (CLOSED → OPEN → HALF-OPEN)
- **`@Bulkhead`** — limit concurrent calls, avoid resource exhaustion
- **`@Fallback`** — returns cached/default data when all fail, or queue retry
- **`@RateLimit`** — limit request rate (requests/window)
- **Execution order**: Bulkhead → CircuitBreaker → Retry → Timeout → Method → Fallback
...
- **Config override** passed `application.properties` — no need to change code
