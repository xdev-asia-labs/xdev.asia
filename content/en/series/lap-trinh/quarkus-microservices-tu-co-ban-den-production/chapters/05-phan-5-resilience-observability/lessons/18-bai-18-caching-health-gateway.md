...
title: 'Lesson 18: Caching, Health Checks & API Gateway'
slug: bai-18-caching-health-checks-api-gateway
description: >-
  Redis caching with @CacheResult, Health Checks (liveness/readiness), API
  Gateway pattern, rate limiting, aggregation.
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="92" y="69" font-family="system-ui,-apple-system,sans-serif" font-size="13" font-weight="600" fill="#a78bfa">💻 Programming — Lesson 17</text>

  <!-- Title -->
  <text x="60" y="140" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">Lesson 18: Caching, Health Checks & API</tspan>
      <tspan x="60" dy="42">Gateway</tspan>
  </text>
...
This article completes the microservices platform with 3 cross-cutting concerns: **Redis Caching** reduces database load, **Health Checks** for Kubernetes readiness/liveness probes, and **API Gateway** as a single entry point for the frontend.
...
Quarkus automatically adds:
- **Database** (datasource): database connection OK?
...
1. Add Redis caching for Product Service (`getById`, `listByCategory`)
2. Implement cache invalidation when creating/update/delete product
3. Create custom Health Checks: database, external service
4. Configure Nginx API Gateway routing to services
5. Add rate limiting in Nginx
6. Test: startup → readiness → liveness probes

## Summary

- **Redis Cache** (`@CacheResult`, `@CacheInvalidate`) reduces DB load
- **Health Checks** (`/q/health/live`, `/q/health/ready`) for Kubernetes probes
...
Next article: Testing — @QuarkusTest & Testcontainers.
