...
description: >-
  ...
  JwtAuthenticationFilter, refresh token flow. Stateless session management for
  REST API.
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="92" y="69" font-family="system-ui,-apple-system,sans-serif" font-size="13" font-weight="600" fill="#c084fc">💻 Programming — Lesson 9</text>

  <!-- Title -->
  <text x="60" y="140" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">Lesson 10: JWT Authentication — Stateless</tspan>
      <tspan x="60" dy="42">Security for REST APIs</tspan>
  </text>
...
HTTP Basic Authentication requires sending credentials on each request — not suitable for production REST APIs. JWT (JSON Web Token) allows stateless authentication: the server does not need to save the session, all information is in the token. This is the most popular authentication standard for REST APIs and SPAs.
...
### 2.5 Security Configuration with JWT
...
- JWT allows stateless authentication: the server does not save the session or any information in the token
- JwtAuthenticationFilter checks and validates the token for each request, setting SecurityContext if valid
- Refresh token flow allows renewing access tokens without needing to log in again

## Exercises

1. Full implementation of JWT authentication flow: register, login, refresh token, access protected endpoint
2. Add roles to JWT claims, configure authorization to distinguish USER and ADMIN endpoints
3. Implement token blacklist (stored in Redis or in-memory) to revoke tokens when user logs out
