...
description: >-
  Model optimization: quantization, pruning, knowledge distillation. ONNX export
  & runtime. TensorRT for NVIDIA GPUs. TFLite for mobile. CoreML for iOS. Jetson
  Nano, Raspberry Pi deployment. Benchmarking & profiling.
duration_minutes: 150
is_free: true
video_url: null
sort_order: 12
section_title: 'Part 5: Deployment & Capstone'
course:
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 340" style="max-width: 100%; height: auto; border-radius: 12px; margin-bottom: 1.5rem;">
  ...
  <text x="60" y="140" font-family="system-ui,-apple-system,sans-serif" font-size="34" font-weight="700" fill="#f1f5f9">
      <tspan x="60" dy="0">Lesson 13: Edge Deployment — TensorRT, ONNX &</tspan>
      <tspan x="60" dy="42">Mobile</tspan>
  </text>
  ...
  <!-- Section -->
  <text x="60" y="268" font-family="system-ui,-apple-system,sans-serif" font-size="13" fill="#64748b" opacity="0.6">Part 5: Deployment & Capstone</text>
...
You have trained a good model — but **where to deploy?** Laptops with GPUs are easy, but in reality the model needs to run on:

- 📱 **Mobile** (Android/iOS) — 2-4GB RAM, no GPU
- 🤖 **Edge devices** (Jetson Nano, Raspberry Pi) — low power
- ☁️ **Cloud** but requires **low latency** (<50ms)

This lesson teaches you the entire process of **optimize → convert → deploy** CV model to production.

> 🎯 **Goal:** Reduce model from 100MB → 10MB, inference from 500ms → 20ms.

---

## 1. Why is Edge Deployment needed?
...
| **Bandwidth** | Consuming bandwidth | No need |
...
### 2.2 Pruning — Pruning unimportant neurons
...
| Concepts | Remember |
|--------|--------|
| **Quantization** | FP32→FP16→INT8, reduced 2-4x size |
| **Pruning** | Cut weights ~0, reduce computation |
| **Distillation** | Big models teach small models |
| **ONNX** | General format, ONNX Runtime is 2-3x faster |
| **TensorRT** | NVIDIA GPU, 3-5x faster, FP16/INT8 |
| **TFLite** | Android/Raspberry Pi, INT8 |
| **CoreML** | iOS/macOS, Neural Engine |

## General exercises

1. **ONNX Pipeline:** Export YOLOv5s → ONNX → ONNX Runtime. Compare latency vs PyTorch native.
2. **Quantization Test:** Apply FP16 + INT8 quantization to MobileNet. Measure accuracy drop on ImageNet validation.
3. **Edge Deploy:** Deploy model classification onto Raspberry Pi (or CPU-only laptop with TFLite). Target: >10 FPS.
4. **Full Benchmark:** Benchmark the same model in 3 formats (PyTorch, ONNX, TensorRT). Create a latency/FPS/accuracy comparison table.

> **Next article:** Capstone — Building an End-to-End CV system, synthesizing all knowledge.
