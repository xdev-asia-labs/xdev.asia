...
description: >-
  Function calling API (OpenAI, Anthropic). Tool definition, schema design.
  ReAct pattern implementation. Tool selection, error handling, retry logic.
  Custom tools: web search, database query, API calls, code execution. Practice
...
> **LLM has no arms. Tool calling is those hands.** A pure LLM only knows how to generate text — it cannot search Google, query the database, or call APIs. But when you equip **Function Calling** (or **Tool Use**), LLM can suddenly interact with the real world: look up real-time information, manipulate data, run code, send emails. This is the leap from **chatbot** to **agent**. In this article, we will delve into all aspects: Function Calling API of OpenAI and Anthropic, how to design an effective tool schema, implement the **ReAct pattern** from scratch, build a set of production-ready custom tools, and finally put it all together into a complete **Research Agent**.

---

## 1. What is Function Calling?

### 1.1. Evolution: From Text Completion to Tool Use

The development of LLM interaction through stages:
...
These two terms are often used interchangeably, but there are slight differences:

| Aspect | Function Calling | Tool Calling |
|--------|-----------------|-------------|
| **Original term** | OpenAI (June 2023) | OpenAI rename (November 2023) |
| **Scope** | Call a function | Call multiple tools (functions + code_interpreter + retrieval) |
| **API field** | `functions` (deprecated) | `tools` (current) |
| **Parallel** | No | Yes (multiple tool calls per turn) |
| **Providers** | OpenAI original | OpenAI, Anthropic, Google, Mistral, ... |

> **Note:** In this article, we use "Tool Calling" and "Function Calling" interchangeably. When talking specifically about the API, we will use the correct parameter name of that provider.
...
> **Warning:** LLM does not execute any code. It just generates a JSON object specifying "I want to call function X with arguments Y". Your code parses that JSON, runs the function, then sends the results back to LLM. This is an important security point — you always control execution.
...
OpenAI uses **JSON Schema** to define tools. Each tool includes: `name`, `description`, `parameters`.
...
> **Tip:** `description` is the **most important** element in tool definition. LLM relies on the description to decide whether and when to call this tool. Writing a good description = agent works more accurately. We will deep dive in part 4.
...
From GPT-4o onwards, LLM can call **multiple tools at the same time** in 1 turn:
...
### 2.4. `tool_choice` Parameters

| Value | Behavior | Use cases |
|-------|----------|----------|
| `"auto"` | LLM decides whether to call the tool or not | Default, most popular |
| `"required"` | LLM **must** call at least 1 tool | When you know for sure that you need a tool |
| `"none"` | LLM **must not** call tool | Force text-only response |
| `{"type": "function", "function": {"name": "get_weather"}}` | Force to call this tool correctly | Force specific tool |
...
Anthropic (Claude) uses a similar format but has some differences:
...
### 3.3. Compare OpenAI vs Anthropic Tool Calling

| Features | OpenAI (GPT-4o) | Anthropic (Claude 3.5+) |
|--------|-----------------|------------------------|
...
| **Parallel calling** | Yes (native) | Yes (from Claude 3.5) |
| **Forced tool** | `tool_choice: {type, function}` | `tool_choice: {type: "tool", name: "..."}` |
| **Streaming** | Yes (delta chunks) | Yes (event stream) |
| **Stop reason** | `finish_reason: "tool_calls"` | `stop_reason: "tool_use"` |
| **Nested objects** | Good support | Good support |
| **Max tools** | 128 | 128 (Claude 3.5) |

> **Tip:** When building an agent that supports multiple LLM providers, create an abstraction layer to normalize the tool calling format. Will cover in lesson 14 (LangChain/LlamaIndex).
...
**Tool description** is the deciding factor in whether LLM chooses the right tool. Like prompt engineering, **description engineering** is an important skill.
...
|--------|--------|-------|
| **What it does** | Main function description | "Search the product catalog" |
| **What it returns** | Description output format | "Returns name, price, availability" |
| **When to use** | When should LLM call this tool | "Use when user asks about products" |
| **When NOT to use** | When not to call | "Don't use for order tracking" |
| **Examples** | Specific input example | "'laptop under $1000', 'Nike size 42'" |
| **Limitations** | Limit | "Max 50 results per query" |

### 4.3. Parameter Validation with Pydantic

Use **Pydantic** to validate tool arguments before executing:
...
### 4.4. Error Handling in Tool Responses

When the execution tool fails, send a structured error message to LLM so it can handle it:
...
> **Tip:** Always return error message **in natural language** for LLM. LLM reads the error message and decides on its own: retry, try another tool, or notify the user. Don't return raw exceptions.
...
**ReAct** (Reasoning + Acting) is a pattern introduced in the paper *"ReAct: Synergizing Reasoning and Acting in Language Models"* (Yao et al., 2022). Core idea: LLM **alternates** between thought and action, instead of just blindly acting.
...
### 5.3. ReAct Implementation from Scratch
...
Combine all tools into one easy-to-manage **Toolbox**:
...
### 7.1. How does LLM choose Tools?

LLM relies on 3 factors to choose a tool:
...
| Scenario | `tool_choice` | Reason |
|----------|--------------|-------|
| General Q&A agent | `"auto"` | LLM decides whether to search or answer from knowledge |
| Data extraction pipeline | `"required"` | Every query needs a tool (extract structured data) |
| Classification step | `"none"` | Just LLM classify, no tool needed |
| Weatherbot | `{"name": "get_weather"}` | Every query needs the weather tool |
| Multi-step agent, step 1 | `"required"` | Force agent to act, not skip |
| Multi-step agent, final step | `"none"` | Force agent synthesizes and responds |
...
### 8.3. Error Message Format for LLM
...
Put it all together into a complete **Research Agent** — the agent can search the web, read web content, calculate, and synthesize information.
...
When running the above agent, the reasoning trace will look like this:
...
1. **Tool Calling = LLM's hands** — LLM generate JSON specify tool name + arguments, your code executes. LLM never directly runs code.
2. **OpenAI used `tools` + `parameters`**, Anthropic used `tools` + `input_schema`. The format is different but the concept is the same. Build abstraction layer (Toolbox) to support both.
3. **Description engineering** is equally important as prompt engineering. Tool description determines whether LLM chooses the right tool. Include: what it does, when to use, when NOT to use, examples.
4. **Pydantic validation** — always validate tool arguments before executing. Prevent injection, ensure type safety, catch errors early.
5. **ReAct pattern** (Thought → Action → Observation loop) creates clear reasoning traces, helps debugging and increases accuracy compared to "blind" tool calling.
6. **Custom tools** should follow class pattern: `name`, `description`, `schema`, `__call__`. Use **Toolbox** class to manage and automatically convert formats for many providers.
7. **`tool_choice`** strategy: `auto` for general use, `required` when you definitely need a tool, specific tool when only one option.
8. **Error handling is non-negotiable** — retry with exponential backoff, fallback chains, structured error messages for LLM to read. Production agent MUST handle tool failures gracefully.
9. **Security first**: allowlist inputs for API tools, validate SQL (read-only), sandbox code execution, sanitize. Tool calling opens attack surface — protect it.
...
**Lesson 14: LangChain & LlamaIndex — Agent Frameworks** — instead of building everything from scratch like this lesson, we will use the two most popular frameworks to build agents faster. You will learn: LangChain Agent architecture (AgentExecutor, Tools, Memory), LlamaIndex agent (ReAct Agent, Query Engine Tools), detailed comparison of the two frameworks, and how to choose the right framework for each use case. From the Research Agent self-build, we will rebuild it with LangChain in 50 lines of code.
