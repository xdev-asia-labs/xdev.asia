...
description: >-
  Capstone project: build a complete AI Agent system. Requirements analysis,
  architecture design. RAG + Tool Calling + Multi-Agent. Conversation
  management, streaming responses. Error handling, fallback strategies. Testing
  AI Agents.
...
> **You've learned the concepts, tools, memory, planning, multi-agent — now it's time to put it ALL together into a real product.** In this capstone lesson, we'll build an **AI Research Assistant** — a production-grade agent system capable of reading documents, searching the web, running code, synthesizing information, and answering complex questions. Not a demo toy — but a project that can be cloned, run, and deployed. 240 minutes, ~1500 lines of code, a complete product.
...
**AI Research Assistant** is an agent that helps researchers:

- **Upload documents** (PDF, text, web URL) to the knowledge base
- **Q&A** based on knowledge base (RAG)
- **Search web** to add the latest information
- **Run Python code** to analyze data and calculate
- **Synthesize** multiple sources into a coherent answer
- **Remember context** through many conversations

### 1.2. Capabilities Matrix

| Capability | Tools/Modules | When to use |
|-----------|-----------|-------------|
| Read knowledge base | RAG Search (ChromaDB) | User asks about uploaded content |
| Find new information | Web Search (Tavily) | Need real-time data, in addition to KB |
| Run code | Python Executor (sandbox) | Calculate, analyze, draw charts |
| Summary | Summarizer | Long answer, multi-source |
| Remember the context | Chat History (SQLite) | Every conversation |
...
| Layers | Technology | Reason for choosing |
|-------|-----------|-----------|
| LLM | OpenAI GPT-4o / Ollama (local) | Flexible: cloud or local |
| Agent Framework | LangGraph | State machine, conditional routing |
| Vector DB | ChromaDB | Lightweight, embedded, easy to setup |
| Embeddings | OpenAI/sentence-transformers | High quality for RAG |
| Web Search | Tavily API | Specializing in AI agents |
...
## 6. Step 3 — Agent Implementation with LangGraph
...
Why use? `TypedDict` + `Annotated`? This is the standard LangGraph pattern:
...
Streaming is critical for UX — users don't want to wait 30 seconds for a long answer:
...
Streaming is handled at the API layer (Section 8).
...
### 9.3. Fallback in System Prompt

The agent is instructed to handle gracefully when the tool fails:
...
### 10.2. Integration Test for Agent
...
## 11. Docker Compose — Run the complete system
...
Create `data/sample_docs/sample_paper.txt` to test:
...
This capstone article has completely built **AI Research Assistant** — a production AI Agent system:

- ✅ **Architecture Design** — clear system design with clear separation of concerns
- ✅ **Knowledge Base** — ingestion pipeline for PDF, web URL, plain text → chunks → ChromaDB
- ✅ **4 Tools** — RAG search, web search (Tavily), sandboxed code execution, summarizer
- ✅ **LangGraph Agent** — state machine with conditional routing, tool calls, iteration safety
- ✅ **Conversation Management** — SQLite chat history, session management
- ✅ **Streaming API** — FastAPI with Server-Sent Events for real-time response
- ✅ **Error Handling** — retry with backoff, graceful degradation, fallback chains
- ✅ **Testing** — unit tests, integration tests, API tests with mocking
- ✅ **Docker Compose** — one-command deployment
- ✅ **Complete code** — can clone and run immediately

Component summary table:

| Components | File(s) | Line Code |
|-----------|---------|-----------|
| Config | `config.py` | ~35 |
| Extractors | `extractors.py` | ~55 |
| Chunkers | `chunker.py` | ~25 |
...
### Exercise 1: Adding Document Management (Medium)

Add endpoints:

- `GET /api/v1/documents` — list of all uploaded documents (excerpt from ChromaDB metadata)
- `DELETE /api/v1/documents/{name}` — remove documents from the knowledge base
- Filter for RAG search: only search in specific documents

### Exercise 2: Adding Citation Sources (Medium)

Expand the response format to return structured citations:
...
### Exercise 3: Multi-User Support (Advanced)

- Add simple API key authentication
- Each user has its own knowledge base (separate ChromaDB collection)
- Session isolation per user

### Exercise 4: Frontend UI (Advanced)

Build a simple chat UI with:

- React/Next.js or plain HTML + JavaScript
- SSE streaming displays tokens real-time
- Upload UI files
- Session list sidebar

### Exercise 5: Observability (Advanced)

Integrate **LangSmith** or **Langfuse** to:
