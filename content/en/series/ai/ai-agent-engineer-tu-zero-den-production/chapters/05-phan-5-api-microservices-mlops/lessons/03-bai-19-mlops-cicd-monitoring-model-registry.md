...
description: >-
  MLOps fundamentals: model versioning, experiment tracking (MLflow, W&B). CI/CD
  pipeline for AI: GitHub Actions, testing strategies. Model registry.
  Monitoring: latency, quality metrics, drift detection. Logging & observability
  (LangSmith).
...
> You push a new model to production at 5pm on Friday — response quality drops 30% and no one knows until Monday. No experiment tracking, no model registry, no monitoring. MLOps is the discipline that helps you ship AI models safely, reproducibly, and always know what's happening in production.

## 1. What is MLOps?

### 1.1. DevOps + ML-specific challenges

**MLOps** (Machine Learning Operations) is a set of practices to deploy and maintain ML/AI models in production in a **reliable** and **efficient** way.

Core differences compared to traditional DevOps:
...
| Challenge | DevOps | MLOps |
|---|---|---|
| Main Artifact | Docker image / binary | Model weights + code + config |
...
Google recommends 5 levels (0–4) for MLOps maturity:
...
> **Goal of this lesson:** Take you from Level 0 to Level 2–3 for AI Agent projects.

## 2. Experiment Tracking with MLflow

### 2.1. Why is Experiment Tracking needed?

Without experiment tracking, you will:
- Forgot which params give the best results
- Don't know which model is running in production
- Cannot reproduce results from 2 weeks ago
- Arguing "my model is 95% accurate" but no one can verify it
...
### 2.3. Logging experiments for AI Agent
...
After running experiments, visit `http://localhost:5000`:
...
| Features | MLflow | W&B |
|---|---|---|
| Hosting | Self-hosted or Databricks | Cloud SaaS (free tier) |
| UI/UX | Functional | More beautiful, more interactive |
| Collaboration | Basic | Real-time team features |
| Hyperparameter sweeps | Not built-in | Built-in sweep agent |
| System metrics | Manual logging | Auto GPU/CPU/Memory |
...
### 3.3. Hyperparameter Sweep with W&B
...
### 4.1. Why do we need Model Registry?
...
### 6.1. Why do we need DVC?

Git tracks code well, but **does not track large data** (model files, datasets, embeddings):
...
| Data privacy | Data on LangChain cloud | Full control (self-hosted) |
...
def retrieve_context(query: str) -> list[str]:
    """RAG retrieval step — automatically traced."""
    # Suppose vector search
...
def generate_response(query: str, context: list[str]) -> str:
    """LLM generation step — automatically traced."""
    context_text = "\n".join(context)
    response = client.chat.completions.create(
        model="gpt-4o-mini",
        messages=[
...
def rag_pipeline(query: str) -> str:
    """Full RAG pipeline — parent trace contains both retrieve + generate."""
    ...
    langfuse_context.score_current_trace(
        name="user_feedback",
        value=1, # or from user feedback
        comment="Auto-scored"
    )
    return response
```

Results in Langfuse UI:
...
✅ **MLOps** = DevOps + data, model, and model quality management — maturity levels 0–4

✅ **Experiment Tracking**: MLflow (self-hosted, open source) or W&B (cloud SaaS) — log everything for reproducible
...
✅ **DVC**: Version data + model files with git, push/pull from S3/GCS
...
✅ **Observability**: Langfuse or LangSmith — trace each step in the chain, debug production issues

✅ **Alerting**: Rules cho latency spike, error rate, cost spike, quality degradation

✅ **MLOps Stack**: Docker Compose running MLflow + Langfuse + Prometheus + Grafana with AI Agent

## Exercise

### Exercise 1: MLflow Experiment Tracking (30 minutes)

1. Install MLflow and run `mlflow ui`
2. Write a script to call OpenAI API (or mock) with 3 different sets of params (temperature, system prompt)
3. Log params, metrics (accuracy, latency), and artifacts (results JSON) into MLflow
4. Compare 3 runs in MLflow UI, choose the best run

### Exercise 2: CI/CD Pipeline (45 minutes)

1. Create GitHub Actions workflow for AI agent project with 3 stages:
   - Lint + unit tests
- Model evaluation (using mock API or small test dataset)
- Quality gate: fail if accuracy < 80% or latency > 3s
2. Push code and verify pipeline run correctly
3. Try changing the code to make quality gate fail, confirm pipeline blocks deployment

### Exercise 3: Monitoring Dashboard (45 minutes)

1. Add Prometheus metrics to FastAPI app (request count, latency histogram, token counter)
2. Setup Prometheus + Grafana using Docker Compose
3. Create Grafana dashboard with:
   - Request rate (req/s)
   - Latency percentiles (p50, p95, p99)
   - Token usage over time
   - Error rate
4. Send simulated traffic and verify dashboard updates in real-time

### Exercise 4: Langfuse Tracing (30 minutes)

1. Deploy Langfuse locally using Docker Compose
2. Instrument a simple RAG pipeline (retrieve → generate) with `@observe()`
3. Submit 5–10 queries, open Langfuse UI to see traces
4. Find which query has the highest latency, analyze which steps are slow
