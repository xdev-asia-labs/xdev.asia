...
description: >-
  ...
  Streaming responses (SSE). WebSocket for real-time chat. File upload
  processing. Authentication, rate limiting. OpenAPI documentation.
...
> You've built a smart AI Agent — but how can *millions of users* call it over HTTP? FastAPI is the fastest, async-native, self-generating Python framework OpenAPI docs — perfect stack for AI services production.

## 1. Why FastAPI for AI Services?

### 1.1. Actual problem

When deploying AI services, you need to address:

- **Streaming output** — LLM generates text for each token, users need to see it in real-time
- **Concurrent requests** — multiple users call inference at the same time, do not block each other
- **Input validation** — reject bad requests before consuming GPU
- **Auto documentation** — the frontend team reads the API spec themselves, no meetings needed

### 1.2. Compare frameworks

| Criteria | FastAPI | Flask | Django REST | Express.js |
|---|---|---|---|---|
| Async native | ✅ Built-in | ❌ WSGI | ❌ WSGI | ✅ |
| Auto OpenAPI | ✅ Self-generating | ❌ Plugins | ⚠️ DRF | ❌ Plugins |
...
> **Conclusion:** FastAPI = async + validation + docs + Python AI ecosystem → best choice for AI services.

### 1.3. Overview architecture
...
Dependency Injection (DI) is the core pattern in FastAPI — managing the lifecycle of LLM clients, DB connections, auth:
...
### 3.3. Benefits of Pydantic validation
...
> **Note:** Pydantic v2 validates 5-50x faster than v1 thanks to the core written in Rust. Always use Pydantic v2 for AI services.

## 4. Async/Await for AI Inference

### 4.1. Why is Async important?

LLM inference takes 2-30 seconds per request. With sync server, each worker is blocked until inference is completed:
...
> **Important:** Use `async def` for I/O-bound (API calls, DB queries). Use `def` (sync) for CPU-bound (numpy, image processing) — FastAPI runs automatically in the threadpool.

## 5. Streaming Responses with SSE

### 5.1. What are Server-Sent Events?

SSE allows the server to continuously push data to the client over a single HTTP connection:
...
## 6. WebSocket for Real-time Chat

### 6.1. SSE vs WebSocket

| Criteria | SSE | WebSockets |
|---|---|---|
| Communication direction | Server → Client (1 way) | Bidirectional (2-way) |
| Protocols | HTTP | WS (upgrade from HTTP) |
| Reconnect | Auto(EventSource) | Manual |
| Use cases | LLM streaming output | Interactive chat, collaborate |
| Overhead | Low | Average |
...
### 9.2. Apply rate limit per endpoint
...
### 9.3. Tiered rate limiting according to API key
...
Used for light tasks (< 30 seconds), no tracking needed:
...
### 10.2. Long-running jobs with status tracking

For heavy tasks (RAG ingestion, batch inference), use job queue pattern:
...
Kubernetes deployment config uses health checks:
...
### 12.1. Automatically generate documents

FastAPI automatically generates OpenAPI schema from type hints and Pydantic models:
...
Below is complete `main.py` Combine all patterns:
...
### 13.2. Dockerfile for production
...
| WebSockets | `@router.websocket("/ws")` | Bidirectional real-time chat |
| Dependencies | `Depends(get_openai)` | Inject services, auth |
| Background tasks | `background_tasks.add_task(fn)` | Logging, DB writes |
...
- ✅ **Pydantic models** — type-safe request/response validation for AI endpoints
- ✅ **Async/Await** — non-blocking inference, concurrent API calls
- ✅ **SSE streaming** — real-time LLM output to client
- ✅ **WebSocket** — bidirectional real-time chat
- ✅ **File upload** — document processing for RAG ingestion
- ✅ **Authentication** — JWT tokens + API keys
- ✅ **Rate limiting** — protect AI endpoints by tier
- ✅ **Background tasks** — async job processing
- ✅ **Error handling** — structured errors, health checks for K8s
- ✅ **OpenAPI docs** — auto-generated, production-ready documentation

All of these patterns combine into a **production-grade AI service** — ready to deploy to Kubernetes.

## Exercises

### Exercise 1: Multi-model Chat API
Build a FastAPI service that supports both OpenAI and Anthropic:
- Endpoints `/chat` receive field `provider: "openai" | "anthropic"`
- Use Dependency Injection to select the corresponding client
- Implement SSE streaming for both providers
- Add fallback: if main provider fails → automatically switch to the other provider

### Exercise 2: RAG Upload Pipeline
...
- `GET /documents/{id}/status` — job tracking with progress percentage
- Add rate limit: 5 uploads/minute, 60 searches/minute

### Exercise 3: Production Hardening
Add production features to AI service:
- Implement tiered rate limiting: free (10/min), pro (100/min), enterprise (1000/min) based on API key
- Add request/response logging middleware (log model, latency, token count)
- Implement circuit breaker: if LLM provider fails 5 times in a row → pay 503 for 60 seconds
- Write tests with `httpx.AsyncClient` and `pytest-asyncio` for all endpoints
- Dockerize and test with `docker compose up`
