...
import { getAISeries, getAISeriesSlugs, formatDate, getAuthorById } from "@/lib/data";
import { getValidImageUrl } from "@/utils/image";
import ContentRenderer from "@/components/ContentRenderer";
import TableOfContents from "@/components/TableOfContents";
import { IconChevronRight, IconBook, IconClock, IconEye, IconStar, IconBrain } from "@/components/Icons";

export const dynamicParams = false;

export function generateStaticParams() {
    return getAISeriesSlugs().map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
    const { slug } = await params;
    const series = getAISeries(slug);
    if (!series) return {};
    return {
...
export default async function AISeriesDetailPage({ params }: { params: Promise<{ slug: string }> }) {
    const { slug } = await params;
    const series = getAISeries(slug);
    if (!series) notFound();
                                            ...
                                            <ul>
                                                {section.lessons.map((lesson, lIdx) => (
                                                    <li key={lesson.id} className={lIdx < section.lessons.length - 1 ? "border-b border-zinc-50" : ""}>
                                                        <Link href={`/ai/lessons/${series.slug}/${lesson.slug}/`} className="flex items-center gap-3 px-5 py-3 text-sm hover:bg-brand-50 transition-colors group">
