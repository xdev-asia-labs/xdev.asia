        ...
        <div className="max-w-3xl mx-auto px-4 py-8">
            ...
            <div className="mb-6">
                ...
                <div className="h-1.5 bg-zinc-100 dark:bg-zinc-800 rounded-full overflow-hidden">
                    <div
                        className="h-full bg-linear-to-r from-brand-500 to-indigo-500 rounded-full transition-all duration-500"
            ...
            <div className="flex flex-wrap gap-1.5 mb-6">
                {quiz.questions.map((_, idx) => {
                    ...
                    const isRight = answered && answers[idx] === quiz.questions[idx].correct;
                    const isCurrent = idx === currentQuestion;
                    let dotClass = "bg-zinc-200 dark:bg-zinc-700";
                    if (answered) dotClass = isRight ? "bg-emerald-500" : "bg-rose-400";
                    if (isCurrent) dotClass += " ring-2 ring-brand-400 ring-offset-2 dark:ring-offset-zinc-900";
                    return (
                        <button
                            key={idx}
                            onClick={() => goToQuestion(idx)}
                            className={`w-7 h-7 rounded-lg text-[10px] font-bold transition-all ${dotClass} ${answered ? "text-white" : "text-zinc-500 dark:text-zinc-400"}`}
                        >
                            {idx + 1}
                        </button>
                    );
            ...
            {/* Question */}
            <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-100 dark:border-zinc-800 p-6 md:p-8 mb-6">
                ...
                <div className="space-y-3">
                    {question.options.map((option, idx) => {
                        ...
                        let optionClass = "border-zinc-200 dark:border-zinc-700 hover:border-brand-300 hover:bg-brand-50/50 dark:hover:bg-brand-900/10 cursor-pointer";
                        if (showResult) {
                            if (isCorrectOption) {
                                optionClass = "border-emerald-400 bg-emerald-50 dark:bg-emerald-900/20 dark:border-emerald-600";
                            } else if (isSelected && !isCorrectOption) {
                                optionClass = "border-rose-400 bg-rose-50 dark:bg-rose-900/20 dark:border-rose-600";
                            } else {
                                optionClass = "border-zinc-100 dark:border-zinc-800 opacity-50 cursor-default";
                            }
                        }
                        ...
                        return (
                            <button
                                ...
                                className={`w-full text-left flex items-start gap-3 p-4 rounded-xl border transition-all duration-200 ${optionClass}`}
                            >
                                <span className={`shrink-0 w-7 h-7 rounded-lg flex items-center justify-center text-xs font-bold ${showResult && isCorrectOption
                                ...
                                <span className="text-sm text-zinc-700 dark:text-zinc-300 pt-0.5">{option}</span>
                                {showResult && isCorrectOption && (
                                    <IconCheckCircle size={18} className="shrink-0 ml-auto text-emerald-600 mt-0.5" />
                                )}
                                {showResult && isSelected && !isCorrectOption && (
                                    <IconXCircle size={18} className="shrink-0 ml-auto text-rose-500 mt-0.5" />
                                )}
                            </button>
                        );
                    })}
                ...
                {showExplanation && (
                    <div className={`mt-6 p-4 rounded-xl border ${isCorrect
                        ? "border-emerald-200 bg-emerald-50/80 dark:bg-emerald-900/10 dark:border-emerald-800"
                        : "border-amber-200 bg-amber-50/80 dark:bg-amber-900/10 dark:border-amber-800"
                        }`}>
                        <p className={`text-xs font-bold uppercase tracking-wider mb-1 ${isCorrect ? "text-emerald-600" : "text-amber-600"}`}>
            ...
            <div className="flex items-center justify-between">
                <button
                    onClick={() => { setCurrentQuestion(Math.max(0, currentQuestion - 1)); setShowExplanation(answers[Math.max(0, currentQuestion - 1)] !== null); }}
                ...
                {selectedAnswer !== null && (
                    <button
                        ...
                        className="flex items-center gap-2 px-6 py-2.5 rounded-xl font-semibold text-white bg-linear-to-r from-brand-500 to-indigo-500 shadow-md hover:shadow-lg transition-all"
                    >
                        {currentQuestion < totalQuestions - 1 ? (
                            <>Câu tiếp <IconArrowRight size={14} /></>
                        ) : (
                            <>Xem kết quả <IconArrowRight size={14} /></>
                        )}
                    </button>
                )}
                ...
                <Link
                    href="/luyen-thi/"
                    className="px-4 py-2 rounded-lg text-sm font-medium text-zinc-400 hover:text-rose-500 transition-colors"
                >
                    Thoát
                </Link>
            </div>
